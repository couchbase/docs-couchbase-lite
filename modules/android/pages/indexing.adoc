...
xref:android:indexing.adoc#ex-create-index[] creates a new index for the `type` and `name` properties, shown in this data model:
...
Couchbase Lite 3.2.2 introduced support for Partial Index - Partial Value and Partial Full-Text Indexes.
...
For details, see xref:android:fts.adoc[].
...
Couchbase Lite 3.2.1 introduced functionality to optimize querying arrays.
xref:android:query-n1ql-mobile.adoc#lbl-unnest[Array `UNNEST`] to unpack arrays within a document to allow joins with the parent object, and array indexes for indexing unnested array's values to allow more efficient queries with `UNNEST`.
