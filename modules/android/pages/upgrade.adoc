...
You can configure the `transports` property on `MultipeerReplicatorConfiguration` to enable Bluetooth Low Energy (BLE) transport.

==== Auto-Switching (Wi-Fi and BLE)

To enable both Wi-Fi and BLE, set both transports.
When both are in range, Couchbase Lite uses Wi-Fi.
