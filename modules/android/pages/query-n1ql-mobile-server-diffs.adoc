...
There are several minor but notable behavior differences between _{sqlpp} for Mobile_ queries and _{sqlpp} for Server_, as shown in  xref:android:query-n1ql-mobile-server-diffs.adoc#tbl-diffs[].
...
xref:android:query-n1ql-mobile-server-diffs.adoc#tbl-logops[] summarizes the result of logical operations with different operand values and also shows where the Couchbase Server behavior differs.
