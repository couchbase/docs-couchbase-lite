...
[#android:query-n1ql-mobile-server-diffs:::]
...
There are several minor but notable behavior differences between _{sqlpp} for Mobile_ queries and _{sqlpp} for Server_, as shown in  <<android:query-n1ql-mobile-server-diffs:::tbl-diffs>>.
...
[#android:query-n1ql-mobile-server-diffs:::tbl-diffs, cols="^1,2a,2a, options="header"]
|===
...
[discrete#android:query-n1ql-mobile-server-diffs:::boolean-logic-rules]
=== Boolean Logic Rules
...
[discrete#android:query-n1ql-mobile-server-diffs:::logical-operations]
==== Logical Operations
...
<<android:query-n1ql-mobile-server-diffs:::tbl-logops>> summarizes the result of logical operations with different operand values and also shows where the Couchbase Server behavior differs.

.Logical Operations Comparison
[#android:query-n1ql-mobile-server-diffs:::tbl-logops, cols="^1m,^1m,^1m,^1m,^1m,^1m,^1m", options-"header"]
|===
...
[discrete#android:query-n1ql-mobile-server-diffs:::crud-operations]
=== CRUD Operations
{sqlpp} for Mobile only supports Read or Query operations.

{sqlpp} for Server fully supports CRUD operation.

[discrete#android:query-n1ql-mobile-server-diffs:::functions]
=== Functions

[discrete#android:query-n1ql-mobile-server-diffs:::division-operator]
==== Division Operator
...
[discrete#android:query-n1ql-mobile-server-diffs:::round-function]
==== Round Function
[,cols="1a,1a", options="header"]
|===
^| {sqlpp} for Server ^| {sqlpp} for Mobile
...
[discrete#android:query-n1ql-mobile-server-diffs:::related-content]
=== Related Content
++++
...
[discrete.colum#android:query-n1ql-mobile-server-diffs:::-2n]
==== {empty}
.Learn more . . .
...
[discrete.colum#android:query-n1ql-mobile-server-diffs:::-3n]
==== {empty}
.Dive Deeper . . .
