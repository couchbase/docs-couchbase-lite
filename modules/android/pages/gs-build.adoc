...
An application that enables minification must ensure that certain pieces of pass:q,a[Couchbase{nbsp}Lite] library code are not changed -- see <<ruleset>> for a near-minimal rule set that retains the needed code:
