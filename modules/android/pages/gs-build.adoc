...
An application that enables minification must ensure that certain pieces of Couchbase{nbsp}Lite library code are not changed -- see xref:android:gs-build.adoc#ruleset[] for a near-minimal rule set that retains the needed code:
