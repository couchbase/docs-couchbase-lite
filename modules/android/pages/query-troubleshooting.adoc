...
==== Output
The output from `https://docs.couchbase.com/mobile/{version-maintenance-android}/couchbase-lite-android/com/couchbase/lite/Query.html#explain--[explain()]` remains the same whether invoked by an app, or `cblite` -- see <<android:query-troubleshooting:::qe-output>> for an example of how it looks.
