...
Related Content -- xref:android:new-logging-api.adoc[Using Logs] | xref:android:querybuilder.adoc#lbl-predquery[Predictive Queries] | xref:android:query-live.adoc[Live Queries] | xref:android:indexing.adoc[Indexing]
--
...
To examine how your query is working, either embed the call inside your app (see: xref:android:troubleshooting-queries.adoc#use-qe-app[]), or use it interactively within a `cblite` shell (see: xref:android:troubleshooting-queries.adoc#use-qe-cblite[]).
...
[#output]
=== Output
The output from `https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-android}{empty}/couchbase-lite-android/com/couchbase/lite/Query.html#explain--[explain()]` remains the same whether invoked by an app, or `cblite` -- see xref:android:troubleshooting-queries.adoc#qe-output[] for an example of how it looks.
...
This output (xref:android:troubleshooting-queries.adoc#qe-output[]) comprises three main elements:
...
Something you may well be looking to optimize -- see xref:android:troubleshooting-queries.adoc#ret-method[] for more.
<.> *Grouping method* --- This line shows that the *Group By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see xref:android:troubleshooting-queries.adoc#order-group[].
<.> *Ordering method* -- This line shows that the *Order By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see xref:android:troubleshooting-queries.adoc#order-group[].
====
...
The _retrieval method_ shows the approach decided upon by the optimizer -- see xref:android:troubleshooting-queries.adoc#ret-meths[].
...
So, if your analysis of the xref:android:troubleshooting-queries.adoc#qe-output[Query Explain output] indicates a sub-optimal query and your rewrites fail to sufficiently optimize it, consider compacting the database.
...
In xref:android:troubleshooting-queries.adoc#like-wild-pfx-qry[] we use a wildcard prefix and suffix.
...
By contrast, by removing the wildcard prefix `%` (in xref:android:troubleshooting-queries.adoc#like-no-wild-pfx-qry[]), we see that the query plan's retrieval method changes to become an index search.
