...
There is no limit on the number of Listeners you may configure -- xref:android:p2psync-websocket-using-passive.adoc#simple-listener-initialization[] shows a simple initialization and configuration process.
...
<.> Identify the local database to be used -- see: xref:android:p2psync-websocket-using-passive.adoc#initialize-the-listener-configuration[]

<.> Optionally, choose a port to use.
By default the system will automatically assign a port -- to over-ride this, see: xref:android:p2psync-websocket-using-passive.adoc#lbl-set-network-and-port[]

<.> Optionally, choose a network interface to use.
By default the system will listen on all network interfaces -- to over-ride this see: xref:android:p2psync-websocket-using-passive.adoc#lbl-set-network-and-port[]

<.> Optionally, choose to sync only changes.
The default is not to enable delta-sync -- see: xref:android:p2psync-websocket-using-passive.adoc#delta-sync[].

<.> Set server security.
TLS is always enabled instantly, so you can usually omit this line.
But you _can_, optionally, disable TLS (*not* advisable in production) -- see: xref:android:p2psync-websocket-using-passive.adoc#lbl-tls-security[]
...
Here we show how basic authentication is configured to authenticate the client-supplied credentials from the http authentication header against valid credentials -- see xref:android:p2psync-websocket-using-passive.adoc#lbl-authenticating-the-client[] for more options. +
Note that client authentication is optional.

<.> Initialize the listener using the configuration settings.

<.> xref:android:p2psync-websocket-using-passive.adoc#lbl-start-listener[]
...
== Initialize the Listener Configuration
Initialize the Listener configuration with the local database -- see xref:android:p2psync-websocket-using-passive.adoc#ex-locdb[]
...
[#port-number]
=== Port number
The Listener will automatically select an available port if you do not specify one -- see xref:android:p2psync-websocket-using-passive.adoc#ex-port[] for how to specify a port.
...
When TLS is enabled, Couchbase Lite provides several options on how the Listener may be configured with an appropriate TLS Identity -- see xref:android:p2psync-websocket-using-passive.adoc#configure-tls-identity-for-listener[].
...
* Setting TLS identity to expect self-signed certificate --  -- see: xref:android:p2psync-websocket-using-passive.adoc#ex-create-tls-id[]
* Setting TLS identity to expect anonymous certificate -- see: xref:android:p2psync-websocket-using-passive.adoc#ex-anon-tls-id[]
...
In this section: xref:android:p2psync-websocket-using-passive.adoc#use-basic-authentication[]  |  xref:android:p2psync-websocket-using-passive.adoc#using-client-certificate-authentication[]  |  xref:android:p2psync-websocket-using-passive.adoc#delete-tls-identity[]  |  xref:android:p2psync-websocket-using-passive.adoc#the-impact-of-tls-settings[]
...
To use client-supplied certificates instead -- see: xref:android:p2psync-websocket-using-passive.adoc#using-client-certificate-authentication[]
...
* Active Peers will validate the server certificate corresponding to the TLSIdentity (as long as they are configured to not skip validation -- see xref:android:p2psync-websocket-using-passive.adoc#lbl-tls-security[]).
...
Once you have completed the Listener's configuration settings you can initialize the Listener instance and start it running -- see: xref:android:p2psync-websocket-using-passive.adoc#initialize-and-start-listener[]
...
It is best practice to check the status of the Listener's connections and stop only when you have confirmed that there are no active connections -- see xref:android:p2psync-websocket-using-passive.adoc#get-connection-counts[].
