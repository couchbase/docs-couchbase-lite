...
Couchbase Lite also provides for the direct handling of JSON data implemented in most cases by the provision of a pass:a,q[`toJSON()`] method on appropriate API classes (for example, on MutableDocument, Dictionary, Blob and Array) -- see xref:android:document.adoc#lbl-json-data[].
...
Convert an `ArrayObject` to and from JSON using the pass:a,q[`toJSON()`] and `toArray` methods -- see xref:android:document.adoc#ex-array[].

Additionally you can:

* Initialize a 'MutableArrayObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor -- see: xref:android:document.adoc#ex-array[]
...
You can use `isBlob()` to check whether a given dictionary object is a blob or not -- see xref:android:document.adoc#ex-blob[].
...
This is done using the `init(json)` constructor-- see: xref:android:document.adoc#ex-dictionary[]
...
Convert a `Document` to and from JSON strings using the pass:a,q[`toJSON()`] and pass:a,q[`setJSON()`] methods -- see xref:android:document.adoc#ex-document[].

Additionally you can:

* Initialize a 'MutableDocument' using data supplied as a JSON string.
This is done using the `init(json)` or `init(id: json:)` constructor -- see: xref:android:document.adoc#ex-document[]
