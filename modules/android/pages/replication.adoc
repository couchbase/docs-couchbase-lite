...
Related Content -- xref:android:conflict.adoc[Handling Data Conflicts] | xref:android:dbreplica.adoc[Intra-Device] | <<p2psync-websocket.adoc,Peer-to-Peer>>
--
...
To use this protocol the replication URL should specify WebSockets as the URL scheme (see the xref:android:replication.adoc#lbl-cfg-tgt[] section below).
...
By default, clients accept only servers presenting certificates that can be verified using the OS bundled Root CA Certificates -- see: xref:android:replication.adoc#lbl-svr-auth[].
...
Other options are available -- see: xref:android:replication.adoc#lbl-client-auth[].

<.> Optionally, register an observer, which will notify you of changes to the replication status -- see: xref:android:replication.adoc#lbl-repl-mon[]

<.> Start the replicator  -- see: xref:android:replication.adoc#lbl-repl-start[].
...
+
--
xref:android:replication.adoc#lbl-cfg-tgt[]
|  xref:android:replication.adoc#lbl-cfg-sync[]
|  xref:android:replication.adoc#lbl-cfg-keep-alive[]
|  xref:android:replication.adoc#lbl-user-auth[]
|  xref:android:replication.adoc#lbl-svr-auth[]
|  xref:android:replication.adoc#lbl-client-auth[]
|  xref:android:replication.adoc#lbl-repl-evnts[]
|  xref:android:replication.adoc#lbl-repl-hdrs[]
|  xref:android:replication.adoc#lbl-repl-ckpt[]
|  xref:android:replication.adoc#lbl-repl-fltrs[]
|  xref:android:replication.adoc#lbl-repl-chan[]
|  xref:android:replication.adoc#anchor-auto-purge-on-revoke[]
|  xref:android:replication.adoc#lbl-repl-delta[]
--
...
The REST API provides configurable control over this replication retry logic using a set of configiurable properties -- see: xref:android:replication.adoc#tbl-repl-retry[].
...
When necessary you can adjust any or all of those configurable values -- see: xref:android:replication.adoc#ex-repl-retry[] for how to do this.
...
You can enable authorization in the sync{nbsp}gateway configuration file, as shown in xref:android:replication.adoc#example-enable-authorization[].
...
[source, Kotlin]
----
include::android:example$codesnippet_collection.kt[tags=p2p-act-rep-config-cacert, indent=0]
----
<.> This is the default.
...
[source, Kotlin]
----
include::android:example$codesnippet_collection.kt[tags=p2p-act-rep-config-self-cert, indent=0]
----
...
[source, Kotlin]
----
include::android:example$codesnippet_collection.kt[tag=p2p-act-rep-config-cacert-pinned, indent=0]
----
// include::android:example$codesnippet_collection.kt[tag=p2p-act-rep-config-cacert-pinned-callouts, indent=0]
--
...
[source, Java]
----
include::android:example$codesnippet_collection.java[tags=p2p-act-rep-config-cacert, indent=0]
----
<.> This is the default.
Only certificate chains with roots signed by a trusted CA are allowed.
Self signed certificates are not allowed.
...
[source, Java]
----
include::android:example$codesnippet_collection.java[tags=p2p-act-rep-config-self-cert, indent=0]
----
<.> Set this to `true` to accept any self signed cert.
Any certificates that are not self-signed are rejected.
--
...
[source, Java]
----
include::android:example$codesnippet_collection.java[tag=p2p-act-rep-config-cacert-pinned, indent=0]
----
...
This all assumes that you have configured the Sync Gateway to provide the appropriate SSL certificates, and have included the appropriate certificate in your app bundle -- for more on this see: xref:android:replication.adoc#lbl-cert-pinning[].


[#lbl-client-auth]
=== Client Authentication

There are two ways to authenticate from a Couchbase Lite client: xref:android:replication.adoc#basic-authentication[] or xref:android:replication.adoc#session-authentication[].
...
xref:android:replication.adoc#ex-base-auth[] shows how to initiate a one-shot replication as the user *username* with the password *password*.
...
See xref:android:replication.adoc#ex-session-auth[], which shows how to initiate a one-shot replication with the session ID returned from the `+POST /{tkn-db}/_session+` endpoint.
...
This feature is useful in passing additional credentials, perhaps when an authentication or authorization step is being done by a proxy server (between Couchbase Lite and Sync Gateway) -- see xref:android:replication.adoc#ex-cust-hdr[].
...
For more information, see xref:android:replication.adoc#auto-purge-on-revoke[Auto Purge on Revoke].
****
...
If a user subsequently regains access to a lost channel, then any previously auto-purged documents still assigned to any of their channels are automatically pulled down by the active Sync Gateway when they are next updated -- see behavior summary in xref:android:replication.adoc#tbl-regain-behavior[]
...
In this section::
xref:android:replication.adoc#lbl-repl-start[]  | xref:android:replication.adoc#lbl-repl-ckpt[]
...
You can, optionally, add a change listener (see xref:android:replication.adoc#lbl-repl-mon[]) before starting the replicator running using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-android}{empty}/couchbase-lite-android/com/couchbase/lite/AbstractReplicator.html#start-boolean-[start()].
...
However, if you do want to force the replication to start again from zero, use the xref:refer-glossary.adoc#checkpoint[checkpoint] reset argument when starting the replicator -- as shown in xref:android:replication.adoc#ex-repl-ckpt[].
...
----
include::android:example$kotlin_snippets/app/src/main/kotlin/com/couchbase/codesnippets/FlowExamples.kt[tag="flow-as-replicator-change-listener"]
----
...
That is, whether it is actively transferring data or if it has stopped -- see: xref:android:replication.adoc#ex-repl-mon[].

The returned _ReplicationStatus_ structure comprises:

* https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-android}{empty}/couchbase-lite-android/com/couchbase/lite/ReplicatorStatus.html#getActivityLevel()[getActivityLevel()] -- stopped, offline, connecting, idle or busy -- see states described in: xref:android:replication.adoc#tbl-states[]
...
[source, Kotlin]
----
include::android:example$codesnippet_collection.kt[tags=p2p-act-rep-add-change-listener;!p2p-act-rep-add-change-listener-label,indent=0]
----
--
+
...
[source, Kotlin]
----
include::android:example$codesnippet_collection.kt[tags=p2p-act-rep-status,indent=0]
----
--
======
...
[source, Java]
----
include::android:example$codesnippet_collection.java[tags=p2p-act-rep-add-change-listener;!p2p-act-rep-add-change-listener-label,indent=0]
----
--
+
...
[source, Java]
----
include::android:example$codesnippet_collection.java[tags=p2p-act-rep-status,indent=0]
----
--
======
=====
...
[#lbl-repl-states]
==== Replication States
xref:android:replication.adoc#tbl-states[] shows the different states, or activity levels, reported in the API; and the meaning of each.
...
For example, the code snippet in xref:android:replication.adoc#ex-reg-doc-listener[] registers a listener to monitor document replication performed by the replicator referenced by the variable `replicator`.
It prints the document ID of each document received and sent.
Stop the listener as shown in xref:android:replication.adoc#ex-stop-doc-listener[].
...
It will stop any active xref:android:replication.adoc#lbl-repl-chng[change listener] once the replication is stopped.
...
[#using-kotlin-flows-and-livedata-2]
=== Using Kotlin Flows and LiveData
...
You can increase the log output for activity related to replication with Sync Gateway -- see xref:android:replication.adoc#ex-logs[].
...
For more on troubleshooting with logs, see: xref:android:new-logging-api.adoc[Using Logs].
