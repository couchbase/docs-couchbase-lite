...
You can enable authorization in the pass:q,a[sync{nbsp}gateway] configuration file, as shown in <<example-enable-authorization>>.
...
This can be done either by setting https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-android}{empty}/couchbase-lite-android/com/couchbase/lite/AbstractReplicatorConfiguration.html#lbl-repl-fltrs>>
...
However, if you do want to force the replication to start again from zero, use the xref:refer-glossary.adoc#ex-repl-ckpt>>.
