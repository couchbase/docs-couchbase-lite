...
There's separate downloads for Kotlin and Java.
...
. Build the project and it pulls Couchbase Lite down.
--
...
Now write your first code in the xref:android:gs-five-minutes.adoc[Quick Start], or read the annotated xref:android:gs-build.adoc[Getting Started App], which demonstrates use of key Create, Read, Update and Delete functionality.
****
...
. Add _Couchbase{nbsp}Lite_ to the `dependencies{}` section:
+
[source,kotlin, subs="attributes+"]
----
dependencies {
...
. Add _Couchbase{nbsp}Lite_ to your `dependencies{}` section:
+
[source,groovy, subs="attributes+"]
----
dependencies {
...
[#installing-the-vector-search-extension-2]
=== Installing the Vector Search Extension
...
An application that enables minification must ensure that certain pieces of Couchbase{nbsp}Lite library code are not changed.
See <<ruleset>> for a near-minimal rule set that retains the code Couchbase{nbsp}Lite needs.
