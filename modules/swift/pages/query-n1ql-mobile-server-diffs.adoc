...
[#swift:query-n1ql-mobile-server-diffs:::]
...
There are several minor but notable behavior differences between _{sqlpp} for Mobile_ queries and _{sqlpp} for Server_, as shown in  <<swift:query-n1ql-mobile-server-diffs:::tbl-diffs>>.
...
[#swift:query-n1ql-mobile-server-diffs:::tbl-diffs, cols="^1,2a,2a, options="header"]
|===
...
[discrete#swift:query-n1ql-mobile-server-diffs:::boolean-logic-rules]
=== Boolean Logic Rules
...
[discrete#swift:query-n1ql-mobile-server-diffs:::logical-operations]
==== Logical Operations
...
<<swift:query-n1ql-mobile-server-diffs:::tbl-logops>> summarizes the result of logical operations with different operand values and also shows where the Couchbase Server behavior differs.

.Logical Operations Comparison
[#swift:query-n1ql-mobile-server-diffs:::tbl-logops, cols="^1m,^1m,^1m,^1m,^1m,^1m,^1m", options-"header"]
|===
...
[discrete#swift:query-n1ql-mobile-server-diffs:::crud-operations]
=== CRUD Operations
{sqlpp} for Mobile only supports Read or Query operations.

{sqlpp} for Server fully supports CRUD operation.

[discrete#swift:query-n1ql-mobile-server-diffs:::functions]
=== Functions

[discrete#swift:query-n1ql-mobile-server-diffs:::division-operator]
==== Division Operator
...
[discrete#swift:query-n1ql-mobile-server-diffs:::round-function]
==== Round Function
[,cols="1a,1a", options="header"]
|===
^| {sqlpp} for Server ^| {sqlpp} for Mobile
...
[discrete#swift:query-n1ql-mobile-server-diffs:::related-content]
=== Related Content
++++
...
[discrete.colum#swift:query-n1ql-mobile-server-diffs:::-2n]
==== {empty}
.Learn more . . .
...
[discrete.colum#swift:query-n1ql-mobile-server-diffs:::-3n]
==== {empty}
.Dive Deeper . . .
