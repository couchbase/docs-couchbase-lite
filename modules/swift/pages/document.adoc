...
NOTE: Couchbase documents are assigned to a xref:swift:database.adoc#database-concepts[Collection].
...
The values can be made of up several different xref:swift:document.adoc#data-types[] such as numbers, strings, arrays, and nested objects.
...
Couchbase Lite also provides for the direct handling of JSON data implemented in most cases by the provision of a `toJSON()` method on appropriate API classes (for example, on MutableDocument, Dictionary, Blob and Array) -- see xref:swift:document.adoc#lbl-json-data[].
...
Couchbase documents are assigned to a xref:swift:database.adoc#database-concepts[Collection].
...
For more on using *Documents*, see xref:swift:document.adoc#document-initializers[] and xref:swift:document.adoc#mutability[].
...
Learn more about xref:swift:document.adoc#using-dictionaries[].
...
Learn more about xref:swift:document.adoc#using-arrays[]
...
.Date precision
====
The `setDate()` function rounds to millisecond precision due to the underlying native Swift call.
If you require greater precision, then use the `Double` type to store the date in microseconds.
====
...
NOTE: Any user change to the value of reserved keys (`_id`, `_rev` or `_deleted`) will be detected when a document is saved and will result in an exception (Error Code 5 -- `CorruptRevisionData`) -- see also xref:swift:document.adoc#lbl-doc-constraints[].
...
NOTE: "_id", :"_rev" and "_sequence" are reserved keywords and must not be used as top-level attributes -- see xref:swift:document.adoc#res-keys[].
...
In this section::
xref:swift:document.adoc#lbl-array[]
| xref:swift:document.adoc#lbl-blob[]
| xref:swift:document.adoc#lbl-dictionary[]
| xref:swift:document.adoc#lbl-document[]
| xref:swift:document.adoc#lbl-result[]
...
Convert an `ArrayObject` to and from JSON using the `toJSON()` and `toArray` methods -- see xref:swift:document.adoc#ex-array[].

Additionally you can:

* Initialize a 'MutableArrayObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor -- see: xref:swift:document.adoc#ex-array[]
...
[#lbl-blob]
=== Blobs
Convert a `Blob` to JSON using the `toJSON` method -- see xref:swift:document.adoc#ex-blob[].

You can use `isBlob()` to check whether a given dictionary object is a blob or not -- see xref:swift:document.adoc#ex-blob[].
...
Convert a `DictionaryObject` to and from JSON using the `toJSON` and `toDictionary` methods -- see xref:swift:document.adoc#ex-dictionary[].

Additionally you can:

* Initialize a 'MutableDictionaryObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor-- see: xref:swift:document.adoc#ex-dictionary[]
...
Convert a `Document` to and from JSON strings using the `toJSON()` and `setJSON()` methods -- see xref:swift:document.adoc#ex-document[].

Additionally you can:

* Initialize a 'MutableDocument' using data supplied as a JSON string.
This is done using the `init(json)` or `init(id: json:)` constructor -- see: xref:swift:document.adoc#ex-document[]
