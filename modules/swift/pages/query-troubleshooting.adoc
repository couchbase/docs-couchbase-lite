...
[source#swift:query-troubleshooting:::like-wild-pfx-qry, swift]
----
let query = QueryBuilder
    .select(SelectResult.all())
    .from(DataSource.collection(collection))
...
[source#swift:query-troubleshooting:::use-like-based-queries, swift]
----
let query = QueryBuilder
    .select(SelectResult.all())
    .from(DataSource.collection(collection))
...
[.column#swift:query-troubleshooting:::-2]
====== {empty}
.Learn more . . .
...
[.column#swift:query-troubleshooting:::-3]
====== {empty}
.Dive Deeper . . .
