...
//
:page-aliases: learn/swift-query-troubleshooting.adoc
:page-role:
...
[discrete#swift:query-troubleshooting:::query-explain]
=== Query Explain


[discrete#swift:query-troubleshooting:::using]
==== Using
Query's https://docs.couchbase.com/mobile/{version-maintenance-ios}/couchbase-lite-swift/Classes/Query.html#/s:18CouchbaseLiteSwift5QueryC7explainSSyKF[explain()] method can provide useful insight when you are trying to diagnose query performance issues and-or optimize queries.
To examine how your query is working, either embed the call inside your app (see: <<swift:query-troubleshooting:::use-qe-app>>), or use it interactively within a `cblite` shell (see: <<swift:query-troubleshooting:::use-qe-cblite>>).

[#use-qe-app]
.Using Query Explain in App


[#swift:query-troubleshooting:::use-qe-app]
====
...
[#swift:query-troubleshooting:::use-qe-cblite]
...
[discrete#swift:query-troubleshooting:::output]
==== Output
The output from `https://docs.couchbase.com/mobile/{version-maintenance-ios}/couchbase-lite-swift/Classes/Query.html#/s:18CouchbaseLiteSwift5QueryC7explainSSyKF[explain()]` remains the same whether invoked by an app, or `cblite` -- see <<swift:query-troubleshooting:::qe-output>> for an example of how it looks.

[#swift:query-troubleshooting:::qe-output]
...
This output (<<swift:query-troubleshooting:::qe-output>>) comprises three main elements:
...
[discrete#swift:query-troubleshooting:::the-query-plan]
=== The Query Plan


[discrete#swift:query-troubleshooting:::format]
==== Format
...
[#swift:query-troubleshooting:::qry-plan]
.A Query Plan
====
[source, console]
----
...
Something you may well be looking to optimize -- see <<swift:query-troubleshooting:::ret-method>> for more.
<.> *Grouping method* --- This line shows that the *Group By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see <<swift:query-troubleshooting:::order-group>>.
<.> *Ordering method* -- This line shows that the *Order By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see <<swift:query-troubleshooting:::order-group>>.
====


[discrete#swift:query-troubleshooting:::ret-method]
==== Retrieval Method
...
The _retrieval method_ shows the approach decided upon by the optimizer -- see <<swift:query-troubleshooting:::ret-meths>>.

[#ret-meths]
.Retrieval methods
[#swift:query-troubleshooting:::ret-meths#,cols="2,8"]
|===
...
[discrete#swift:query-troubleshooting:::order-group]
==== Order and Group
...
[discrete#swift:query-troubleshooting:::queries-and-indexes]
=== Queries and Indexes
...
[discrete#swift:query-troubleshooting:::working-with-the-query-optimizer]
...
So, if your analysis of the <<swift:query-troubleshooting:::qe-output,Query Explain output>> indicates a sub-optimal query and your rewrites fail to sufficiently optimize it, consider compacting the database.
...
[discrete#swift:query-troubleshooting:::use-like-based-queries]
...
In <<swift:query-troubleshooting:::like-wild-pfx-qry>> we use a wildcard prefix and suffix.
...
[sourc#swift:query-troubleshooting:::like-wild-pfx-qrye, swift]
----
let query = QueryBuilder
    .select(SelectResult.all())
    .from(DataSource.collection(collection))
...
By contrast, by removing the wildcard prefix `%` (in <<swift:query-troubleshooting:::like-no-wild-pfx-qry>>), we see that the query plan's retrieval method changes to become an index search.
...
[#swift:query-troubleshooting:::like-no-wild-pfx-qry]
====
...
[discrete#swift:query-troubleshooting:::use-functions-wisely]
...
For example, you can observe a similar situation to that shown in <<swift:query-troubleshooting:::use-like-based-queries>> when using the `https://docs.couchbase.com/mobile/{version-maintenance-ios}/couchbase-lite-swift/Classes/Function.html#/s:18CouchbaseLiteSwift8FunctionC5loweryAA18ExpressionProtocol_pAaE_pFZ[lower()]` function on an indexed property.
...
[sourc#swift:query-troubleshooting:::use-like-based-queriese, swift]
----
let query = QueryBuilder
    .select(SelectResult.all())
    .from(DataSource.collection(collection))
...
<.> Here we use the `https://docs.couchbase.com/mobile/{version-maintenance-ios}/couchbase-lite-swift/Classes/Function.html#/s:18CouchbaseLiteSwift8FunctionC5loweryAA18ExpressionProtocol_pAaE_pFZ[lower()]` function in the _Where_ expression
...
But removing the `https://docs.couchbase.com/mobile/{version-maintenance-ios}/couchbase-lite-swift/Classes/Function.html#/s:18CouchbaseLiteSwift8FunctionC5loweryAA18ExpressionProtocol_pAaE_pFZ[lower()]` function, changes things:
...
<.> Here we have removed `https://docs.couchbase.com/mobile/{version-maintenance-ios}/couchbase-lite-swift/Classes/Function.html#/s:18CouchbaseLiteSwift8FunctionC5loweryAA18ExpressionProtocol_pAaE_pFZ[lower()]` from the _Where_ expression
...
[discrete#swift:query-troubleshooting:::optimization-considerations]
...
[discrete#swift:query-troubleshooting:::related-content]
=== Related Content
++++
...
[discrete.colum#swift:query-troubleshooting:::-2n]
==== {empty}
.Learn more . . .
...
[discrete.colum#swift:query-troubleshooting:::-3n]
==== {empty}
.Dive Deeper . . .
