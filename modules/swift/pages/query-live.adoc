...
Save the token in order to detach the listener and stop the query later -- se xref:swift:query-live.adoc#ex-qry-stop[]
...
<.> Here we use the change lister token from xref:swift:query-live.adoc#ex-qry-start[] to remove the listener.
