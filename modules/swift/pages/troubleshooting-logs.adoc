...
* xref:swift:troubleshooting-logs.adoc#lbl-file-logs[]
+
Here logs are written to xref:swift:troubleshooting-logs.adoc#log-file-outputs[separate log files] filtered by log level, with each log level supporting individual retention policies.

* xref:swift:troubleshooting-logs.adoc#lbl-console-logs[]
+
--
...
* xref:swift:troubleshooting-logs.adoc#lbl-custom-logs[]
+
--
...
To change default settings use database's https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-swift/Classes/Log.html[Log] method to set the required values -- see xref:swift:troubleshooting-logs.adoc#eg-cons-log[]
...
[#lbl-file-logs]
== File based logging
File based logging is disabled by default --  see: xref:swift:troubleshooting-logs.adoc#eg-file-log[] for how to enable it.
...
+
Use this format and a decoder, such as *cbl-log*, to view them -- see: xref:swift:troubleshooting-logs.adoc#decoding-binary-logs[].
* Plaintext
...
To do this, apps must implement the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-swift/Classes/Logger.html[Logger] interface -- see xref:swift:troubleshooting-logs.adoc#eg-impl-log[] -- and enable custom logging using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-swift/Classes/Log.html#/s:18CouchbaseLiteSwift3LogC6customAA6Logger_pSgvp[log.custom] -- see xref:swift:troubleshooting-logs.adoc#eg-cust-log[].
...
Here we introduce the code that implements the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-swift/Classes/Logger.html[Logger] interface.
...
This example show how to enable the custom logger from xref:swift:troubleshooting-logs.adoc#eg-impl-log[].
...
You can use the *cbl-log* tool to decode binary log files -- see xref:swift:troubleshooting-logs.adoc#eg-cbl-log[].
