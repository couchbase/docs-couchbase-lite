...
Related Content -- xref:swift:new-logging-api.adoc[Using Logs] | xref:swift:troubleshooting-crashes.adoc[Decoding Crash Logs] | xref:swift:querybuilder.adoc#lbl-predquery[Predictive Queries] | xref:swift:query-live.adoc[Live Queries] | xref:swift:indexing.adoc[Indexing]
--
...
To examine how your query is working, either embed the call inside your app (see: xref:swift:troubleshooting-queries.adoc#use-qe-app[]), or use it interactively within a `cblite` shell (see: xref:swift:troubleshooting-queries.adoc#use-qe-cblite[]).
...
[#output]
=== Output
The output from `https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-swift/Classes/Query.html#/s:18CouchbaseLiteSwift5QueryC7explainSSyKF[explain()]` remains the same whether invoked by an app, or `cblite` -- see xref:swift:troubleshooting-queries.adoc#qe-output[] for an example of how it looks.
...
This output (xref:swift:troubleshooting-queries.adoc#qe-output[]) comprises three main elements:
...
Something you may well be looking to optimize -- see xref:swift:troubleshooting-queries.adoc#ret-method[] for more.
<.> *Grouping method* --- This line shows that the *Group By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see xref:swift:troubleshooting-queries.adoc#order-group[].
<.> *Ordering method* -- This line shows that the *Order By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see xref:swift:troubleshooting-queries.adoc#order-group[].
====
...
The _retrieval method_ shows the approach decided upon by the optimizer -- see xref:swift:troubleshooting-queries.adoc#ret-meths[].
...
So, if your analysis of the xref:swift:troubleshooting-queries.adoc#qe-output[Query Explain output] indicates a sub-optimal query and your rewrites fail to sufficiently optimize it, consider compacting the database.
...
In xref:swift:troubleshooting-queries.adoc#like-wild-pfx-qry[] we use a wildcard prefix and suffix.
...
[source#like-wild-pfx-qry, swift]
----
...
By contrast, by removing the wildcard prefix `%` (in xref:swift:troubleshooting-queries.adoc#like-no-wild-pfx-qry[]), we see that the query plan's retrieval method changes to become an index search.
