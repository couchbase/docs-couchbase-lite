...
include::swift:example$code_snippets/VectorSearch.swift[tags=vs-create-default-config;vs-create-custom-config]

----

First, initialise the `config` object with the `VectorIndexConfiguration()` method with the following parameters:

* The expression of the data as a vector.

* The width or `dimensions` of the vector index is set to `300`.

* The amount of `centroids` is set to `8`.
This means that there will be eight buckets with a single centroid each that gathers together similar vectors.
...
From there, you create an index within a given collection, in our case `vector_index`, using our previously generated `config` object.
...
The key difference to note is that the `config` object uses the output of the `prediction()` function as the `expression` parameter to generate the word embedding.
...
include::swift:example$code_snippets/VectorSearch.swift[tags=vs-use-vector-match]
...
include::swift:example$code_snippets/VectorSearch.swift[tags=vs-use-vector-distance]
