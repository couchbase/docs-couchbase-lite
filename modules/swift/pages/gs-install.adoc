...
To get started coding Couchbase Lite for Swift apps, just create or open an existing _XCode_ project and install the required _Couchbase Lite_ version, for example {version-maintenance}, using one of the <<swift:gs-install:::lbl-install-tabs>> methods shown.
...
[source,ruby,subs="attributes+"]
----
binary "http://packages.couchbase.com/releases/couchbase-lite-vector-search/carthage/CouchbaseLiteVectorSearch.json" ~> {vs-version-maintenance} // <.>
----
+
<.> Specify the required version number, here we use the latest version
