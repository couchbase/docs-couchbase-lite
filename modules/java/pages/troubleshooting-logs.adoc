...
+
Here logs are written to xref:java:troubleshooting-logs.adoc#log-file-outputs[separate log files] filtered by log level, with each log level supporting individual retention policies.
...
You'll primarily use `Database.log.getConsole()` to get the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ConsoleLogger.html[ConsoleLogger] to control console logging.
...
[#lbl-file-logs]
== File based logging
File based logging is disabled by default --  see: xref:java:troubleshooting-logs.adoc#eg-file-log[] for how to enable it.
...
Here we introduce the code that implements the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Logger.html[Logger] interface.
