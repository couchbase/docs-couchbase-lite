...
It delivers flexibility in how Couchbase Lite generates and retains logs, while also maintaining the logging level that Couchbase Support requires for investigating issues.

Log output is split into the following streams:

* xref:java:troubleshooting-logs.adoc#lbl-file-logs[]
+
Couchbase Lite writes logs to xref:java:troubleshooting-logs.adoc#log-file-outputs[separate log files] filtered by log level, with each log level supporting individual retention policies.

* xref:java:troubleshooting-logs.adoc#lbl-console-logs[]
+
--
...
* xref:java:troubleshooting-logs.adoc#lbl-custom-logs[]
+
--
...
In all instances, you control what the application logs and at what level using the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Log.html[Log] class.
...
Console logging enables by default.
To change default settings use database's https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Log.html[Log] method to set the required values -- see xref:java:troubleshooting-logs.adoc#eg-cons-log[]

Use `Database.log.getConsole()` to get the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ConsoleLogger.html[ConsoleLogger] to control console logging.
...
<.> Set the domains you want to log.
This example turns on logging for all available domains -- see: https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ConsoleLogger.html#setDomains-java.util.EnumSet-[log.getConsole().setDomain()] and enumeration https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/LogDomain.html[LogDomain]
<2> Turn on the most verbose log level -- see: https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ConsoleLogger.html#setLevel-com.couchbase.lite.LogLevel-[log.getConsole().setLevel()] and enumeration https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/LogLevel.html[LogLevel]. +
To turn off logging for the specified https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/LogDomain.html[LogDomain] set the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/LogLevel.html[LogLevel] to `None`.
...
[#lbl-file-logs]
== File based logging
File based logging defaults to off --  see: xref:java:troubleshooting-logs.adoc#eg-file-log[] for how to enable it.

Use https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Log.html[log.getFile()] and https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/FileLogger.html[FileLogger] to control file-based logging.

[#formats]
=== Formats
Available file based logging formats:

* Binary -- most efficient for storage and performance.
Binary format is the default format for file based logging.
+
Use this format and a decoder, such as *cbl-log*, to view them -- see: xref:java:troubleshooting-logs.adoc#decoding-binary-logs[].
* Plaintext
...
With file based logging you can also use the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/LogFileConfiguration.html[LogFileConfiguration] class's properties to specify the:

* log file path to the directory to store the log file
When the file exceeds this limit, a new log file starts.
* log file format +
...
<.> Change the max rotation count from the default (1) to 5. +
*Note* this means six files may exist at any one time; the five rotated log files, plus the active log file
<.> Set the maximum size (bytes) for the log file
<.> Select the binary log format (included for reference only as binary format is the default)
<.> Increase the log output level from the default (_warnings_) to _info_ -- see: https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/FileLogger.html#setLevel-com.couchbase.lite.LogLevel-[log.getFile().setLevel()]
+
...
Couchbase Lite allows for the registration of a callback function to receive Couchbase Lite log messages, which you can log using any external logging framework.

To do this, apps must implement the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Logger.html[Logger] interface -- see xref:java:troubleshooting-logs.adoc#eg-impl-log[] -- and enable custom logging using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Log.html[log.setCustom()] -- see xref:java:troubleshooting-logs.adoc#eg-cust-log[].
...
This example shows the code that implements the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Logger.html[Logger] interface.
...
This example show how to enable the custom logger from xref:java:troubleshooting-logs.adoc#eg-impl-log[].
...
<.> Set the custom logger with a level of 'warning'.
The custom logger receives every log and may choose to filter it, using its configured level.
...
You can use the *cbl-log* tool to decode binary log files -- see xref:java:troubleshooting-logs.adoc#eg-cbl-log[].
