...
If you're using a Couchbase Lite release prior to 2.5 see <<pre-2x5-logging,Deprecated functionality>>
...
It delivers flexibility in generating and retaining logs, while also maintaining the logging level that Couchbase Support requires for investigating issues.

Log output splits into the following streams:

* xref:java:troubleshooting-logs.adoc#lbl-file-logs[]
+
Here Couchbase Lite write logs to xref:java:troubleshooting-logs.adoc#log-file-outputs[separate log files] filtered by log level, with each log level supporting individual retention policies.
...
In all instances, you control what's logged and at what level using the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Log.html[Log] class.
...
You can fine-tune their diagnostic content to meet the needs of a particular debugging scenario, perhaps by increasing the verbosity and-or choosing to focus on messages from a specific domain; to better focus on the problem area.
...
It's the default for file based logging.
+
...
Once this limit is exceeded, logging starts a new log file.
* log file format +
...
Here the code that implements the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Logger.html[Logger] interface is introduced.
...
This example shows how to enable the custom logger from xref:java:troubleshooting-logs.adoc#eg-impl-log[Implementing logger interface].
...
The application calls the custom logger with every log, and the logger may choose to filter it using its configured level.
