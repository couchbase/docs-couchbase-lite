...
Database queries defined with the QueryBuilder API use the query statement format shown in xref:java:querybuilder.adoc#ex-query-form[].
...
| xref:java:querybuilder.adoc#lbl-select[]
...
--
In this section::
xref:java:querybuilder.adoc#lbl-return-properties[]{nbsp}{nbsp}|{nbsp}{nbsp} xref:java:querybuilder.adoc#lbl-return-all[]
Related::
xref:java:querybuilder.adoc#lbl-resultsets[Handling result sets]
--
...
Use the `SelectResult.all()` method to return all the properties of selected documents -- see: xref:java:querybuilder.adoc#ex-select-all[].
...
The query.execute statement returns the results in a dictionary, where the key is the database name -- see xref:java:querybuilder.adoc#ex-return-all[].
...
See: xref:java:querybuilder.adoc#lbl-resultsets[] for more on processing query results.


[#lbl-return-properties]
=== Return Selected Properties
To access only specific properties, specify a comma-separated list of `SelectResult` expressions, one for each property, in the select statement of your query  -- see: xref:java:querybuilder.adoc#ex-select-properties[]
...
The `query.execute` statement returns one or more key-value pairs, one for each SelectResult expression, with the property-name as the key -- see xref:java:querybuilder.adoc#ex-return-properties[]
...
In this section::
xref:java:querybuilder.adoc#lbl-comp-ops[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-coll-ops[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-like-ops[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-regex-ops[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-deleted-ops[]
...
=== Like Operator
In this section::
xref:java:querybuilder.adoc#lbl-string-match[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-wild-match[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-wild-chars[]

[#lbl-string-match]
==== String Matching
The https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Expression.html#like-com.couchbase.lite.Expression-[Like()] operator can be used for string matching -- see xref:java:querybuilder.adoc#ex-like-case-insensitive[]
...
In xref:java:querybuilder.adoc#ex-wldcd-match[] below, we are looking for documents of `type` "landmark" where the name property matches any string that begins with "eng" followed by zero or more characters, the letter "e", followed by zero or more characters.
...
In xref:java:querybuilder.adoc#ex-wldcd-char-match[] below, we are looking for documents of type "landmark" where the `name` property matches any string that begins with "eng" followed by exactly 4 wildcard characters and ending in the letter "r".
...
Similar to the wildcards in `like` expressions, `regex` based pattern matching allow you to introduce an element of fuzziness in your search string -- see the code shown in xref:java:querybuilder.adoc#ex-regex[].
...
[#lbl-deleted-ops]
=== Deleted Document
You can query documents that have been deleted (tombstones) footnote:fn2x5[Starting in Couchbase Lite 2.5] as shown in xref:java:querybuilder.adoc#ex-del-qry[].
...
For example to combine airline details with route details, linked by the airline id -- see xref:java:querybuilder.adoc#ex-join[].
...
The query shown in xref:java:querybuilder.adoc#ex-grpby-qry[] generates the following output:
--
...
It is possible to sort the results of a query based on a given expression result -- see xref:java:querybuilder.adoc#ex-orderby-qry[]
...
The query shown in xref:java:querybuilder.adoc#ex-orderby-qry[] generates the following output:
...
Couchbase Lite documents support a xref:java:querybuilder.adoc#initializers[date type] that internally stores dates in ISO 8601 with the GMT/UTC timezone.
...
== Result Sets
In this section::
xref:java:querybuilder.adoc#lbl-process-resultset[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-all-sel[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-specific-sel[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-id-sel[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-count-sel[]{nbsp}{nbsp}|{nbsp}{nbsp}
xref:java:querybuilder.adoc#lbl-pagination[]
...
* SelectResult.all -- see: xref:java:querybuilder.adoc#lbl-all-sel[All Properties]
* SelectResult.expression(property("name")) -- see: xref:java:querybuilder.adoc#lbl-specific-sel[Specific Properties]
* SelectResult.expression(meta.id) --  Metadata (such as the `_id`) -- see: xref:java:querybuilder.adoc#lbl-id-sel[Document ID Only]
* SelectResult.expression(Function.count(Expression.all())).as("mycount") --  see: xref:java:querybuilder.adoc#lbl-count-sel[]
...
* The result set of an aggregate, count-only, query is a key-value pair -- see xref:java:querybuilder.adoc#lbl-count-sel[] -- which you can access using the count name as its key.
...
[#query]
==== Query
The `Select` statement for this type of query, returns all document properties for each document matching the query criteria -- see xref:java:querybuilder.adoc#ex-all-qry[]
...
For each result object, the key is the database name and the 'value' is a dictionary representing each document property as a key-value pair -- see: xref:java:querybuilder.adoc#ex-all-rtn[].
...
In this case access the retrieved document properties by converting each row's value, in turn, to a dictionary -- as shown in xref:java:querybuilder.adoc#ex-all-acc[].
...
[#query-2]
==== Query
Here we use `SelectResult.expression(property("<property-name>")))` to specify the document properties we want our query to return -- see: xref:java:querybuilder.adoc#ex-specific-qry[].
...
Each result object comprises a key-value pair for each selected document property -- see xref:java:querybuilder.adoc#ex-specific-rtn[]
...
[#result-set-access-2]
==== Result Set Access
Access the retrieved properties by converting each row into a dictionary -- as shown in xref:java:querybuilder.adoc#ex-specific-acc[].
...
[#query-3]
==== Query
You would typically use this type of query if retrieval of document properties directly would consume excessive amounts of memory and-or processing time -- see: xref:java:querybuilder.adoc#ex-id-qry[].
...
Each result object has `id` as the key and the ID value as its value -- -see xref:java:querybuilder.adoc#ex-id-rtn[].
...
In this case, access the required document's properties by unpacking the `id` and using it to get the document from the database -- see: xref:java:querybuilder.adoc#ex-id-acc[].
...
The key is the count name, as defined using `SelectResult.as` -- see: xref:java:querybuilder.adoc#ex-count-rtn[] for the format and xref:java:querybuilder.adoc#ex-count-qry[] for the query.
...
Access the count using its alias name (`mycount` in this example) -- see xref:java:querybuilder.adoc#ex-count-acc[]
...
Use the `limit` and `offset` feature, to return a defined number of results starting from a given offset -- see: xref:java:querybuilder.adoc#ex-pagination[].
...
. xref:java:querybuilder.adoc#integrate-the-model[Integrate the Model]
. xref:java:querybuilder.adoc#register-the-model[Register the Model]
. xref:java:querybuilder.adoc#create-an-index[Create an Index (Optional)]
. xref:java:querybuilder.adoc#run-a-prediction-query[Run a Prediction Query]
...
To integrate a model with Couchbase Lite, you must implement the `PredictiveModel` interface which has only one function called `predict()` -- see: xref:java:querybuilder.adoc#int-pred-model[].
...
* xref:java:querybuilder.adoc#value-index[Value Index]
* xref:java:querybuilder.adoc#predictive-index[Predictive Index]
