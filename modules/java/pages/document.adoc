...
NOTE: Couchbase documents are assigned to a xref:java:database.adoc#database-concepts[Collection].
...
The values can be made of up several different xref:java:document.adoc#data-types[] such as numbers, strings, arrays, and nested objects.
...
Couchbase Lite also provides for the direct handling of JSON data implemented in most cases by the provision of a pass:a,q[`toJSON()`] method on appropriate API classes (for example, on MutableDocument, Dictionary, Blob and Array) -- see xref:java:document.adoc#lbl-json-data[].
...
Couchbase documents are assigned to a xref:java:database.adoc#database-concepts[Collection].
...
For more on using *Documents*, see xref:java:document.adoc#document-initializers[] and xref:java:document.adoc#mutability[].
...
Learn more about xref:java:document.adoc#using-dictionaries[].
...
Learn more about xref:java:document.adoc#using-arrays[]
...
NOTE: Any user change to the value of reserved keys (`_id`, `_rev` or `_deleted`) will be detected when a document is saved and will result in an exception (Error Code 5 -- `CorruptRevisionData`) -- see also xref:java:document.adoc#lbl-doc-constraints[].
...
NOTE: "_id", :"_rev" and "_sequence" are reserved keywords and must not be used as top-level attributes -- see xref:java:document.adoc#res-keys[].
...
In this section::
xref:java:document.adoc#lbl-array[]
| xref:java:document.adoc#lbl-blob[]
| xref:java:document.adoc#lbl-dictionary[]
| xref:java:document.adoc#lbl-document[]
| xref:java:document.adoc#lbl-result[]
...
Convert an `ArrayObject` to and from JSON using the pass:a,q[`toJSON()`] and `toArray` methods -- see xref:java:document.adoc#ex-array[].

Additionally you can:

* Initialize a 'MutableArrayObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor -- see: xref:java:document.adoc#ex-array[]
...
[#lbl-blob]
=== Blobs
Convert a `Blob` to JSON using the `toJSON` method -- see xref:java:document.adoc#ex-blob[].

You can use `isBlob()` to check whether a given dictionary object is a blob or not -- see xref:java:document.adoc#ex-blob[].
...
Convert a `DictionaryObject` to and from JSON using the `toJSON` and `toDictionary` methods -- see xref:java:document.adoc#ex-dictionary[].

Additionally you can:

* Initialize a 'MutableDictionaryObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor-- see: xref:java:document.adoc#ex-dictionary[]
...
Convert a `Document` to and from JSON strings using the pass:a,q[`toJSON()`] and pass:a,q[`setJSON()`] methods -- see xref:java:document.adoc#ex-document[].

Additionally you can:

* Initialize a 'MutableDocument' using data supplied as a JSON string.
This is done using the `init(json)` or `init(id: json:)` constructor -- see: xref:java:document.adoc#ex-document[]
...
Use https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Result.html#toJSON--[Result.toJSON()] to transform your result string into a JSON string, which can easily be serialized or used as required in your application. See xref:java:document.adoc#ex-json[] for a working example.
