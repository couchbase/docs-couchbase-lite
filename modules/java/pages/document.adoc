...
Couchbase Lite also provides for the direct handling of JSON data implemented in most cases by the provision of a `toJSON()` method on appropriate API classes (for example, on MutableDocument, Dictionary, Blob and Array) -- see xref:java:document.adoc#lbl-json-data[].
...
Convert an `ArrayObject` to and from JSON using the `toJSON()` and `toArray` methods -- see xref:java:document.adoc#ex-array[].
...
Convert a `Document` to and from JSON strings using the `toJSON()` and `setJSON()` methods -- see xref:java:document.adoc#ex-document[].
