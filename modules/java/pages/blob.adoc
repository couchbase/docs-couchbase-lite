...
Couchbase{nbsp}Lite for Java uses _blobs_ to store the contents of images, other media files and similar format files as binary objects.
...
When a document is synchronized, the Couchbase{nbsp}Lite replicator adds an `_attachments` dictionary to the document's properties if it contains a blob -- see xref:java:blob.adoc#img-blob[].


[#blob-objects]
== Blob Objects

The blob as an object appears in a document as dictionary property -- see, for example _avatar_ in xref:java:blob.adoc#img-blob[].

Other properties include `length` (the length in bytes), and optionally `content_type` (typically, its MIME type).

The blob's data (an image, audio or video content) is not stored in the document, but in a separate content-addressable store, indexed by the `digest` property -- see xref:java:blob.adoc#lbl-using[].
...
The code in xref:java:blob.adoc#ex-blob[] shows how you might add a blob to a document and save it to the database. Here we use `avatar` as the property key and a jpeg file as the blob data.
...
Saving the document generates a random access key for each blob stored in `digest` a SHA-1 encrypted property -- see: xref:java:blob.adoc#img-blob[]. +
We can use the `avatar` key to retrieve the blob object later.
Note, this is the identity of the blob assigned by us; the replication auto-generates a blob for attachments and assigns its own name to it (for example, `blob_1`) -- see xref:java:blob.adoc#img-blob[].
...
If you view a sync'd blob document in {cbs} Admin Console, you will see something similar to xref:java:blob.adoc#img-blob[], which shows the document with its generated `_attachments` dictionary, including the `digest`.
