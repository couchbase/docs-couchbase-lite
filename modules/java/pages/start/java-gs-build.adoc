...
* Create a database
+
The app creates its database in the `{nmSampleAppDbPath}` directory relative to its root location when run (See: {xref-cbl-bmk-database-findfile}).

TIP: Explicitly state your required database location when creating your database (see: {xref-cbl-bmk-database-findfile} for how to do this)
