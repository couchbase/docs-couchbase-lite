...
//
:page-aliases: learn/java-query-troubleshooting.adoc
:page-role:
...
[discrete#java:query-troubleshooting:::query-explain]
=== Query Explain


[discrete#java:query-troubleshooting:::using]
==== Using
Query's https://docs.couchbase.com/mobile/{version-maintenance-java}/couchbase-lite-java/com/couchbase/lite/Query.html#explain--[explain()] method can provide useful insight when you are trying to diagnose query performance issues and-or optimize queries.
To examine how your query is working, either embed the call inside your app (see: <<java:query-troubleshooting:::use-qe-app>>), or use it interactively within a `cblite` shell (see: <<java:query-troubleshooting:::use-qe-cblite>>).

[#use-qe-app]
.Using Query Explain in App


[#java:query-troubleshooting:::use-qe-app]
====
...
[#java:query-troubleshooting:::use-qe-cblite]
...
[discrete#java:query-troubleshooting:::output]
==== Output
The output from `https://docs.couchbase.com/mobile/{version-maintenance-java}/couchbase-lite-java/com/couchbase/lite/Query.html#explain--[explain()]` remains the same whether invoked by an app, or `cblite` -- see <<java:query-troubleshooting:::qe-output>> for an example of how it looks.

[#java:query-troubleshooting:::qe-output]
...
This output (<<java:query-troubleshooting:::qe-output>>) comprises three main elements:
...
[discrete#java:query-troubleshooting:::the-query-plan]
=== The Query Plan


[discrete#java:query-troubleshooting:::format]
==== Format
...
[#java:query-troubleshooting:::qry-plan]
.A Query Plan
====
[source, console]
----
...
Something you may well be looking to optimize -- see <<java:query-troubleshooting:::ret-method>> for more.
<.> *Grouping method* --- This line shows that the *Group By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see <<java:query-troubleshooting:::order-group>>.
<.> *Ordering method* -- This line shows that the *Order By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see <<java:query-troubleshooting:::order-group>>.
====


[discrete#java:query-troubleshooting:::ret-method]
==== Retrieval Method
...
The _retrieval method_ shows the approach decided upon by the optimizer -- see <<java:query-troubleshooting:::ret-meths>>.

[#ret-meths]
.Retrieval methods
[#java:query-troubleshooting:::ret-meths#,cols="2,8"]
|===
...
[discrete#java:query-troubleshooting:::order-group]
==== Order and Group
...
[discrete#java:query-troubleshooting:::queries-and-indexes]
=== Queries and Indexes
...
[discrete#java:query-troubleshooting:::working-with-the-query-optimizer]
...
So, if your analysis of the <<java:query-troubleshooting:::qe-output,Query Explain output>> indicates a sub-optimal query and your rewrites fail to sufficiently optimize it, consider compacting the database.
...
[discrete#java:query-troubleshooting:::use-like-based-queries]
...
In <<java:query-troubleshooting:::like-wild-pfx-qry>> we use a wildcard prefix and suffix.
...
[sourc#java:query-troubleshooting:::like-wild-pfx-qrye, Java]
----
Query query = QueryBuilder
    .select(SelectResult.all())
    .from(DataSource.collection(collection))
...
By contrast, by removing the wildcard prefix `%` (in <<java:query-troubleshooting:::like-no-wild-pfx-qry>>), we see that the query plan's retrieval method changes to become an index search.
...
[#java:query-troubleshooting:::like-no-wild-pfx-qry]
====
...
[discrete#java:query-troubleshooting:::use-functions-wisely]
...
For example, you can observe a similar situation to that shown in <<java:query-troubleshooting:::use-like-based-queries>> when using the `https://docs.couchbase.com/mobile/{version-maintenance-java}/couchbase-lite-java/com/couchbase/lite/Function.html#lower-com.couchbase.lite.Expression-[lower()]` function on an indexed property.
...
[sourc#java:query-troubleshooting:::use-like-based-queriese, Java]
----
Query query = QueryBuilder
    .select(SelectResult.all())
    .from(DataSource.collection(collection))
...
<.> Here we use the `https://docs.couchbase.com/mobile/{version-maintenance-java}/couchbase-lite-java/com/couchbase/lite/Function.html#lower-com.couchbase.lite.Expression-[lower()]` function in the _Where_ expression
...
But removing the `https://docs.couchbase.com/mobile/{version-maintenance-java}/couchbase-lite-java/com/couchbase/lite/Function.html#lower-com.couchbase.lite.Expression-[lower()]` function, changes things:
...
<.> Here we have removed `https://docs.couchbase.com/mobile/{version-maintenance-java}/couchbase-lite-java/com/couchbase/lite/Function.html#lower-com.couchbase.lite.Expression-[lower()]` from the _Where_ expression
...
Knowing this, you can consider how you create the index; for example, using https://docs.couchbase.com/mobile/{version-maintenance-java}/couchbase-lite-java/com/couchbase/lite/Function.html#lower-com.couchbase.lite.Expression-[lower()] when you create the index and then always using lowercase comparisons.

[discrete#java:query-troubleshooting:::optimization-considerations]
...
[discrete#java:query-troubleshooting:::related-content]
=== Related Content
++++
...
[discrete.colum#java:query-troubleshooting:::-2n]
==== {empty}
.Learn more . . .
...
[discrete.colum#java:query-troubleshooting:::-3n]
==== {empty}
.Dive Deeper . . .
