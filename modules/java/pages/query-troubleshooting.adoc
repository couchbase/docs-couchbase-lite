...
To examine how your query is working, either embed the call inside your app (see: xref:java:query-troubleshooting.adoc#use-qe-app[]), or use it interactively within a `cblite` shell (see: xref:java:query-troubleshooting.adoc#use-qe-cblite[]).
...
[#output]
=== Output
The output from `https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Query.html#explain--[explain()]` remains the same whether invoked by an app, or `cblite` -- see xref:java:query-troubleshooting.adoc#qe-output[] for an example of how it looks.
...
This output (xref:java:query-troubleshooting.adoc#qe-output[]) comprises three main elements:
...
Something you may well be looking to optimize -- see xref:java:query-troubleshooting.adoc#ret-method[] for more.
<.> *Grouping method* --- This line shows that the *Group By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see xref:java:query-troubleshooting.adoc#order-group[].
<.> *Ordering method* -- This line shows that the *Order By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see xref:java:query-troubleshooting.adoc#order-group[].
====
...
The _retrieval method_ shows the approach decided upon by the optimizer -- see xref:java:query-troubleshooting.adoc#ret-meths[].
...
So, if your analysis of the xref:java:query-troubleshooting.adoc#qe-output[Query Explain output] indicates a sub-optimal query and your rewrites fail to sufficiently optimize it, consider compacting the database.
...
In xref:java:query-troubleshooting.adoc#like-wild-pfx-qry[] we use a wildcard prefix and suffix.
...
By contrast, by removing the wildcard prefix `%` (in xref:java:query-troubleshooting.adoc#like-no-wild-pfx-qry[]), we see that the query plan's retrieval method changes to become an index search.
