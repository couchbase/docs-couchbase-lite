...
* Create a database
+
The app creates its database in the `/getting-started.cblite2` directory relative to its root location when run (See: <<lbl-find-db-loc,Finding a Database File>>).

TIP: Explicitly state your required database location when creating your database (see: <<lbl-find-db-loc,Finding a Database File>> for how to do this)
