...
* Create a database
+
The app creates its database in the `{nmSampleAppDbPath}` directory relative to its root location when run (See: {database--xref-bmk-findfile}).

TIP: Explicitly state your required database location when creating your database (see: {database--xref-bmk-findfile} for how to do this)
