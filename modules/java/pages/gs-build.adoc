...
This section explains how to validate your configured build environment by building a starter app that uses many of {cbl} on {JP}'s more common features.
...
The GettingStarted app demonstrates how to use {cbl} on {JP}. Console and Web App versions are available.
...
* Create a database
+
The app creates its database in the `{nmSampleAppDbPath}` directory relative to its root location when run (See: xref:database.adoc#lbl-find-db-loc[Finding a Database File]).

TIP: Explicitly state your required database location when creating your database (see: xref:database.adoc#lbl-find-db-loc[Finding a Database File] for how to do this)

* Add content to the DB
* Run a simple query counting the DB rows
* Start a one-shot, bi-directional replication using pass:q,a[{sgw-te}] and pass:q,a[_Couchbase{nbsp}Server_]
* Produce a simple report on the db Contents
+
// !!!GBM: THIS IMAGE PROBABLY NEEDS UPDATING
image::java:cblOutput.png[,400]

NOTE: Before starting your app ensure you have started both your pass:q,a[_Couchbase{nbsp}Server_] and pass:q,a[{sgw-te}] instances.
