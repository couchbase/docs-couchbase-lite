...
This section explains how to validate your configured build environment by building a starter app that uses many of Couchbase{nbsp}Lite's more common features.
...
The GettingStarted app demonstrates how to use Couchbase{nbsp}Lite. Console and Web App versions are available.
...
TIP: Ensure you added the Couchbase{nbsp}Lite dependency to your `build.gradle` file
...
* Create a database
+
The app creates its database in the `/getting-started.cblite2` directory relative to its root location when run (See: xref:java:database.adoc#lbl-find-db-loc[Finding a Database File]).

TIP: Explicitly state your required database location when creating your database (see: xref:java:database.adoc#lbl-find-db-loc[Finding a Database File] for how to do this)

* Add content to the DB
* Run a simple query counting the DB rows
* Start a one-shot, bi-directional replication using _Sync{nbsp}Gateway and _Couchbase{nbsp}Server_
* Produce a simple report on the db Contents
+
// !!!GBM: THIS IMAGE PROBABLY NEEDS UPDATING
image::java:cblOutput.png[,400]

NOTE: Before starting your app ensure you have started both your _Couchbase{nbsp}Server_ and _Sync{nbsp}Gateway instances.
