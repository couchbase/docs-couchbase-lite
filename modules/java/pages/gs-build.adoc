...
The GettingStarted app demonstrates how to use {cblJP}. Console and Web App versions are available.
...
* Create a database
+
The app creates its database in the `/resources/getting-started.cblite2` directory relative to its root location when run (See: {loc--finding-db-file--xref}).
...
* Start a one-shot, bi-directional replication using pass:q,a[_Sync{nbsp}Gateway_] and pass:q,a[_Couchbase{nbsp}Server_]
* Produce a simple report on the db Contents
+
// !!!GBM: THIS IMAGE PROBABLY NEEDS UPDATING
image::java:cblOutput.png[,400]

NOTE: Before starting your app ensure you have started both your pass:q,a[_Couchbase{nbsp}Server_] and pass:q,a[_Sync{nbsp}Gateway_] instances.
