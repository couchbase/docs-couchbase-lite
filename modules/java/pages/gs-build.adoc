...
:loc--finding-db-file--xref: <<lbl-find-db-loc,Finding a Database File>>
...
* Create a database
+
The app creates its database in the `/getting-started.cblite2` directory relative to its root location when run (See: {loc--finding-db-file--xref}).
