...
Before proceeding to xref:gs-install.adoc[], you will need to setup your build environment, and make the supplied support libraries available.
...
.
. Unpack the downloaded file to a location accessible to -- and usable by -- your chosen *build* environment, for example `your_dir/couchbase-lite-java-{version-maintenance-java}`.
...
You will need to add the paths of the directories `<lib-directory>/support/libc++` and `<lib-directory>/support/libicu` to  `LD_LIBRARY_PATH`.
