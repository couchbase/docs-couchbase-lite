...
{cblJP} enables development and deployment of Couchbase{nbsp}Lite applications to a JVM environment.
...
If you want to use {cbljp} with _Couchbase{nbsp}Server_ and/or _Sync{nbsp}Gateway_ you will need to have installed operational instances of these before completing the installation and test build steps.
...
* Create a RBAC user for _Sync{nbsp}Gateway_ with username = `sync-gateway` and password = `password`

== Downloaded Binaries

===  Package Contents

The download package contains a license file, jar libraries for the appropriate edition of the Couchbase{nbsp}Lite and a set of Linux _shared libraries_.
...
. Include the following dependency in your `build.gradle` file, you can remove any Couchbase{nbsp}Lite Maven references:
+
----
Dependencies {
...
Where <pathToCbl> is the location of the downloaded Couchbase{nbsp}Lite library.
