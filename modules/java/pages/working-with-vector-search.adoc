...
You procedurally update the vectors in the index by looping through the vectors in batches equivalent to the `limit` parameter.
...
Must match the expression specified in the vector index.
|target-vector
|image:ROOT:yes.png[]
|The target vector.
|metric
