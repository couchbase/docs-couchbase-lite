...
This means that there will be 100 buckets with a single centroid each that gathers together similar vectors.
...
IMPORTANT: Similar to the xref:java:fts.adoc[Full Text Search] `match()` function, `vector_match()` can only be called alone or at the top level `AND` expression.
