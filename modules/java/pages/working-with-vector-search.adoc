...
To configure a project to use vector search, follow the xref:java:gs-install.adoc[installation instructions] to add the Vector Search extension.
...
For more information on specific configurations, see xref:java:vector-search.adoc[Vector Search.]
...
|Distance Metric
|image:couchbase-lite/current/_images/no.png[]
...
IMPORTANT: Similar to the xref:java:fts.adoc[Full Text Search] `match()` function, the `APPROX_VECTOR_DISTANCE()` function and Hybrid Vector Search cannot use the `OR` expression with the other expressions in the related `WHERE` clause.
...
[#approx_vector_distancevector-expr-target-vector-metric-nprobes-accurate]
