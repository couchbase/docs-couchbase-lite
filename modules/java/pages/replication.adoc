...
Related Content -- xref:java:conflict.adoc[Handling Data Conflicts] | xref:java:dbreplica.adoc[Intra-Device] | <<p2psync-websocket.adoc,Peer-to-Peer>>
--
...
To use this protocol the replication URL should specify WebSockets as the URL scheme (see the xref:java:replication.adoc#lbl-cfg-tgt[] section below).
...
xref:java:replication.adoc#ex-simple-repl[] shows the configuration and initialization process.
...
By default, clients accept only servers presenting certificates that can be verified using the OS bundled Root CA Certificates -- see: xref:java:replication.adoc#lbl-svr-auth[].
...
Other options are available -- see: xref:java:replication.adoc#lbl-client-auth[].
...
<.> Initialize the replicator using your configuration -- see: xref:java:replication.adoc#lbl-init-repl[].

<.> Optionally, register an observer, which will notify you of changes to the replication status -- see: xref:java:replication.adoc#lbl-repl-mon[]

<.> Start the replicator  -- see: xref:java:replication.adoc#lbl-repl-start[].
...
+
--
xref:java:replication.adoc#lbl-cfg-tgt[]
|  xref:java:replication.adoc#lbl-cfg-sync[]
|  xref:java:replication.adoc#lbl-cfg-keep-alive[]
|  xref:java:replication.adoc#lbl-user-auth[]
|  xref:java:replication.adoc#lbl-svr-auth[]
|  xref:java:replication.adoc#lbl-client-auth[]
|  xref:java:replication.adoc#lbl-repl-evnts[]
|  xref:java:replication.adoc#lbl-repl-hdrs[]
|  xref:java:replication.adoc#lbl-repl-ckpt[]
|  xref:java:replication.adoc#lbl-repl-fltrs[]
|  xref:java:replication.adoc#lbl-repl-chan[]
|  xref:java:replication.adoc#anchor-auto-purge-on-revoke[]
|  xref:java:replication.adoc#lbl-repl-delta[]
--
...
The REST API provides configurable control over this replication retry logic using a set of configiurable properties -- see: xref:java:replication.adoc#tbl-repl-retry[].
...
When necessary you can adjust any or all of those configurable values -- see: xref:java:replication.adoc#ex-repl-retry[] for how to do this.
...
You can enable authorization in the sync{nbsp}gateway configuration file, as shown in xref:java:replication.adoc#example-enable-authorization[].
...
This all assumes that you have configured the Sync Gateway to provide the appropriate SSL certificates, and have included the appropriate certificate in your app bundle -- for more on this see: xref:java:replication.adoc#lbl-cert-pinning[].


[#lbl-client-auth]
=== Client Authentication

There are two ways to authenticate from a Couchbase Lite client: xref:java:replication.adoc#basic-authentication[] or xref:java:replication.adoc#session-authentication[].
...
xref:java:replication.adoc#ex-base-auth[] shows how to initiate a one-shot replication as the user *username* with the password *password*.
...
See xref:java:replication.adoc#ex-session-auth[], which shows how to initiate a one-shot replication with the session ID returned from the `+POST /{tkn-db}/_session+` endpoint.
...
This feature is useful in passing additional credentials, perhaps when an authentication or authorization step is being done by a proxy server (between Couchbase Lite and Sync Gateway) -- see xref:java:replication.adoc#ex-cust-hdr[].
...
For more information, see xref:java:replication.adoc#auto-purge-on-revoke[Auto Purge on Revoke].
****
...
If a user subsequently regains access to a lost channel, then any previously auto-purged documents still assigned to any of their channels are automatically pulled down by the active Sync Gateway when they are next updated -- see behavior summary in xref:java:replication.adoc#tbl-regain-behavior[]
...
In this section::
xref:java:replication.adoc#lbl-repl-start[]  | xref:java:replication.adoc#lbl-repl-ckpt[]
...
You can, optionally, add a change listener (see xref:java:replication.adoc#lbl-repl-mon[]) before starting the replicator running using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/AbstractReplicator.html#start-boolean-[start()].
...
However, if you do want to force the replication to start again from zero, use the xref:refer-glossary.adoc#checkpoint[checkpoint] reset argument when starting the replicator -- as shown in xref:java:replication.adoc#ex-repl-ckpt[].
...
In this section::
xref:java:replication.adoc#lbl-repl-chng[]  |
xref:java:replication.adoc#lbl-repl-status[]  |
xref:java:replication.adoc#lbl-repl-evnts[] |
xref:java:replication.adoc#lbl-repl-pend[] |
xref:java:replication.adoc#lbl-repl-correlation-id[]

You can monitor a replication's' status by using a combination of xref:java:replication.adoc#lbl-repl-chng[] and the `replication.status.activity` property -- see; https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ReplicatorStatus.html#getActivityLevel--[getActivityLevel()].
This enables you to know, for example, when the replication is actively transferring data and when it has stopped.

You can also choose to monitor document changes -- see: xref:java:replication.adoc#lbl-repl-evnts[].

[#lbl-repl-chng]
=== Change Listeners
Use this to monitor changes and to inform on sync progress; this is an optional step.
You can add and a replicator change listener at any point; it reports changes from the point it's' registered.

.Best Practice
TIP: Don't' forget to save the token so you can remove the listener later

Use the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/Replicator.html[Replicator] class to add a change listener as a callback to the Replicator (https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/AbstractReplicator.html#addChangeListener-java.util.concurrent.Executor-com.couchbase.lite.ReplicatorChangeListener-[addChangeListener()]) -- see: xref:java:replication.adoc#ex-repl-mon[].
...
That is, whether it is actively transferring data or if it has stopped -- see: xref:java:replication.adoc#ex-repl-mon[].

The returned _ReplicationStatus_ structure comprises:

* https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ReplicatorStatus.html#getActivityLevel--[getActivityLevel()] -- stopped, offline, connecting, idle or busy -- see states described in: xref:java:replication.adoc#tbl-states[]
...
[#lbl-repl-states]
==== Replication States
xref:java:replication.adoc#tbl-states[] shows the different states, or activity levels, reported in the API; and the meaning of each.
...
For example, the code snippet in xref:java:replication.adoc#ex-reg-doc-listener[] registers a listener to monitor document replication performed by the replicator referenced by the variable `replicator`.
It prints the document ID of each document received and sent.
Stop the listener as shown in xref:java:replication.adoc#ex-stop-doc-listener[].
...
It will stop any active xref:java:replication.adoc#lbl-repl-chng[change listener] once the replication is stopped.
...
You can increase the log output for activity related to replication with Sync Gateway -- see xref:java:replication.adoc#ex-logs[].
