...
xref:java:p2psync-websocket-using-active.adoc#simple-replication-to-listener[] shows the initialization and configuration process.
...
By default, clients accept only servers presenting certificates that can be verified using the OS bundled Root CA Certificates -- see: xref:java:p2psync-websocket-using-active.adoc#authenticate-listener[].

<.> Configure the credentials the client will present to the server.
Here we say to provide _Basic Authentication_ credentials. Other options are available -- see: xref:java:p2psync-websocket-using-active.adoc#configuring-client-authentication[].

<.> Configure how the replication should perform xref:java:p2psync-websocket-using-active.adoc#conflict-resolution[].
...
== Configure Replicator
In this section::
xref:java:p2psync-websocket-using-active.adoc#lbl-cfg-tgt[]
|  xref:java:p2psync-websocket-using-active.adoc#lbl-cfg-sync[]
|  xref:java:p2psync-websocket-using-active.adoc#lbl-cfg-retry[]
|  xref:java:p2psync-websocket-using-active.adoc#authenticate-listener[]
|  xref:java:p2psync-websocket-using-active.adoc#lbl-authclnt[]
...
The REST API provides configurable control over this replication retry logic using a set of configiurable properties -- see: xref:java:p2psync-websocket-using-active.adoc#tbl-repl-retry[].
...
When necessary you can adjust any or all of those configurable values -- see: xref:java:p2psync-websocket-using-active.adoc#ex-repl-retry[] for how to do this.
...
You can, optionally, add a change listener (see xref:java:p2psync-websocket-using-active.adoc#lbl-repl-mon[]) before starting the replicator running using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/AbstractReplicator.html#start-boolean-[start()].
...
In this section::
xref:java:p2psync-websocket-using-active.adoc#lbl-repl-chng[]  |
xref:java:p2psync-websocket-using-active.adoc#lbl-repl-status[]  |
xref:java:p2psync-websocket-using-active.adoc#lbl-repl-pend[]

You can monitor a replication’s status by using a combination of xref:java:p2psync-websocket-using-active.adoc#lbl-repl-chng[] and the `replication.status.activity` property -- see; https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ReplicatorStatus.html#getActivityLevel--[getActivityLevel()].
...
That is, whether it is actively transferring data or if it has stopped -- see: xref:java:p2psync-websocket-using-active.adoc#ex-repl-mon[].

The returned _ReplicationStatus_ structure comprises:

* https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ReplicatorStatus.html#getActivityLevel--[getActivityLevel()] -- stopped, offline, connecting, idle or busy -- see states described in: xref:java:p2psync-websocket-using-active.adoc#tbl-states[]
...
[#lbl-repl-states]
=== Replication States
xref:java:p2psync-websocket-using-active.adoc#tbl-states[] shows the different states, or activity levels, reported in the API; and the meaning of each.
...
It will stop any active xref:java:p2psync-websocket-using-active.adoc#lbl-repl-chng[change listener] once the replication is stopped.
...
To use a different policy, specify a _conflict resolver_ using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-java}{empty}/couchbase-lite-java/com/couchbase/lite/ReplicatorConfiguration.html#setConflictResolver-com.couchbase.lite.ConflictResolver-[conflictResolver] as shown in xref:java:p2psync-websocket-using-active.adoc#using-conflict-resolvers[].
...
To address that possibility, a version of the `Database.save()` method also takes a conflict resolver as shown in xref:java:p2psync-websocket-using-active.adoc#ex-merge-props[].
