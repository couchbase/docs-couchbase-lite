...
The `openssl` command can be used to create a new self-signed certificate and convert the `.pem` file to a `.cert` file (see xref:sync-gateway::security.adoc#creating-your-own-self-signed-certificate[creating your own self-signed certificate]).
You should then have 3 files: `cert.pem`, `cert.cer` and `key.pem`.

The `cert.pem` and `key.pem` can be used in the Sync Gateway configuration (see xref:sync-gateway::security.adoc#installing-the-certificate[installing the certificate]).
