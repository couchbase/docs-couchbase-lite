...
. *Pre-release only:* Add `http://mobile.nuget.couchbase.com/nuget/Developer/` to your Nuget package sources.
...
+
. Your app must call the relevant `Activate()` function inside of the class that is included in the support assembly (there is only one public class in each support assembly, and the support assembly itself is a nuget dependency).
+
For example, UWP looks like `Couchbase.Lite.Support.UWP.Activate()`.
Currently the support assemblies provide dependency injected mechanisms for default directory logic, and platform specific logging (i.e.
Android will log to logcat with correct log levels and tags.
No more "mono-stdout" at always info level).
...
* Install the Couchbase Lite 2.0 framework in your project (see the link:csharp.html#getting-started[installation options]).
...
This functionality is related to the way conflicts are being handled in Couchbase Lite 2.0 (see link:swift.html#handling-conflicts[Handling Conflicts]).
...
Therefore, to use replication with Couchbase Lite 2.0, the target Sync Gateway instance must also be upgraded to 2.0.

Sync Gateway 2.0 will continue to accept clients that connect through the 1.x protocol.
...
This allows for a smoother transition to get all your user base onto a version of your application built with Couchbase Lite 2.0.
...
Alternatively, the `Database(string name, DatabaseConfiguration config)` initializer can be used to provide specific options in the http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/db022/html/T_Couchbase_Lite_DatabaseConfiguration.htm[`DatabaseConfiguration`] object such as the database directory.
...
* .NET Core: `Path.Combine(AppContext.BaseDirectory, "CouchbaseLite")` (unless the app context is altered [e.g.
by XUnit], this will be the same directory as the output binary)
...
The `Document` class now offers a set of http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/db022/html/T_Couchbase_Lite_DictionaryObject.htm[`property accessors`] for various scalar types, including boolean, integers, floating-point and strings.
...
To check whether a given property exists in the document, you should use the http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/db022/html/M_Couchbase_Lite_Document_Contains.htm[`Document.Contains(string key)`] method.
...
The http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/db022/html/T_Couchbase_Lite_Query_IExpression.htm[comparison operators] can be used in the WHERE statement to specify on which property to match documents.
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/db022/html/T_Couchbase_Lite_Query_ArrayFunction.htm[Collection operators] are useful to check if a given value is present in an array.
...
The http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/db022/html/M_Couchbase_Lite_Query_IExpression_Like.htm[`like`] operator can be used for string matching.
...
The http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/db022/html/M_Couchbase_Lite_Query_IExpression_Regex.htm[`regex`] operator can be used for case sensitive matches.
...
This protocol has been designed to be fast, efficient, easier to implement, and symmetrical between the client and server.
...
For platform specific installation instructions, refer to the Sync Gateway https://developer.couchbase.com/documentation/mobile/1.5/installation/sync-gateway/index.html[installation guide].

=== Starting a Replication

Replication objects are now bidirectional, this means you can start a `push`/`pull` replication with a single instance.
The replication's parameters can be specified through the http://docs.couchbase.com/mobile/2.0/couchbase-lite-net/db022/html/T_Couchbase_Lite_Sync_ReplicatorConfiguration.htm[`ReplicatorConfiguration`] object; for example, if you wish to start a `push` only or `pull` only replication.
...
Some load balancers are not configured for WebSocket connections by default (NGINX for example); so it might be necessary to explicitly enable them in the load balancer's configuration (see https://developer.couchbase.com/documentation/mobile/1.5/guides/sync-gateway/nginx/index.html[Load Balancers]).

By default, the WebSocket protocol uses compression to optimize over speed and bandwidth.
...
- https://github.com/couchbase/couchbase-lite-net/issues/1005[*#1005*] Add Encryption to EE
- https://github.com/couchbase/couchbase-lite-net/issues/1006[*#1006*] Add P2P

*Bugs*

- https://github.com/couchbase/couchbase-lite-net/issues/1000[*#1000*] 2.0. Xamarin. Replicator fails to get out of Connecting State when connecting to unreachable host
- https://github.com/couchbase/couchbase-lite-net/issues/1010[*#1010*] [Xamarin][db023] continuus synchronisation stopping after a while
- https://github.com/couchbase/couchbase-lite-net/issues/1011[*#1011*] Xamarin Android cannot recover from loss of connection to LAN address
- https://github.com/couchbase/couchbase-lite-net/issues/1012[*#1012*] Reusing the same collation in a query will cause all of them to be affected by the last modification
- https://github.com/couchbase/couchbase-lite-net/issues/1020[*#1020*] Closing/Disposing database throws Exception because of unfinished replicators
- https://github.com/couchbase/couchbase-lite-net/issues/1048[*#1048*] Couchbase Lite parsing nested value as UInt64 then throwing exception on update.
- https://github.com/couchbase/couchbase-lite-net/issues/1051[*#1051*] Heap corruption/Access Violation with unreachable replication endpoint on Windows x86

*Known Issues*

- https://github.com/couchbase/couchbase-lite-net/issues/1040[*#1040*] HTTP Proxy Support
...
* https://github.com/couchbase/couchbase-lite-net/issues/647[*#647*] Equivalent to full text search
* https://github.com/couchbase/couchbase-lite-net/issues/964[*#964*] AddLoggerProvider

*Bugs*

* https://github.com/couchbase/couchbase-lite-net/issues/884[*#884*] Replication with DNS endpoint failing on (at least) Mac
* https://github.com/couchbase/couchbase-lite-net/issues/894[*#894*] Crash on iOS Simulator 9.x on Query.Run() execution
* https://github.com/couchbase/couchbase-lite-net/issues/957[*#957*] SG doesn't have any docs in its cache after replication
* https://github.com/couchbase/couchbase-lite-net/issues/966[*#966*] Incorrect callback type(?) leads to AOT compilation failure for Xamarin
* https://github.com/couchbase/couchbase-lite-net/issues/994[*#994*] 2.0db023 SetBinaryLogDirectory(string) path ignored, still written to AppContext.BaseDirectory

*Known Issues*

* https://github.com/couchbase/couchbase-lite-net/issues/1002[*#1002*] System proxy is not being used for replication
* https://github.com/couchbase/couchbase-lite-net/issues/1012[*#1012*] Reusing the same collation in a query will cause all of them to be affected by the last modification
