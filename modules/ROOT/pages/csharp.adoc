...
:url-api-references: http://docs.couchbase.com/mobile/2.5.0/couchbase-lite-net
...
Alternatively, the `Database(string name, DatabaseConfiguration config)` initializer can be used to provide specific options in the {url-api-references}/html/T_Couchbase_Lite_DatabaseConfiguration.htm[`DatabaseConfiguration`] object such as the database directory.
...
If the database does not exist, the app should copy it from the app bundle using the {url-api-references}/html/M_Couchbase_Lite_Database_Copy.htm[`Database.Copy(string, DatabaseConfiguration)`] method as shown below.
...
The `Document` class now offers a set of {url-api-references}/html/T_Couchbase_Lite_DictionaryObject.htm[`property accessors`] for various scalar types, including boolean, integers, floating-point and strings.
...
To check whether a given property exists in the document, you should use the {url-api-references}/html/M_Couchbase_Lite_Document_Contains.htm[`Document.Contains(string key)`] method.
...
The {url-api-references}/html/T_Couchbase_Lite_Query_IExpression.htm[comparison operators] can be used in the WHERE statement to specify on which property to match documents.
...
{url-api-references}/html/T_Couchbase_Lite_Query_ArrayFunction.htm[Collection operators] are useful to check if a given value is present in an array.
...
The {url-api-references}/html/M_Couchbase_Lite_Query_IExpression_Like.htm[`like`] operator can be used for string matching.
...
The {url-api-references}/html/M_Couchbase_Lite_Query_IExpression_Regex.htm[`regex`] operator is case sensitive.
...
The replication's parameters can be specified through the {url-api-references}/html/T_Couchbase_Lite_Sync_ReplicatorConfiguration.htm[`ReplicatorConfiguration`] object;
...
Couchbase Lite does not do anything to a replication when the app is backgrounded.
It will continue running as long as the remote system does not terminate the connection and the app does not terminate.
