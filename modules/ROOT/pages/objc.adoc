...
The getting started instructions are not currently available in Objective-C. Refer to the xref:swift.adoc#getting-started[Swift] version to find installation instructions.
...
Refer to the https://docs.couchbase.com/tutorials/todo-app/develop/swift/adding-synchronization.html#resolve-conflicts[Resolve Conflicts] section of the `TodoApp` tutorial to learn how to resolve conflicts in your application.
...
* *Extended type support:* Transparent support for common types that don't have a JSON representation, like NSDate and NSData.
You can even represent references to other model objects.
* *Mutable state:* Properties can be `readwrite`, so they can be changed in memory, then later saved back to the document.
* *Key-Value Observing:* You can observe the value of a property and get notified when it changes.
On Mac OS X, you can also use bindings to connect properties to UI controls.
* *Dynamic typing:* You can use the `CBLModelFactory` to associate each model class with a document type.
You can create a hierarchy of model classes and have the appropriate subclass instantiated at runtime according to the document's type.
...
* *Booleans:* Declare boolean properties as the C99 type `bool`, not the legacy Objective-C type `BOOL`.
(Why not?
The latter is really just a typedef for `char`, so at runtime it just looks like an 8-bit integer, which means it'll be stored in JSON as 0 or 1, not true or false.)
...
* *NSData:* JSON doesn't support binary data, so the data will be encoded as Base64 and stored as a string.
(The size and CPU overhead of the conversion make this inefficient for large data.
Consider using an attachment instead.)
* *Other CBLModel classes:* You can create a one-to-one references to another model object by declaring a persistent property whose type is a CBLModel subclass.
The value will be persisted as a JSON string containing the document ID of the model object.
* *NSArray:* An NSArray is saved as a JSON array, with each element of the array converted to JSON according to the rules in this section.
...
See below.
* *Any class implementing CBLJSONEncoding:* `CBLJSONEncoding` is a protocol defined by Couchbase Lite.
Any class of yours that implements this protocol can be used as the type of a persistent property;
CBLModel will call the CBLJSONEncoding API to tell the object to convert itself to/from JSON.
...
it will _not_ be written with a JSON `null` as a value.
...
*CBLModel* has no public initializer methods, and you should not implement any yourself in subclasses.
...
The `+modelForDocument:` method is inherited from CBLModel;
...
Once you've done this, you can now call `+modelForDocument:` directly on CBLModel, instead of on your subclass;
...
This doesn't look much different from the usual style;
...
Sometimes you don't want to save immediately, but you want to get the model's current state as a JSON object, i.e. the data that _would_ be saved to the document.
...
* https://github.com/couchbase/couchbase-lite-ios/issues/1379[#1379] ForestDB update_seq on view query less than db's update_seq

.Enhancements
* https://github.com/couchbase/couchbase-lite-ios/issues/1400[#1400] Support for _oidc_refresh response without ID token

.Bugs
* https://github.com/couchbase/couchbase-lite-ios/issues/1369[#1369] PSWebSocket crash iOS 10
* https://github.com/couchbase/couchbase-lite-ios/issues/1379[#1379] ForestDB update_seq on view query less than db's update_seq
* https://github.com/couchbase/couchbase-lite-ios/issues/1387[#1387] TvOS Unit Test Build has a lot of warnings
* https://github.com/couchbase/couchbase-lite-ios/issues/1392[#1392] When OIDC Refresh token failed, there is no replication change notification sent.
* https://github.com/couchbase/couchbase-lite-ios/issues/1403[#1403] CBLSocketChangeTracker endless loop of JSON Parsing Errors
* https://github.com/couchbase/couchbase-lite-ios/issues/1406[#1406] Encryption is a no-op with the system libsqlite3 on macOS 12

=== 1.3 release

.Performance Improvements
* https://github.com/couchbase/couchbase-lite-ios/issues/1114[#1114] LiveQuery shouldn't re-run query if view index didn't change
* https://github.com/couchbase/couchbase-lite-ios/issues/1150[#1150] Optimize & animate CBLUITableController by using the Levenshtein algorithm
* https://github.com/couchbase/couchbase-lite-ios/issues/1165[#1165] Incrementally purge oldest revisions in SQLite databases
* https://github.com/couchbase/couchbase-lite-ios/issues/1242[#1242] Fix inefficiencies replicating docs with many revisions
* https://github.com/couchbase/couchbase-lite-ios/issues/1340[#1340] Optimized multipart download performance

.Enhancements
* https://github.com/couchbase/couchbase-lite-ios/issues/394[#394] Start using NSURLSession in the replicator
* https://github.com/couchbase/couchbase-lite-ios/issues/984[#984] Comparing CBLQuery objects
* https://github.com/couchbase/couchbase-lite-ios/issues/1100[#1100] External property on kCBLDatabaseChangeNotification when CBL is remote
* https://github.com/couchbase/couchbase-lite-ios/issues/1107[#1107] Logging overhaul
* https://github.com/couchbase/couchbase-lite-ios/issues/1117[#1117] Redact passwords/tokens in URLs in log messages
* https://github.com/couchbase/couchbase-lite-ios/issues/1118[#1118] API: Public method for inserting existing revisions (putExistingRevisionWithProperties:...)
* https://github.com/couchbase/couchbase-lite-ios/issues/1148[#1148] API: Make -[View updateIndex] public; add async version
* https://github.com/couchbase/couchbase-lite-ios/issues/1150[#1150] Optimize & animate CBLUITableController by using the Levenshtein algorithm
* https://github.com/couchbase/couchbase-lite-ios/issues/1165[#1165] Incrementally purge oldest revisions in SQLite databases
* https://github.com/couchbase/couchbase-lite-ios/issues/1181[#1181] API: Document expiration time (TTL)
* https://github.com/couchbase/couchbase-lite-ios/issues/1201[#1201] Replicator should immediately fail if URL path is invalid
* https://github.com/couchbase/couchbase-lite-ios/issues/1245[#1245] Expose server error messages in NSErrors from public API
* https://github.com/couchbase/couchbase-lite-ios/issues/1266[#1266] OpenID Connect authenticator
* https://github.com/couchbase/couchbase-lite-ios/issues/1271[#1271] Disable App Transport Security on Mac OSX LiteServ
* https://github.com/couchbase/couchbase-lite-ios/issues/1291[#1291] Allow configuring revs_limit in LiteServ
* https://github.com/couchbase/couchbase-lite-ios/issues/1334[#1334] Scope cookie storage by database, not by replication

.Bugs
* https://github.com/couchbase/couchbase-lite-ios/issues/641[#641] Enumerate sequence in descending order in CBForest not working
* https://github.com/couchbase/couchbase-lite-ios/issues/966[#966] CBLIS : Data loss
* https://github.com/couchbase/couchbase-lite-ios/issues/1005[#1005] Replication fails without starting on wifi if network depends on a proxy
* https://github.com/couchbase/couchbase-lite-ios/issues/1015[#1015] Crash when [CBLRemoteRequest connection:didFailWithError:]
* https://github.com/couchbase/couchbase-lite-ios/issues/1045[#1045] POST _bulk_docs does not support HTTPBodyStream
* https://github.com/couchbase/couchbase-lite-ios/issues/1071[#1071] [Testfest] Items not showing up in offline mode
* https://github.com/couchbase/couchbase-lite-ios/issues/1085[#1085] CBLIS : NSIncrementalStore accessed by multiple context dispatch queues
* https://github.com/couchbase/couchbase-lite-ios/issues/1095[#1095] Obsolete rows left in view index when documentType is used (1.1.1)
* https://github.com/couchbase/couchbase-lite-ios/issues/1106[#1106] CBLListener readOnly property prevents pulls from updating the database
* https://github.com/couchbase/couchbase-lite-ios/issues/1120[#1120] Warn if map function calls emit(nil, ...)
* https://github.com/couchbase/couchbase-lite-ios/issues/1124[#1124] Multipart Uploader cannot upload the attachment when using basic auth
* https://github.com/couchbase/couchbase-lite-ios/issues/1128[#1128] Crash pushing docs with non-downloaded attachments
* https://github.com/couchbase/couchbase-lite-ios/issues/1129[#1129] CBLIS: NSSet was mutated while being enumerated.
* https://github.com/couchbase/couchbase-lite-ios/issues/1131[#1131] CBLVersion() incorrectly returns "(unofficial)" in official releases
* https://github.com/couchbase/couchbase-lite-ios/issues/1132[#1132] CBLReplicator.isDocumentPending is inaccurate when replicator is offline
* https://github.com/couchbase/couchbase-lite-ios/issues/1138[#1138] Continuous puller with WebSocket not responding with 401 error when changing password
* https://github.com/couchbase/couchbase-lite-ios/issues/1139[#1139] ChangeTracker POST request are incompatible with CouchDB
* https://github.com/couchbase/couchbase-lite-ios/issues/1143[#1143] Swift API glitches
* https://github.com/couchbase/couchbase-lite-ios/issues/1152[#1152] CBLRestReplicator does not properly end bgTask when app is foregrounded
* https://github.com/couchbase/couchbase-lite-ios/issues/1163[#1163] Fixed macOS 10.10 dependency & updated deployment version to 10.9
* https://github.com/couchbase/couchbase-lite-ios/issues/1180[#1180] Doc can be saved with missing attachment file, leading to errors
* https://github.com/couchbase/couchbase-lite-ios/issues/1188[#1188] CBForest bad memory access while purging doc
* https://github.com/couchbase/couchbase-lite-ios/issues/1191[#1191] Liteserv crashes with Forestdb option when creating local docs after delete operation
* https://github.com/couchbase/couchbase-lite-ios/issues/1192[#1192] Manual compaction returns error if auto-compact is already running
* https://github.com/couchbase/couchbase-lite-ios/issues/1200[#1200] Replication rejects attachments generated by PouchDB with revpos:0
* https://github.com/couchbase/couchbase-lite-ios/issues/1216[#1216] REST API: POST /_replicate can start/stop once, but not restart?
* https://github.com/couchbase/couchbase-lite-ios/issues/1228[#1228] One-shot push from empty db never stops
* https://github.com/couchbase/couchbase-lite-ios/issues/1233[#1233] Failure to encrypt SQLite database when using WAL journaling
* https://github.com/couchbase/couchbase-lite-ios/issues/1260[#1260] Inconsistent response when no conflicts are present and ?conflicts=true
* https://github.com/couchbase/couchbase-lite-ios/issues/1263[#1263] Incorrect "error" property in REST response for GET of deleted doc
* https://github.com/couchbase/couchbase-lite-ios/issues/1274[#1274] PendingDocumentIDs is incorrect
* https://github.com/couchbase/couchbase-lite-ios/issues/1279[#1279] Don't stop replicator if one doc gets an error
* https://github.com/couchbase/couchbase-lite-ios/issues/1292[#1292] _replicate between 2 dbs on same LiteServ fails
* https://github.com/couchbase/couchbase-lite-ios/issues/1323[#1323] Cannot push encrypted attachments when testing with ToDoLite
* https://github.com/couchbase/couchbase-lite-ios/issues/1325[#1325] LiteServ crash when delete dbs asyncronously
* https://github.com/couchbase/couchbase-lite-ios/issues/1327[#1327] Archive build is missing symbols in iOS frameworks
* https://github.com/couchbase/couchbase-lite-ios/issues/1335[#1335] Database upgrades from 1.1 not copying "content_type" of attachments
* https://github.com/couchbase/couchbase-lite-ios/issues/1348[#1348] OIDC tokens in Keychain should be per-database
* https://github.com/couchbase/couchbase-lite-ios/issues/1362[#1362] App crashing on iOS 8.3 device
* https://github.com/couchbase/couchbase-lite-ios/issues/1370[#1370] TodoLite showing wrong list when deleting tasks