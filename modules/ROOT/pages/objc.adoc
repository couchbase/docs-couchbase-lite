...
link:[Objective-C API References]
...
To do that, in the target's Build Phases editor, press the "+" button below the "Link
...
[quote]
Remember: *a view is not a query, it`'s an index.*
...
* https://github.com/couchbase/couchbase-lite-ios/issues/1461[#1461] ForestDB: Assertion failure in -link:[MYBackgroundMonitor beginBackgroundTaskNamed:] MYBackgroundMonitor.m:66
