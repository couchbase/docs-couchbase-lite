...
{cbl-te}_'s_ support for pre-built databases means you can pre-load your app with data instead of syncing it down from {sgw-te} during startup.
...
Then initiate a sync to update the data -- see <<deploy-db>>
****
...
If you are using en encrypted database, note that
