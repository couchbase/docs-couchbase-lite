...
[IMPORTANT]
--
*Always start with a fresh database for each app version*; this ensures there are no {_glos-pfx}checkpoint[checkpoint] issues

*Otherwise:*  You will invalidate the cached {_glos-pfx}checkpoint[checkpoint] in the packaged database, and instead reuse the same database in your build process (for subsequent app versions).
--
...
*Otherwise:* ... there will be a {_glos-pfx}checkpoint[checkpoint] mismatch and the app will attempt to pull the data down again
...
* You can write a simple CBL app to just initiate the required pull sync -- see: {cbl--xref} replication.adoc[Remote Sync Gateway]
...
*Otherwise:* Each copy of the app will invalidate the other apps' {_glos-pfx}checkpoint[checkpoints] because a new UUID was not generated.

. Open the database; you can now start querying the data and using it

. Start a pull replication, to sync any changes
+
The replicator uses the pre-built database's {_glos-pfx}checkpoint[checkpoint] as the timestamp to sync from; only documents changed since then are synced
+
[IMPORTANT]
--
...
*Otherwise:* You will get a {_glos-pfx}checkpoint[checkpoint] mismatch. +
