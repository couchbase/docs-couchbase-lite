...
:param-related: {cbl--xref} conflict.adoc[Handling Data Conflicts] | {cbl--xref} dbreplica.adoc[Intra-Device] | xref: p2psync-websocket.adoc[Peer-to-Peer]
...
include::ROOT:partial$block-caveats.adoc[tags=code-disclaimer]
...
And since Couchbase Lite 2.x+ only supports the new protocol, you will need to run a version of Sync Gateway that supports it -- see: {cbl--xref}compatibility.adoc[Compatibility].
...
[NOTE]
--
include::ROOT:partial$block-caveats.adoc[tags=android-threads]
--
endif::[]
...
<.> Configure how the replication should handle conflict resolution -- see: {cbl--xref} conflict.adoc[Handling Data Conflicts] topic for mor on conflict resolution.
...
include::ROOT:partial$commons/common-sgw-replication-cfg-tgt.adoc[]
...
include::ROOT:partial$commons/common-sgw-replication-cfg-network-interface.adoc[]
...
include::ROOT:partial$commons/common-sgw-replication-cfg-sync-mode.adoc[]

[#lbl-cfg-keep-alive]
=== Retry Configuration

include::ROOT:partial$commons/common-sgw-replication-cfg-retry.adoc[]
...
include::ROOT:partial$commons/common-set-server-authentication.adoc[]
...
.Push Filter
include::ROOT:partial$block_tabbed_code.adoc[]
:param-tags!:
...
{cbl--xref} dbreplica.adoc[Intra-Device]
replications automatically *disable* delta sync, whilst
xref: p2psync-websocket.adoc[Peer-to-Peer]
...
include::ROOT:partial$commons/common-sgw-replication-init.adoc[]
...
Replicators use {_glos-pfx}checkpoint[checkpoints] to keep track of documents sent to the target database.

Without {_glos-pfx}checkpoint[checkpoints], Couchbase Lite would replicate the entire database content to the target database on each connection, even though previous replications may already have replicated some or all of that content.

This functionality is generally not a concern to application developers.
However, if you do want to force the replication to start again from zero, use the {_glos-pfx}checkpoint[checkpoint] reset argument when starting the replicator -- as shown in <<ex-repl-ckpt>>.
...
include::ROOT:partial$commons/common-sgw-replication-monitor.adoc[leveloffset=+1]
...
include::ROOT:partial$commons/common-sgw-replication-stop.adoc[]
...
For more on troubleshooting with logs, see: {cbl--xref} troubleshooting-logs.adoc[Using Logs].
endif::is-c[]
