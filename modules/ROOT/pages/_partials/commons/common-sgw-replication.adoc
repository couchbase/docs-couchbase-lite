...
As part of the syncing setup, the Gateway has to map the Couchbase Lite database to the database being synced against on Capella. 
...
include::{root-partials}block_tabbed_code_example.adoc[]
:param-tags!:
<.> Here we have opted to turn off the auto purge behavior. By default auto purge is enabled (false)
