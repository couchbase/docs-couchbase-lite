...
A query can only be fast if there's a pre-existing database index it can search to narrow down the set of documents to examine -- see: <<lbl-indexing>>, which shows how to create an index, and also the {query-troubleshooting--xref} topic.
...
footnote:[Starting in Couchbase Lite 2.5] as shown in <<ex-del-qry>>.
