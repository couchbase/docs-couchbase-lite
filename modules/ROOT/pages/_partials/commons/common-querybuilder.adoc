...
Couchbase Lite for {param-title} provides two ways to build and run database queries; the QueryBuilder API described in this topic and the {query-n1ql-mobile--xref}.

Database queries defined with the QueryBuilder API use query statements of the form shown in <<ex-query-form>>.
The structure and semantics of the query format are based on that of Couchbase's xref:server:learn:data/n1ql-versus-sql.adoc[{sqlpp_cbm} query language].
...
A query can only be fast if there's a pre-existing database index it can search to narrow down the set of documents to examine -- see: <<ex-indexing>>, which shows how to create an index, and also the {query-troubleshooting--xref} topic.

TIP: See the {indexing--xref} topic to learn more about indexing.

A query can only be fast if there's a pre-existing database index it can search to narrow down the set of documents to examine -- see: <<ex-indexing>>, which shows how to create an index and our {query-troubleshooting--xref} topic.

However, every index has to be updated whenever a document is updated.
So too many indexes can hurt performance.
