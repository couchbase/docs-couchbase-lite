...
Client can instantiate any number of “WebsocketEndpointListener” lnstances, each serving a different database.
...
A passive client that is listening for connections via WebsocketEndpointListener must be capable of actively opening up replications to other passive WebsocketMessageEndpointListener clients .
