...
The structure and semantics of the query format are based on that of Couchbase Server's {sqlpp} query language -- see {svr--xref}n1ql:n1ql-language-reference/index.adoc[{sqlpp} Reference Guide] and {svr--xref}learn:data/n1ql-versus-sql.adoc[{sqlpp} Data Model].


== Running

The database can create a query object with the SQL++ string.
See {cbl--xref} query-resultsets.adoc[Query Resultsets] for how to work with result sets.
...
See: {cbl--xref} query-resultsets.adoc[Query Resultsets] for more on processing query results.
