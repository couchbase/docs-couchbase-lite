...
|Function |Description |Arguments |Return Value |Further Information
...
If `date1` is not a valid date format, `null` is returned.
|`STR_TO_TZ(date1, tz)`
...
|If `date1` is not an integer, `null` is returned.
|`MILLIS_TO_UTC(date1)`
...
|Returns an integer representing the elapsed time (based on the specified `part`) between both dates.
a|

* If `date1` is greater than `date2` then the value returned will be positive, otherwise the returned value is negative.

* If `date1` or `date2` are not valid string formats, `null` is returned.

* If an invalid `part` is passed to the function, `null` is returned.
|`DATE_DIFF_MILLIS(date1, date2, part)`
|Finds the elapsed time between two Epoch/UNIX timestamps. This elapsed time is measured from `date2` to  `date1`. 
a|
...
* If `date1` or `date2` are not integers, `null` is returned.
...
* If `date1` is not a valid string format, `null is returned`.

* If a non-integer is passed to the argument `n`, `null` is returned.
