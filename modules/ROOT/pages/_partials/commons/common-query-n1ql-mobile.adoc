...
.Date and Time Functions
[tbl-func-agg,cols="1m,10", options="header"]
|===
...
|STR_TO_UTC(expr)
|Returns the ISO 8601 UTC date string of the given ISO 8601 date input string.

|STR_TO_TZ(expr)
|Returns the current system timezone date in string format of the given ISO 8601 date input string.

|MILLIS_TO_STR(expr)
|Returns a ISO 8601 date string in device local timezone of the given number of milliseconds since the unix epoch expression.

|MILLIS_TO_UTC(expr)
|Returns the UTC ISO 8601 date string of the given number of milliseconds since the unix epoch expression.

|MILLIS_TO_TZ(expr)
|Returns the current system timezone date in string format of the given number of milliseconds since the unix epoch expression.

|DATE_DIFF_STR(expr)
|Returns the elapsed time between two ISO 8601 date input strings as a date string.

|DATE_DIFF_MILLIS(expr)
|Returns the elapsed time between two unix timestamps in string format.

|DATE_ADD_STR(expr)
| Returns the sum of an ISO 8601 date string and a given integer value.

|DATE_ADD_MILLIS(expr)
|Returns the sum of a unix timestamp and a given integer value.
