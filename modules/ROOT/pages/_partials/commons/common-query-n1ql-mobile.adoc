...
See {query-resultsets--xref} for how to work with result sets.
...
:param-tags!:
We are accessing the current database using the shorthand notation *`_`* -- see the <<lbl-from>> clause for more on data source selection and <<lbl-query-params>> for more on parameterized queries.
...
You can also use a new type of index, the xref:{param-module}:indexing.adoc#array-indexing[Array Index], to allow querying with `UNNEST` more efficiently.
...
CAUTION: `"unnest"` will be defined as a new keyword in the SQL++ syntax.
You cannot use the term as an identifier for a property name or data source unless you escape it using backticks.
For example - `SELECT * FROM foo where ````unnest``` = true`

=== Examples

For examples of using Array Indexes in conjunction with `UNNEST`, see xref:{param-module}:indexing.adoc#array-indexing[Array Index].

We are also accessing the current database using the shorthand notation *`_`* -- see the <<lbl-from>> clause for more on data source selection and <<lbl-query-params>> for more on parameterized queries.
