...
You can find further information on database operations in {xref-cbl-pg-database}.

.Best Practice
[TIP]
--
. When you start a change listener, save the returned token, you will need it when you remove the listener
. You can ensure the replication has completely stopped by checking for a replication status = STOPPED
--
...
We can then remove any active <<lbl-repl-chng, change listener>>.
