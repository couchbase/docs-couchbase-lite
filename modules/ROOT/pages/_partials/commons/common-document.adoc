// Inclusion
:param-related: {cbl--xref} database.adoc[Databases] | {cbl--xref} blob.adoc[Blobs] | {cbl--xref} indexing.adoc[Indexing] |
...
See {cbl--xref} database.adoc[Databases] for more information
...
For more on working with blobs, see {cbl--xref} blob.adoc[Blobs]
...
* Use the {url-api-references-classes}{api-pfx}Collection.html{sep}Couchbase_Lite_Collection_GetDocument_System_String_[Collection.GetDocument()] method to get a document.
...
To make an immutable document mutable, use {url-api-references-classes}{api-pfx}documents.html#ga0356b98d2f1798adc8f549510d3eef67[CBLDocument_MutableCopy()].
...
See also: {cbl--xref} blob.adoc[Blobs]
endif::is-c[]
