...
NOTE: the ID must be unique within the database.
You cannot change it after you have written the document.

The document also has a value which contains the actual application data.
This value is stored as a dictionary collection of key-value (k-v) pairs.
...
* Use the {url-api-method-database-getdocument} method to get a document.
If the document doesn't exist in the database, the method will return `null`.
