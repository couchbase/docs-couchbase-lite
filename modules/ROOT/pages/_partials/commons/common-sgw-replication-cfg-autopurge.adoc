// BEGIN -- inclusion -- common-sgw-replication-cfg-autopurge.adoc
//  Used from:
...
In 2.x, CBL does and auto-purge only in the case when the user loses access to a document by removing the doc from all of the channels belong to the user.
...
//
// END -- inclusion -- common-sgw-replication-cfg-autopurge.adoc