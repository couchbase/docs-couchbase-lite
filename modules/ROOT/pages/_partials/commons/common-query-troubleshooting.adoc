...
:tslinks: {cbl--xref} troubleshooting-logs.adoc[Using Logs]
ifdef::is-ios[]
...
:param-related:  {tslinks} | {cbl--xref} querybuilder.adoc#lbl-predquery[Predictive Queries] | {cbl--xref} query-live.adoc[Live Queries] | {cbl--xref} indexing.adoc[Indexing]
endif::[]
ifndef::topic-group[]
...
include::ROOT:partial$commons/common-indexing.adoc[tag=overview]
...
TIP: For more on indexes -- see: {cbl--xref} indexing.adoc[Indexing]
...
Consider Using Full Text Search instead of complex like or regex patterns -- see {cbl--xref} fts.adoc[Full Text Search]. +
{empty}
