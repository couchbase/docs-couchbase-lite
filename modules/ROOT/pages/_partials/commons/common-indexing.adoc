...
A key difference between a value index and a full-text index is that a full-text index is not selected by SQLite’s query optimizer. Instead, it is explicitly selected by the match(indexName, terms) function. This means that a partial full-text index will always be used when the match() function is invoked, regardless of other predicates in the WHERE clause.
