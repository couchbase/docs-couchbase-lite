...
== Introduction
// tag::overview[]
Querying documents using a pre-existing database index is much faster because an index narrows down the set of documents to examine -- see: the {cbl--xref} query-troubleshooting.adoc[Query Troubleshooting] topic.
...
include::ROOT:partial$block-caveats.adoc[tag=index-partial-value]
...
TIP: See the {cbl--xref} querybuilder.adoc[QueryBuilder] topic to learn more about QueryBuilder.
