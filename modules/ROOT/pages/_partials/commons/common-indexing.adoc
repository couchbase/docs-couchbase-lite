...
Array `UNNEST` to unpack arrays within a document to allow joins with the parent object, and array indexes for indexing nested array properties and allow more efficient queries with `UNNEST`. 
...
|`expr`
|image:ROOT:no.png[]
|Path to the nested array within the document.

|Update when documents are deleted or purged
|image:ROOT:yes.png[]
|Assigns the alias name to the unnested array.
When assigning the alias, `AS` may be ommitted.
...
public class ArrayIndexConfiguration : IndexConfiguration {
  ...
  let expressions: [String]?
   ...
   ArrayIndexConfiguration(path: String, expressions: [String]? = nil)
}
...
[CAUTION]
--
The `InvalidArgumentException` will be thrown in the following cases:

* `expressions` is empty.

* `expressions` contains a single empty string.

* `expressions` contains null if your platform allows null values.
--
...
[source, swift]
--
let config = ArrayIndexConfiguration(path: "likes", expressions: nil)
collection.createIndex(withName: "myindex", config: config)
--
...
Below is an example of using array indexes with dictionary values:

[source, swift]
--
let config = ArrayIndexConfiguration(path: "contacts[].phones", expressions: ["type"])
collection.createIndex(withName: "myindex", config: config)
--
