...
If a scope is not explicitly referenced in calls to the database, the `_default` scope is used. 

If no collection name is given, the `_default` collection is used.
