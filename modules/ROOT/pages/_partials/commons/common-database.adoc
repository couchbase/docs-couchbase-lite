...
:param-related: {cbl--xref} blob.adoc[Blobs] | {cbl--xref} document.adoc[Documents] | {cbl--xref} indexing.adoc[Indexing]
...
This also closes
ifndef::is-c[footnote:fn28[Commencing with Release 2.8]]
...
TIP: Before closing, check that any attached listeners (query/replication/change) indicate they are at least at `connected` status before closing -- see for example: {cbl--xref} replication.adoc#lbl-repl-mon[Monitor Status].
endif::is-c[]
...
include::ROOT:partial$commons/common-database-encryption.adoc[]
...
For more on using Couchbase logs -- see: {cbl--xref} troubleshooting-logs.adoc[Using Logs].
