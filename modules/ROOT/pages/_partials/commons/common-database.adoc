...
If a scope is not explicitly referenced in calls to the database, then scope is assumed to `_default`. Likewise, if no collection name is given then then the `_default` collection is assumed.

.Storing local configuration
****
It's likely that you will not need to sync all the data related for a particular application, in which case you can set up a scope that syncs data, and a second stop that doesn't.
