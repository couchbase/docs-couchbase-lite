// = {sqlpp_cbm} for Analytics vs. {sqlpp_cbm} for Mobile
// :description: A comparison between {sqlpp_cbm} for Analytics and {sqlpp_cbm} for Query.
...
There are several minor but notable behavior differences between _{sqlpp_cbm} for Mobile_ queries and _{sqlpp_cbm} for Server_, as shown in  <<tbl-diffs>>.

In some instances, if required, you can force {sqlpp_cbm} for Mobile to work in the same way as {sqlpp_cbm} for Server.
...
h| Feature
h| {sqlpp_cbm} for Couchbase Server
...
| ORDER BY
|Result sequencing is based on specific rules described in {svr-{sqlpp_cbm}-reference-orderby--xref}
...
|===
^| {sqlpp_cbm} for Couchbase Server ^| {sqlpp_cbm} for Mobile
...
You can choose to use _Couchbase Server's {sqlpp_cbm} rules_ by using the `TOBOOLEAN(expr)` function to convert a value to its boolean value.
...
In {sqlpp_cbm} for Mobile -- unlike {sqlpp_cbm} for Server -- `NULL` is implicitly converted to `FALSE` before evaluating logical operations.
...
|===
^| {sqlpp_cbm} for Server ^| {sqlpp_cbm} for Mobile

| {sqlpp_cbm} for Server always performs float division regardless of the types of the operands.
...
| {sqlpp_cbm} for Server `ROUND()` uses the _Rounding to Nearest Even_ convention (for example, `ROUND(1.85)` returns 1.8).
