...
// DO NOT EDIT
== Introduction
This content provides code and configuration examples covering the implementation of {_glos-pfx}peer-to-peer-sync[Peer-to-Peer Sync] over WebSockets.
Specifically, it covers the implementation of a {_glos-pfx}passive-peer[Passive Peer].

Couchbase's Passive Peer (also referred to as the server, or Listener) will accept a connection from an {_glos-pfx}active-peer[Active Peer] (also referred to as the client or replicator) and replicate database changes to synchronize both databases.
...
ifdef::is-android[]
You can query the port using {url-api-references-urlendpointconfiguration-property}getPort-int-[getPort].
endif::[]
...
Note that the server must always authenticate itself with the client -- see: {cbl--xref} p2psync-websocket-using-active.adoc#authenticate-listener[Authenticate Listener on Active Peer] for how the client deals with this.
