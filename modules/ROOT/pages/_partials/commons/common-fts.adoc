...
ifndef::is-c[You can choose to use {sqlpp_cbm} or QueryBuilder syntaxes to create and use FTS indexes.]
...
FullTextSearch is enabled using the {sqlpp_cbm} match() function.
...
See the https://github.com/couchbasedeps/sqlite3-unicodesn/blob/HEAD/stopwords_en.h[full list of these **stopwords**].

The following example finds all hotels mentioning _Michigan_ in their _Overview_ text.

.Using {sqlpp_cbm} Full Text Search
[#ex-indexbuilder]
