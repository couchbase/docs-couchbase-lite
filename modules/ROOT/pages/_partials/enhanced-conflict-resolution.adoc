...
- It is recommended for the same conflict resolver to be associated with all replicators.
If that's not possible, it is recommended for the resolver to handle all exceptions, failing to do so could potentially lead to data loss.
...
. If there is an exception thrown in the `handle()` method, the exception will be caught and handled:
// TODO: is the following point correct?
.. The save operation will be cancelled and return `false`.
.. The exception will be reported in the warning logs.
