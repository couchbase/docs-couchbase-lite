...
`cblite` is a command-line tool for inspecting and querying Couchbase Lite 2.x databases. To use it, download Couchbase Lite from the https://www.couchbase.com/downloads[downloads page] and navigate to the *Tools* folder.

image:cblite-tool.png[image,,300]
...
- `URLEndpoint`: to replicate data between a local Couchbase Lite database and remote Sync Gateway database using the WebSocket protocol ([start here]).
- `DatabaseEndpoint`: to replicate data between two local Couchbase Lite databases. This target can be used to create a local back-up of a database ([database replicas])
- `MessageEndpoint`: allows a local database to replicate with another Couchbase Lite database via a custom transportation protocol such iOS Multipeer Connectivity, Android WiFi Direct, Android NearByConnection, socket based transportation etc. (see xref:#message-endpoint[Message Endpoint])
...
====
The Message Endpoint API is an link:https://www.couchbase.com/downloads[Enterprise Edition] feature.
====

The Message Endpoint API allows a local database to replicate with another Couchbase Lite database via an appropriate platform specific _communication framework_.

For peer-to-peer communication between iOS devices, we recommend to use link:https://developer.apple.com/documentation/multipeerconnectivity[Multipeer Connectivity framework] as the _communication framework_. The framework will handle sending and receiving messages and supports multiple transport technologies – WiFi and Bluetooth and uses whichever is available to establish a connection between devices. Note that the Multipeer Connectivity framework is only supported on iOS devices.

A peer can take on one of these two roles:

- *Active Peer:* The peer that initiates the connection and replication (i.e the "client" side).
- *Passive Peer:* The peer that receives a connection request and is the replication endpoint that the active peer connects to (i.e the "server" side).

*The following sections describe the classes, properties and methods of the Message Endpoint API only. Peer-to-peer replication won't work until the communication framework code is added where required, which is not being discussed here.*

=== Active Peer Side

Integrating with the Couchbase Message Endpoint API involves the following steps on the active peer.

. xref:#instantiating-a-replicator[Instantiating a replicator] object to connect with the passive peer.
. xref:#implementing-the-message-endpoint-delegate-protocol[Implementing the Message Endpoint Delegate protocol] in order to create a `MessageEndpointConnection` object.
. xref:#implementing-the-message-endpoint-connection-on-the-active-peer[Implementing the Message Endpoint Connection on the Active Peer] to open the connection, send and receive messages.
. xref:#handling-incoming-messages-received-from-the-passive-peer[Handling Incoming Messages Received from the Passive Peer]

==== Instantiating a Replicator

In this section, we assume that the communication framework code is already in place to browse for remote peers. When a new peer is found, you can initialize and start a new replication to that peer as shown in the diagram below.

image:state-1.png[]

The following code instantiates a replicator with a `MessageEndpoint` target object.

[source,swift]
----
include::{examplesdir}/{snippetsdir}[tags=message-endpoint-replicator]
----
* This code instantiates a new `MessageEndpoint` object.
** The first argument (`uid`) is a unique ID for the endpoint.
** The second argument (`target`) is the information needed to connect to the endpoint (URL, local network ID, etc) and will be used later.
** The third argument (`protocolType`) specifies the kind of transport you intend to implement.  There are two choices.  The default (`MessageStream`) means that you want to "send a series of messages," or in other words you will control the formatting of messages so that there are clear boundaries between messages.  The alternative (`ByteStream`) means that you just want to send raw bytes over the stream and Couchbase should format for you to ensure that messages get delivered in full.  To illustrate this, if you get a large message it is possible that it will get split into multiple packets when traveling over a network.  In `MessageStream` mode you, or your messaging framework, are responsible for collecting the relevant packets and reassembling them.  In `ByteStream` mode, you can simply send the bytes as is without doing any analysis and Couchbase will know how to reassemble them.
** The final argument is the delegate that will implement the `MessageEndpointConnection` protocol.
* The replicator configuration is then initialized with the `MessageEndpoint` instance as the target.

==== Implementing the Message Endpoint Delegate protocol

Next, the `MessageEndpointDelegate` protocol must be implemented to create a new connection. The following code creates a new class called `MessageEndpointManager` that implements the `MessageEndpointDelegate` protocol.

[source,swift]
----
include::{examplesdir}/{snippetsdir}[tags=message-endpoint-delegate]
----
<1> This protocol is responsible for creating the actual connection object that will be used to send data over the wire. The return type is `MessageEndpointConnection` which is an abstract class. The following section describes how to implement it.

The connection lifecycle will be covered in the next section.

==== Implementing the Message Endpoint Connection on the Active Peer

The `MessageEndpointConnection` protocol must be implemented by the application to integrate with the "communications framework". This class delegates the process of opening the connection, sending messages and closing the connection to the "communications framework".

image:state-2.png[]

The following code is a subclass of `MessageEndpointConnection` and implements the required method. Each method will be called by the replicator to open a connection, close a connection, and send a message to the remote peer.

[source,swift]
----
include::{examplesdir}/{snippetsdir}[tags=message-endpoint-connection]
----
<1> A `ReplicationConnection` global variable. This will be set once the message endpoint connection is opened. See below.
<2> Called by the replicator to open the connection with the other peer. This method must open a connection with the other peer using a _communication framework_ of your choice (Multipeer Connectivity, Android NearbyConnection etc) and call the completion handler. You can use the `MessageError` in the completion block to specify if the error is recoverable or not. If it is a recoverable error, the replicator will kick off a retry process which will result to creating a new `MessageEndpointConnection` instance.
<3> Called by the replicator to send the message to the other peer. This method sends the message to the other peer using the _communication framework_. Once the data is sent, call the completion block to acknowledge the completion. You can use the `MessageError` in the completion block to specify if the error is recoverable or not. If it is a recoverable error, the replicator will kick off a retry process which will result to creating a new `MessageEndpointConnection`.
<4> The incoming data is converted to the `Message` type. This method will be called later on when xref:#handling-incoming-messages-received-from-the-passive-peer[handling incoming messages received from the passive peer].
<5> The replication connection's `receive` method is called which then process the data in order to persist it to the local database.
<6> Called by the replicator to close the current connection with the other peer. This method must close the connection with the other peer using a transport protocol of your choice (Multipeer Connectivity, Android NearbyConnection etc). Call the completion block to acknowledge the completion.

==== Handling Incoming Messages Received from the Passive Peer

To process a message that is received from the the remote peer, you invoke the `receive` method on the `MessageEndpointConnection` instance.

[source,swift]
----
include::{examplesdir}/{snippetsdir}[tags=communications-framework-active-side]
----
<1> This method would be part of the _communication framework_ and would be invoked when the active peer receives data.
<2> Invoke the receive method on the `MessageEndpointConnection`. You had created the _connectionManager_ while <<Implementing a MessageEndpointDelegate protocol>> .

=== Passive Peer Side

The passive side (also referred to as the server side) reacts to things that it receives but does not initiate any communication on its own. The server must make use of the `MessageEndpointListener` class, which is the counterpart to the `Replicator` class on the active side. Integrating with the Message Endpoint API on the passive peer involves the following steps.

. xref:#instantiating-a-message-endpoint-listener[Instantiating a Message Endpoint Listener] with the database to be used in the replication.
. xref:#implementing-the-message-endpoint-connection-on-the-passive-peer[Implementing the Message Endpoint Connection on the Passive Peer] for opening connections, sending and receiving messages.
. xref:#associating-the-connection-with-the-listener[Associating the Message Endpoint Connection with the Message Endpoint Listener].
. xref:#handling-incoming-messages-received-from-the-active-peer[Handling Incoming Messages Received from the Active Peer]

==== Instantiating a Message Endpoint Listener

image:state-3.png[]

The following code instantiates a `MessageEndpointListener` with a given database.

[source,swift]
----
include::{examplesdir}/{snippetsdir}[tags=message-endpoint-listener]
----
<1> The `MessageEndpointListenerConfiguration` object is created with the database to be associated with the listener
<2> The `MessageEndpointListener` is instantiated with the relevant configuration created in previous step
<3> Optionally, change listener can be registered with the `MessageEndpointListener` to monitor the listener's activity

==== Implementing the Message Endpoint Connection on the Passive Peer

image:state-4.png[]

The `MessageEndpointConnection` protocol must be implemented by the application on the passive peer side as well. The implementation is more or less identical to what is done on the active peer side.
...
include::{examplesdir}/{snippetsdir}[tags=message-endpoint-connection]
----
<1> A `ReplicationConnection` variable. This will be set once the message endpoint connection is opened. See below.
<2> Called by the replicator to open the connection with the other peer. This method must open a connection with the other peer using a _communication framework_ of your choice (Multipeer Connectivity, Android NearbyConnection etc) and call the completion handler.
<3> Called by the replicator to send the message to the other peer. This method send the message to the other peer using the appropriate _communication framework_. Once the data is sent, call the completion block to acknowledge the completion.
<4> The incoming data is converted to `Message` type
<5> The replication connection's `receive` method is called which then process the data in order to persist it to the local database.
<6> Called by the replicator to close the current connection with the other peer. This method must close the connection with the other peer using a transport protocol of your choice (Multipeer Connectivity, Android NearbyConnection etc). Call the completion block is used for acknowledging the completion.

==== Associating the Connection with the Listener

The `MessageEndpointConnection` is associated with the `MessageEndpointListener` when a connection is established between the peers (using the _communication framework_).

[source,swift]
----
include::{examplesdir}/{snippetsdir}[tags=associating-listener-and-connection]
----
<1> Variable to track the `MessageEndpointConnection` instance.
<2> This method would be part of the _communication framework_ and would be invoked  when it receives data.
<3> Create an instance of _ConnectionManager_ which implements the `MessageEndpointConnection` class.
<4> Invoke the `accept` method on the `MessageEndpointListener` that was created as discussed in xref:#instantiating-a-message-listener-endpoint[Instantiating a Message Listener Endpoint].

==== Handling Incoming Messages Received from the Active Peer

To process a message that is received from the active peer, you invoke the `receive` method on the `MessageEndpointConnection` instance.

[source,swift]
----
include::{examplesdir}/{snippetsdir}[tags=communications-framework-active-side]
----
<1> This method would be part of the _communication framework_ and would be invoked when peer receives data.
<2> Invoke the receive method on the `MessageEndpointConnection`. You had created the `connectionManager` while xref:#associating-the-connection-with-the-listener[Associating the Message Endpoint Connection with the Message Endpoint Listener].
