...
----
The replicator configuration is first initialized with a `MessageEndpoint` object as the target.

==== Implementing the Message Endpoint Delegate protocol

Next, the `MessageEndpointDelegate` protocol must be implemented in order to create a `MessageEndpointConnection` instance. The following code creates a new class called `MessageEndpointManager` that implements the `MessageEndpointDelegate` protocol.

[source,swift]
----
include::{examplesdir}/{snippetsdir}[tags=message-endpoint-delegate]
----
<1> The `MessageEndpointDelegate` protocol method will be called by Couchbase Lite's internal APIs when it starts a new replication. The return type is `MessageEndpointConnection` which is an abstract class. The following section describes how to implement it.
...
Integrating with the Couchbase Message Endpoint API on the passive peer involves the following steps.
