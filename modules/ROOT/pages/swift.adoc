...
image:message-endpoint-diagram.png[]
...
Next, the `MessageEndpointDelegate` protocol must be implemented. The following code creates a new class called `MessageEndpointManager` that implements the `MessageEndpointDelegate` protocol.
...
The `MessageEndpointConnection` protocol must be implemented by the application to integrate with the "communications framework".
