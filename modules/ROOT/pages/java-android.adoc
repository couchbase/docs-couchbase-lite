...
The operating systems listed in the table <<tblSupAndVrsns>> refer to *Certified* versions of Android.
We do not test against, or guarantee support for, uncertified Android versions (for example, versions built from source).

.Deprecation Notice
WARNING: Support for Android API 19 and 21 is deprecated in this release and will be removed in a future release.

anchor:tblSupAndVrsns[Supported Android Versions]
