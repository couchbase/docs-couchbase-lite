...
// DOC-7341 - Attach Listener Before Replicator.Start() https://issues.couchbase.com/browse/DOC-7341
...
The following example uses the `ArrayFunction.contains` to find documents whose `public_likes` array property contain a value equal to "Armani Langworth".
...
When running in continuous mode (`continuous=true`) the replicator monitors the network so it can quickly recover from disruption.
This monitoring relies on _ACCESS_NETWORK_STATE_ permission, without it recovery could take up to 10 minutes longer.

.Get Permission

. Include a `<uses-permission>` element in the _manifest_
+
...
+
You should _explicitly_ request permission from the user, explaining why it's needed.
Otherwise, Android OS requests permission the first time replicator needs it, but gives no reason.

. Check permission on each use
+
Users can revoke permission at any time.
So your app must verify it has permission before each use.
