...
* Planning to run {cblJP} on Linux?
+
...
TIP: This means you can declare them as `compileOnly` dependencies
...
Database queries are based on expressions, of the form "SELECT {blank-field} FROM documents WHERE {blank-field}, ORDERed by {blank-field}", with semantics based on Couchbase's N1QL query language.
