...
+
We'll refer to the following location `<your dir>/{packageNmCE}` as the `<pathToCbl>`.

*To use a Maven package:*

You will need to point your `build.gradle` file to the appropriate repository and add the {cbl} package as a dependency (see <<bmkUsingMavenRepos>> in the next step, <<Prepare Your Build Environment>>).
...
This section shows how to set up {gpBuildTool} and {gpIDE} Community Edition to build {cbljp} applications.
...
. Copy the {cbl} jar lib into the project
+
----
cp -r <pathToCbl>lib/ libs
----

. For a web app project, create a webapp folder for the `.html` and `.jsp` components
+
----
...
+
The contents of your `build.gradle` file will depend on the type of project you are building, whether you are using Community or Enterprise edition and whether you prefer to use Maven-distributed binaries or a download.
+
TIP: Example `build.gradle` file contents are given in  <<Build a Getting Started App>>

** If you are using downloaded binaries, your file should contain:
+
----
Dependencies {
...
+
** If you are using Maven repositories, it should include:
+
...
. Import your project structure into {gpIDE}

.. Within {gpIDE}, open the project folder
+
{gpIDE} will detect the {gpBuildTool} project content and prompt you to import it

.. Accept the prompt to Import Gradle Project
+
Note the {gpBuildTool} menu on the right of the screen. You will use this to build your project and distributables (war)
+
...
This section explains how to use your configured build environment to create a starter {cblJP} app.

=== About Building {cblJP} Web Apps
...
We'll cover how to use {gpIDE} and {gpBuildTool} to build a simple {cblJP} console application.


. Within {gpIDE}, open the project we created in <<Preparing Your Build Environment>>

. Replace the `build.gradle` content with:
----
include::{snippet-java-jvm}[tags=getting-startedGradle,indent=0]
----
. Replace the app.java code with the following GettingStarted.java code:
+
[source,java]
----
...
+
. Within {gpIDE}'s {gpBuildTool} menu:
  ** Use *Gradle > Tasks > Build* to compile the code
  ** Use *Gradle > Tasks > Run* to Run the code
  ** Use *Gradle > Tasks > Jar* to build a jar file


You can run your app using by any of these methods:
** Using {gpIDE}'s Build > Run option
** Launching the app from a command line using:
+
[source,bashrc]
...
----
+
Or, alternatively, if you are using {gpBuildTool} then you can use:
+
[source,java]
...
----
+
However you launch it you should see the document ID and property printed to the console together with a query result showing the number of rows in the database.
...
This section explains how to set-up a build project to create {cblJP} web apps using of {gpBuildTool} and {gpIDE}.

anchor:bmkMultCblJapps[Running multiple {cblJP} web apps]

Before building and deploying multiple {cblJP} web apps, y*ou should note that, based on this link:https://tomcat.apache.org/tomcat-9.0-doc/class-loader-howto.html[tomcat documentation], each web application has its own class loader (WebappX).
This application-specific loader loads all of the classes, jars, and resources found in the application's `WEB-INF/classes` and `WEB-INF/lib` folders.
In addition to this, the common class loader loads the shared jar files from `$CATALINA_BASE/lib` and makes them available for all web applications.
...
anchor:bmkGradleWebAppBuild[]
*Steps*

. Within {gpIDE}, open the project we created in <<Preparing Your Build Environment>>

. Replace the content of your `build.gradle` file with:
+
[source, bashrc]
----
...
. Add a *gettingstarted* class in `src/main/java` using *File > New > Java Class*
+
. Paste the servlet code (below) into the class:
+
[source, java]

----
include::{snippet-java-jvm}[tags=GsWebApp,indent=0]
----

. Create an index html file in `src/main/webapp`
.. Use *File > New > HTML*
.. Enter `index.html` as _Name_
.. *<Enter>* to continue
.. Add the following content to the file:
+
----
include::{snippet-java-jvm}[tags=GsWebAppIndex,indent=0]
----

. Create a view file in `src/main/webapp`
.. *File > New > File*
.. Enter `showDbItems.jsp` as _Name_
.. *<Enter>* to continue
...
+
----
include::{snippet-java-jvm}[tags=GsWebAppShowDbItems,indent=0]
----
. Use the Gradle menu to build the app and the war when required:
+
image::GradleMenuWebApp.png[,300]

.. Use *Gradle > Tasks > Build*
.. Use *Gradle > Tasks > War*
+
The war file will be in the project's `build.lib` folder.

. To run the app:

.. Ensure {gpWebServerEnv} is running
.. Use the 'localhost:8080/manager' function to navigate-to, select and deploy your `.war` file
...
----
+
This pulls-in the required {gpBuildTool} plugins to embed Tomcat.
...
----
+
This shows us that the following web application tasks are now available:
...
. At this point you can run an empty Tomcat container using:
+
[source, bashrc]
+
----
...
. You can also create a distributable `.war` file using:
+
[source, bashrc]
+
----
...
----
+
Of course, at this stage running the war will launch only the embedded {gpWebServerEnv} container, as no web app has been added yet.
+
. You can now *Add Some Java*
.. Add a *gettingstarted* class in `src/main/java` using *File > New > Java Class*
+
.. Paste the servlet code (below) into the class:
+
...
tomcatRun
----
.. From the tomcatRun window select the *http://localhost:8080* link
+
This opens the browser at your index.html page.
..  Select the *here* link
+
