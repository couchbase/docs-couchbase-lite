...
:packageNmCE: couchbase-lite-java-platform-{version-full}
:packageNmEE: couchbase-lite-java-platform-ee-{version-full}
:url-api-references: http://docs.couchbase.com/mobile/{version}/couchbase-lite-java
:hdIndent:
...
Binaries are available for both Community (CE) and Enterprise (EE) editions in `.zip` format here: link:{url-cb-downloads-mobile}[{nmCBdwnlds}] page.
...
The _.zip_ includes jars for the appropriate edition of the {cblJP} and any relevant dependencies, plus a set of Linux _shared libraries_.
...
So you when you unpack, expect to see:
...
* A `support` folder containing the following libraries:
** libz
** libc++
** libicu

+
These shared libraries must be linked-to when building and-or running in a Linux environment.

+
WARNING: Use only when deploying to a Linux environment.
See <<Additional Steps for Linux Only>> for how to use these.
...
. Unzip the file.
+
Choose a location accessible to -- and usable by -- your chosen *build* environment.
We'll refer to this as the `<pathToCbl>`.

anchor:bmkLinuxSharedLibs[]

=== Additional Steps for Linux Only

. Copy the _shared libraries_ (`<pathToCbl>/support`) to a location accessible to -- and usable by -- your build and runtime environments.
. Add the _shared libraries_ to your library path using either of these methods;
** Set the `LD_LIBRARY_PATH` environment variable using the following commands.
+
[source, bashrc]

----
export LD_LIBRARY_PATH=<pathToCbl>/couchbase-lite-java-ee-2.7.0-
63/support/linux/x86_64/libicu:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=<pathToCbl>/couchbase-lite-java-ee-2.7.0- 63/support/linux/x86_64/libz:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=<pathToCbl>/couchbase-lite-java-ee-2.7.0- 63/support/linux/x86_64/libc++:$LD_LIBRARY_PATH
----

+
TIP: Save typing-time and add the above commands to your `.bashrc` file

** Alternatively, declare the libraries in the `java.library.path` parameter of the `java` command
+
[source,bash]

----
java -Djava.library.path=<pathToCbl>/couchbase-lite-java-ee-2.7.0-63/support/linux/x86_64/libicu:
<pathToCbl>/couchbase-lite-java-ee-2.7.0-63/support/linux/x86_64/libz:
<pathToCbl>/couchbase-lite-java-ee-2.7.0-63/support/linux/x86_64/libc++:
----
...
+
--
WARNING: {gpIDE} _Ultimate_ is needed to build web apps.
...
* If your {gpWebServerEnv} server runs Linux, follow the steps in <<bmkLinuxSharedLibs>> to make the shared libraries available.
* Ensure the {cbl} jars you downloaded (`<pathToCbl>/lib`) are either:
** On the executable path within {gpWebServerEnv}, or,
** Packed into the `.war` as both compile and runtime dependencies
...
. [Optionally] Select *Include in project build* to create the jar on each build
+
TIP: This means there is no need to call _Build Artifacts_ after each build

+
TIP: You can (optionally) configure the IDE to launch {gpWebServerEnv}
...
The simplest way to do so is to deploy it in an embedded tomcat harness.
You can build a simple harness as follows:

*Prerequisites*

You will need to get the embedded tomcat `tar.gz` from link::[https://tomcat.apache.org/download-90.cgi]

*Steps*

. Create a project for the harness
. Add a `libs` directory to the project containing the following library containers:
** libMyJar - for jar dependencies needed for the harness
** libMyWar -- for the war and any associated dependencies
. Unpack the embedded tomcat `tar.gz` into libMyJar
. Paste the embed harness code below (or similar) into the `tcWebAppHarness.java` file:
+
[source, java]

----
include::{snippet-java-jvm}[tags=tcWebAppHarness,indent=0]
----
+
. Configure any dependencies for the harness. That is:
** The embedded tomcat jars and javax servlet already copied into libMyJar
** Any `.war` it is required to run
. Build the harness app with {gpIDE} or {gpBuildTool}
+
At this stage running the jar will launch the embedded {gpWebServerEnv}, but no web app has been added yet.
...
NOTE: See <<Prerequisites>> above if you have not already done so.
...
If you haven't already done so, download link:{url-cb-mobStarterApp}[{nmMobStarterApp}].
...
NOTE: By default this app stores its database in the `./` directory. For {gpWebServerEnv} that is `{gpWebServerEnv}/webapps/cblDbDir`
...
+
. Within the IDE, select *Build > Build Project* to compile the code and assemble the output .jar file.
+
...
But first you need prepare the associated {cbl} components - see <<Run Your App>>
...
NOTE: On Linux: You must have loaded the shared libraries as described in <<Linux platforms>>

==== Install Couchbase

If you have not already done so:

. Install {svr} and {sg} following steps outlined in xref:{sgGetStartedDoc-installation}[Installing Sync Gateway ->]
...
NOTE: The app creates its `{nmSampleAppDb} database` directory in the same directory you run the application from; for web apps this is `$CATALINA_BASE` by default.
...
. Navigate to the directory containing the jar created in <<First app build>>
. Launch the app using either of:
** Gradle
+
[source,java]
----
...
Refer to the link:https://docs.couchbase.com/couchbase-lite/{version}/java.html#database[java code snippets] provided in the {android} section while implementing the Java app.
...
NEED JVM VERSION - prototype below --

Typically your database is stored within your apps project folder, `resources`for example. By default it is created in a directory with the root level called, for example '<applicationName>.cblite2'.

A database's directory is governed by the API _DatabaseConfiguration_ class as show in {url-api-references}/com/couchbase/lite/DatabaseConfiguration.html.

Use Getter/Setter to read/write this as needed.
