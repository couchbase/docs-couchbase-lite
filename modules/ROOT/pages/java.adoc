...
NOTE: On Android, foreground and background events are available on the Activity class.
Detecting when the whole application enters the foreground or background, as opposed to a single Activity, requires additional logic.
For example, the code referenced in https://gist.github.com/steveliles/11116937[this gist] is one way to implement an abstraction layer over the Activity API to provide Application lifecycle events.
