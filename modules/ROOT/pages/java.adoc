...
:idprefix:
:idseparator: -
:snippet: {examplesdir}/java/app/src/main/java/com/couchbase/code_snippets/Examples.java
...
:url-api-references: http://docs.couchbase.com/mobile/2.6.0/couchbase-lite-java
...
As of version 2.6, Couchbase Lite spins up a single executor and a number of threads proportional to the number of CPUs `2 x CPUs`.

Note that in some applications with other processes running in parallel, you may see `RejectedExecutionExceptions` error and the Couchbase Lite listener being aborted.
This happens when there are more than `2 x CPUs` events being processed at the same time.

The solution to this problem is for the client code to specify an executor in the overloaded versions of the above calls.
The client executor can enforce an application policy for delivery ordering and the number of threads. The examples below show different policies:
...
* The handling of replicator notifications has been enhanced to use a single Executor and 2 x CPU threads.
