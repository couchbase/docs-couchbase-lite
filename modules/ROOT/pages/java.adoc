...
----
dependencies {
    implementation 'com.couchbase.lite:couchbase-lite-android-ee:2.0.0'
}
----
...
The level of compression is set on Sync Gateway and can be tuned in the configuration file (xref:sync-gateway::config-properties.adoc#2.0/replicator_compression[`replicator_compression`]).
...
|`IDLE` |The replication caught up with all the changes available from the server.
