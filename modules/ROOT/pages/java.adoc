...
The SDK cannot be used in Java applications prior to xref:2.7@java-platform.adoc[Couchbase Lite 2.7].
...
----
dependencies {
    implementation 'com.couchbase.lite:couchbase-lite-android:{maintRelVrsn}'
}
----
...
----
dependencies {
    implementation 'com.couchbase.lite:couchbase-lite-android-ee:{maintRelVrsn}'
}
----
...
This maintenance release includes the following important fix:

- https://issues.couchbase.com/browse/CBL-619[*CBL-619*] Shared Keys created in an unloaded state potentially leading to crashes while reading document.
