...
http://docs.couchbase.com/mobile/2.1.2/couchbase-lite-java[Java SDK API References]
...
NOTE: As of Android Pie, version 9, API 28, cleartext support is disabled, by default.  Although `wss:` protocol URLs
are not affected, in order to use the `ws:` protocol, applications must target API 27 or lower, or must configure application network security as described https://www.couchbase.com/downloads[here].
