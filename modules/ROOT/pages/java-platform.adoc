= Couchbase Lite Java Framework (Java Platform)
...
== Getting Started
{cblJP} provides a Java framework for Couchbase Lite development. It can be deployed to a desktop or web server (for example, {webServerEnv}).

You can use this page to familiarize yourself with the product: its components, deployment, and use.

NOTE: We use _IntelliJ IDEA CE_ and _Gradle_ for the examples presented in this content.
You are free to use the IDE and build tools of your choice.

You will learn:

* Where to get the necessary binaries -- <<Get the required binaries>>
* What the binaries contain -- <<Package Contents>>
* How to depoy {cblJP} -- <<Package Contents>>
* How to use {cblJP} -- <<Package Contents>>


=== Where to get {cblFrmWk}
You need to get the appropriate binaries for either the {cbl} Community or Enterprise editions).

Both packages are available for download in `.zip` file format on the link:http://couchbase.com/downloads[Couchbase Downloads page].
You can download the file here: link:{url-cb-downloads-mobile}[{nmCBdwnlds}]

** Community Edition
+
couchbase-lite-java-platform-2.7.0-63

** Enterprise Edition
+
couchbase-lite-java-platform-ee-2.7.0-63.zip



=== Package Contents

The _.zip_ includes the {cblJP} jar, plus jars for any relevant dependencies, plus a set of Linux _shared libraries_.
...
So you when you unpack, you can expect to see:

* A `lib` folder containing all necessary JAR files:
** couchbase-lite-java-ee-2.7.0-63.jar
** json-20180813.jar
** okhttp-3.9.1.jar
** okio-1.13.0.jar
* A `support` folder
+
This contains the shared system libraries (libz, libc++, icu ) that must be linked-to when building on Linux.
+
WARNING: Use only when deploying to a Linux environment.

+
See <<Prepare the Linux environment>> for how to use these.


=== Deploy the package

In order to deploy the package you will need to:
[loweralpha]
. Download the _zip_ file
. Unpack the _zip_ file to a location accessible to, and usable by, your chosen *build* environment
. Add the jars directory and its contents (see: <<jars>>) as dependencies in the Build tool and-or IDE of your choice
. For Linux only:
[lowerroman]
.. Make the supplied _shared libraries_ accessible to, and usable by, your chosen Linux build and runtime environment
.. Set the `$LD_LIBRARY_PATH` environment variable to the path-to-the _shared lbraries_

With these tasks completed, you can now build and run using the .jars (you do not need the support libraries).

TIP: See <<Building your first {cblJP} app>> for an exmple of how to do this.
//.How to deploy

==== Set-up in a Non-Linux Desktop environment

For an IDE:

. In *IntelliJ*, select *File > New > Project...* and click *Next*.
+
...
. Open the *Project Structure* window and select *+ > JARs or directories...*.
+
image::java-add-jars.png[,600]
+
In the Finder window, select the follwing JAR files from the `lib` folder of the package you downloaded and unpacked.
+
* couchbase-lite-java-ee-2.7.0-DB001.jar`
* `json-20180813.jar`
* `okhttp-3.9.1.jar`
* `okio-1.13.0.jar`
+
image::java-select-jars.png[,600]


==== Additional steps for Linux only

[arabic]
. Follow the instructions for non-Linux above if you have not already done so
. Load the _shared libraries_ using either of these methods;
[lowerroman]
.. Add the `LD_LIBRARY_PATH` environment variable declaratives to your `bashrc`
+
----
export LD_LIBRARY_PATH=/path/to/couchbase-lite-java-ee-2.7.0-
63/support/linux/x86_64/libicu:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=/path/to/couchbase-lite-java-ee-2.7.0- 63/support/linux/x86_64/libz:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=/path/to/couchbase-lite-java-ee-2.7.0- 63/support/linux/x86_64/libc++:$LD_LIBRARY_PATH
----
.. Declare the libraries in `java.library.path` parameter when runnning `java`
+
[source,bash]

----
java -Djava.library.path=/path/to/couchbase-lite-java-ee-2.7.0 -63/support/linux/x86_64/libicu:/path/to/couchbase-lite-java-ee-2.7.0 -63/support/linux/x86_64/libz:path/to/couchbase-lite-java-ee-2.7.0 -63/support/linux/x86_64/libc++:
----

=== Test your deployment

You can quickly validate your build environment (if you have Gradle installed) using our  starter app (see: <<Download Links>> section).
This Couchbase Lite enabled Java app demonstrates the basic CRUD and sync functionality.
Whilst it requires {sg} and {svr} instances to run, you can validate your build environment simply by:

. Downloading the app
. Navigating to the app folder and running the gradle build in terminal:
+
----
...
--
Desktop using Gradle::
--
[loweralpha]
. Create a Gradle project folder for {cbl} (`<path-to-couchbase-gradle-project>`)
. Download the zip to (for example) your `~/Downloads` folder
. Expand the zip
. Copy the .jar files to your {cbl} project folder
+
[source, bash]
----
`cp -R lib <path-to-couchbase-gradle-project>/cbl-libs`
----
. Add `libs` to the `build.gradle` file if it is not already included
[source, bash]
----
  implementation fileTree(include:["*.jar"]), dir:"cbl-libs")
----

The jar libraries will then be available for use in the Gradle project.


== Building your first {cbl} project
...
- Install the Sync Gateway following the steps outlined in the xref:sync-gateway::getting-started.adoc#installation[Installing Sync Gateway ->]
...
=== Build the app
If you haven't already done so, download the link:https:/blah/view?usp=sharing[Sample Starter App].

Open *Main.java* in IntelliJ and copy the following code in the `main` method.
...
[source,java]
----
include::{snippet-java-platform}[tag=getting-started,indent=0]
----
...
NOTE: The `mydb.cblite2` database directory is created in the directory you run application from
...
Databases that were created with Couchbase Lite 1.2 or later can be used with {cbl} {version-full}.
Upon detecting it as a 1.x database file, Couchbase Lite will automatically upgrade it to 2.0.

NOTE: This feature is only available for the default storage type (i.e., not a ForestDB database).
...
[source]
----
include::{snippet-java-platform}[tag=sdk-initializer,indent=0]
----
...
Read more in the <<initializer>> section.
