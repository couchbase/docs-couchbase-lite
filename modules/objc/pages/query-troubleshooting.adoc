...
==== Output
The output from `https://docs.couchbase.com/mobile/{version-maintenance-ios}/couchbase-lite-objc/Classes/CBLQuery.html#/c:objc(cs)CBLQuery(im)explain:[explain()]` remains the same whether invoked by an app, or `cblite` -- see <<objc:query-troubleshooting:::qe-output>> for an example of how it looks.
