...
+
Use the API's https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLDatabase.html#/c:objc(cs)CBLDatabase(cm)copyFromPath:toDatabase:withConfig:error:[copyFromPath()] method -- see: <<lbl-code>>; this ensures that a UUID is generated for each copy
+
