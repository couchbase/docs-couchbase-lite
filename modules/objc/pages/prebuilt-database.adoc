...
****
. Create a new Couchbase Lite database with the required dataset -- see xref:objc:prebuilt-database.adoc#crt-db[]
. Incorporate the pre-built database with your app bundle as an asset/resource -- see xref:objc:prebuilt-database.adoc#bundle-db[]
...
Start a sync to update the data -- see xref:objc:prebuilt-database.adoc#deploy-db[]
****
...
* You can write a simple CBL app to start a pull sync -- see: xref:objc:replication.adoc[Remote Sync Gateway]
...
+
Use the API's https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLDatabase.html#/c:objc(cs)CBLDatabase(cm)copyFromPath:toDatabase:withConfig:error:[copyFromPath()] method -- see: xref:objc:prebuilt-database.adoc#lbl-code[]; this ensures that a UUID is generated for each copy
+
