...
<.> Set the list of local collections using the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLURLEndpointListenerConfiguration.html[URLEndpointListenerConfiguration].
...
Use the Listener's `https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLURLEndpointListener.html#/c:objc(cs)CBLURLEndpointListener(py)status[status]` property/method to get counts of total and active connections -- see: <<get-connection-counts>>.
