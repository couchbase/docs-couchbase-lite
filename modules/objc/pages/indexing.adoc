...
xref:objc:indexing.adoc#ex-create-index[] creates a new index for the `type` and `name` properties, shown in this data model:
...
For details, see xref:objc:fts.adoc[].
...
xref:objc:query-n1ql-mobile.adoc#lbl-unnest[Array `UNNEST`] to unpack arrays within a document to allow joins with the parent object, and array indexes for indexing unnested array's values to allow more efficient queries with `UNNEST`.
