...
* *A replication saves a document change* -- in which case the change with the _most-revisions wins_ (unless one change is a delete). See the example xref:objc:conflict.adoc#lbl-conflicts-when-replicating[Case 1: Conflicts when a replication is in progress]
* *An application saves a document change directly to a database instance* -- in which case, _last write wins_, unless one change is a delete -- see xref:objc:conflict.adoc#conflicts-when-saving[Case 2: Conflicts when saving a document]
...
Couchbase Lite uses the following rules to handle conflicts such as those described in xref:objc:conflict.adoc#bmkRepConScene[]:
...
For additional control over the handling of conflicts, including when a replication is in progress, see xref:objc:conflict.adoc#custom-conflict-resolution[].
...
If a custom conflict resolver is not provided, the system will automatically resolve conflicts as discussed in xref:objc:conflict.adoc#automatic-conflict-resolution[Automatic Conflict Resolution], and as a consequence there will be no conflicting revisions in the database.
...
. xref:objc:conflict.adoc#conflict-resolver[Conflict Resolver]
. xref:objc:conflict.adoc#configure-the-replicator[Configure the Replicator]
...
A xref:objc:conflict.adoc#replication-events[document replication event] will be posted with an error and an error message will be logged.
+
...
** The exception will be reported in the xref:objc:conflict.adoc#replication-events[document replication event].
+
...
= https://docs.couchbase.com/mobile/3.2.1/couchbase-lite-objc/index.html[https://docs.couchbase.com/mobile/3.2.1/couchbase-lite-objc/index.html]
