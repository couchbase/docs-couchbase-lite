...
[#objc:conflict:::causes-of-conflicts]
== Causes of Conflicts
...
* *A replication saves a document change* -- in which case the change with the _most-revisions wins_ (unless one change is a delete). See the example <<objc:conflict:::lbl-conflicts-when-replicating,Case 1: Conflicts when a replication is in progress>>
* *An application saves a document change directly to a database instance* -- in which case, _last write wins_, unless one change is a delete -- see <<objc:conflict:::conflicts-when-saving,Case 2: Conflicts when saving a document>>
...
[#objc:conflict:::lbl-conflicts-when-replicating]
...
[#objc:conflict:::automatic-conflict-resolution]
...
Couchbase Lite uses the following rules to handle conflicts such as those described in <<objc:conflict:::bmkRepConScene>>:
...
For additional control over the handling of conflicts, including when a replication is in progress, see <<objc:conflict:::custom-conflict-resolution>>.

[#objc:conflict:::custom-conflict-resolution]
...
If a custom conflict resolver is not provided, the system will automatically resolve conflicts as discussed in <<objc:conflict:::automatic-conflict-resolution,Automatic Conflict Resolution>>, and as a consequence there will be no conflicting revisions in the database.
...
. <<objc:conflict:::conflict-resolver,Conflict Resolver>>
. <<objc:conflict:::configure-the-replicator,Configure the Replicator>>

[#objc:conflict:::conflict-resolver]
=== Conflict Resolver
...
[#objc:conflict:::important-guidelines-and-best-practices]
...
A <<objc:conflict:::replication-events,document replication event>> will be posted with an error and an error message will be logged.
+
...
** The exception will be reported in the <<objc:conflict:::replication-events,document replication event>>.
+
...
[#objc:conflict:::configure-the-replicator]
...
[#objc:conflict:::wx-conflict-resolver]
====


// Show Main Snippet
// include::objc:example$code_snippets/SampleCodeTest.m[tags="replication-conflict-resolver", indent=0]
[source, objc]
----
...
[#objc:conflict:::conflicts-when-saving]
...
. Your code updates the document with its modified properties, for example using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLDatabase.html#/c:objc(cs)CBLDatabase(im)saveDocument:error:[CBLDatabase saveDocument:error:].
====

[#objc:conflict:::automatic-conflict-resolution-2]
=== Automatic Conflict Resolution
...
====
Save operations::
https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLDatabase.html#/c:objc(cs)CBLDatabase(im)saveDocument:concurrencyControl:error:[CBLDatabase saveDocument:concurrencyControl:error:] -- attempts to save the document with a concurrency control.
+
...
* https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLDatabase.html#/c:objc(cs)CBLDatabase(im)deleteDocument:error:[CBLDatabase deleteDocument:error:]: The last write will win if there is a conflict.
* https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLDatabase.html#/c:objc(cs)CBLDatabase(im)deleteDocument:concurrencyControl:error:[CBLDatabase -deleteDocument:concurrencyControl:error:]: attempts to delete the document with a concurrency control.
...
[#objc:conflict:::custom-conflict-handlers]
...
= https://docs.couchbase.com/mobile/3.2.1/couchbase-lite-objc/index.html[API&#160;References]
