...
To use Couchbase Lite 3.1+ with these earlier versions, you can use the `default` Collection as a backup option.
...
<.> Optionally, register an observer, which notifies you of changes to the replication status -- see: xref:objc:replication.adoc#lbl-repl-mon[]
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags=p2p-act-rep-config-cacert, indent=0]
----
<.> This is the default.
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags=p2p-act-rep-config-self-cert, indent=0]
----
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tag=p2p-act-rep-config-cacert-pinned, indent=0]
----
// include::objc:example$code_snippets/SampleCodeTest.m[tag=p2p-act-rep-config-cacert-pinned-callouts, indent=0]
--
...
The default `false` is shown here for completeness only; it's unlikely you would explicitly use it in practice.
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags=p2p-act-rep-add-change-listener;!p2p-act-rep-add-change-listener-label,indent=0]
----
--
+
...
On other platforms, Couchbase Lite does not react to OS backgrounding or foregrounding events and replication(s) continues running as long as the remote system does not end, the connection and the app does not stop.
It's' generally recommended to stop replications before going into the background otherwise socket connections may be closed by the OS and this may interfere with the replication process.
