...
Related Content -- xref:objc:conflict.adoc[Handling Data Conflicts] | xref:objc:dbreplica.adoc[Intra-Device] | <<p2psync-websocket.adoc,Peer-to-Peer>>
--
...
To use this protocol the replication URL should specify WebSockets as the URL scheme (see the xref:objc:replication.adoc#lbl-cfg-tgt[] section below).
...
xref:objc:replication.adoc#ex-simple-repl[] shows the configuration and initialization process.
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags="sgw-rep-func;!sgw-rep-config-cacert;!sgw-rep-config-cacert-pinned", indent=0]
----
...
By default, clients accept only servers presenting certificates that can be verified using the OS bundled Root CA Certificates -- see: xref:objc:replication.adoc#lbl-svr-auth[].
...
Other options are available -- see: xref:objc:replication.adoc#lbl-client-auth[].
...
<.> Initialize the replicator using your configuration -- see: xref:objc:replication.adoc#lbl-init-repl[].

<.> Optionally, register an observer, which will notify you of changes to the replication status -- see: xref:objc:replication.adoc#lbl-repl-mon[]

<.> Start the replicator  -- see: xref:objc:replication.adoc#lbl-repl-start[].
...
+
--
xref:objc:replication.adoc#lbl-cfg-tgt[]
|  xref:objc:replication.adoc#lbl-network-interface[]
|  xref:objc:replication.adoc#lbl-cfg-sync[]
|  xref:objc:replication.adoc#lbl-cfg-keep-alive[]
|  xref:objc:replication.adoc#lbl-user-auth[]
|  xref:objc:replication.adoc#lbl-svr-auth[]
|  xref:objc:replication.adoc#lbl-client-auth[]
|  xref:objc:replication.adoc#lbl-repl-evnts[]
|  xref:objc:replication.adoc#lbl-repl-hdrs[]
|  xref:objc:replication.adoc#lbl-repl-ckpt[]
|  xref:objc:replication.adoc#lbl-repl-fltrs[]
|  xref:objc:replication.adoc#lbl-repl-chan[]
|  xref:objc:replication.adoc#anchor-auto-purge-on-revoke[]
|  xref:objc:replication.adoc#lbl-repl-delta[]
--
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags="sgw-rep-target", indent=0]
----
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags="sgw-act-rep-configure-target", indent=0]
----
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags="sgw-act-rep-config-type;sgw-act-rep-config-cont", indent=0]
----
...
The REST API provides configurable control over this replication retry logic using a set of configiurable properties -- see: xref:objc:replication.adoc#tbl-repl-retry[].
...
When necessary you can adjust any or all of those configurable values -- see: xref:objc:replication.adoc#ex-repl-retry[] for how to do this.
...
You can enable authorization in the sync{nbsp}gateway configuration file, as shown in xref:objc:replication.adoc#example-enable-authorization[].
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags=sgw-rep-config-cacert, indent=0]
----
<.> This is the default.
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags=sgw-rep-config-self-cert, indent=0]
----
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tag=sgw-rep-config-cacert-pinned, indent=0]
----
--
...
This all assumes that you have configured the Sync Gateway to provide the appropriate SSL certificates, and have included the appropriate certificate in your app bundle -- for more on this see: xref:objc:replication.adoc#lbl-cert-pinning[].


[#lbl-client-auth]
=== Client Authentication

There are two ways to authenticate from a Couchbase Lite client: xref:objc:replication.adoc#basic-authentication[] or xref:objc:replication.adoc#session-authentication[].
...
xref:objc:replication.adoc#ex-base-auth[] shows how to initiate a one-shot replication as the user *username* with the password *password*.
...
See xref:objc:replication.adoc#ex-session-auth[], which shows how to initiate a one-shot replication with the session ID returned from the `+POST /{tkn-db}/_session+` endpoint.
...
This feature is useful in passing additional credentials, perhaps when an authentication or authorization step is being done by a proxy server (between Couchbase Lite and Sync Gateway) -- see xref:objc:replication.adoc#ex-cust-hdr[].
...
For more information, see xref:objc:replication.adoc#auto-purge-on-revoke[Auto Purge on Revoke].
****
...
If a user subsequently regains access to a lost channel, then any previously auto-purged documents still assigned to any of their channels are automatically pulled down by the active Sync Gateway when they are next updated -- see behavior summary in xref:objc:replication.adoc#tbl-regain-behavior[]
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags="sgw-config-autopurge", indent=0]
----
...
This can be done either by setting https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLReplicatorConfiguration.html#/c:objc(cs)CBLReplicatorConfiguration(py)enableAutoPurge[enableAutoPurge] to false, or for finer control by applying pull-filters -- see: xref:objc:replication.adoc#tbl-pull-filters[] and xref:objc:replication.adoc#lbl-repl-fltrs[]
...
In this section::
xref:objc:replication.adoc#lbl-repl-start[]  | xref:objc:replication.adoc#lbl-repl-ckpt[]
...
You can, optionally, add a change listener (see xref:objc:replication.adoc#lbl-repl-mon[]) before starting the replicator running using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLReplicator.html#/c:objc(cs)CBLReplicator(im)start[start()].
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags="sgw-start-repl", indent=0]
----
...
However, if you do want to force the replication to start again from zero, use the xref:refer-glossary.adoc#checkpoint[checkpoint] reset argument when starting the replicator -- as shown in xref:objc:replication.adoc#ex-repl-ckpt[].
...
In this section::
xref:objc:replication.adoc#lbl-repl-chng[]  |
xref:objc:replication.adoc#lbl-repl-status[]  |
xref:objc:replication.adoc#lbl-repl-evnts[] |
xref:objc:replication.adoc#lbl-repl-pend[] |
xref:objc:replication.adoc#lbl-repl-correlation-id[]

You can monitor a replication's' status by using a combination of xref:objc:replication.adoc#lbl-repl-chng[] and the `replication.status.activity` property -- see; https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLReplicatorStatus.html#/c:objc(cs)CBLReplicatorStatus(py)activity[activity enum].
This enables you to know, for example, when the replication is actively transferring data and when it has stopped.

You can also choose to monitor document changes -- see: xref:objc:replication.adoc#lbl-repl-evnts[].
...
Use the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLReplicator.html[Replicator] class to add a change listener as a callback to the Replicator (https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLReplicator.html#/c:objc(cs)CBLReplicator(im)addChangeListener:[addChangeListener(_:)]) -- see: xref:objc:replication.adoc#ex-repl-mon[].
...
That is, whether it is actively transferring data or if it has stopped -- see: xref:objc:replication.adoc#ex-repl-mon[].

The returned _ReplicationStatus_ structure comprises:

* https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Classes/CBLReplicatorStatus.html#/c:objc(cs)CBLReplicatorStatus(py)activity[activity enum] -- stopped, offline, connecting, idle or busy -- see states described in: xref:objc:replication.adoc#tbl-states[]
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags=p2p-act-rep-add-change-listener,indent=0]
----
--
+
...
[source, objc]
----
include::objc:example$code_snippets/SampleCodeTest.m[tags=p2p-act-rep-status,indent=0]
----
--
======
...
[#lbl-repl-states]
==== Replication States
xref:objc:replication.adoc#tbl-states[] shows the different states, or activity levels, reported in the API; and the meaning of each.
...
image::objc:replicator-states.png[]
...
For example, the code snippet in xref:objc:replication.adoc#ex-reg-doc-listener[] registers a listener to monitor document replication performed by the replicator referenced by the variable `replicator`.
It prints the document ID of each document received and sent.
Stop the listener as shown in xref:objc:replication.adoc#ex-stop-doc-listener[].
...
[#blip-sync-protocol-version-differences]
===== Blip Sync Protocol Version Differences
...
This prevents unnecessary document removal when the document remains accessible to the user through alternative channels.
...
It will stop any active xref:objc:replication.adoc#lbl-repl-chng[change listener] once the replication is stopped.
...
so it might be necessary to explicitly enable them in the its configuration (see xref:sync-gateway::load-balancer.adoc[Load Balancers]).
...
You can increase the log output for activity related to replication with Sync Gateway -- see xref:objc:replication.adoc#ex-logs[].
...
For more on troubleshooting with logs, see: xref:objc:new-logging-api.adoc[Using Logs].
