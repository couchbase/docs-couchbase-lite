...
. Within the *ViewController.m* file, replace the boilerplate codein with that shown in xref:objc:gs-build.adoc#ex-starter-code[].
Note you can also get this from github.
. Build and Run the code. +
You should see the document ID and property printed to the console, indicating that the document was successfully persisted to the database as shown in xref:objc:gs-build.adoc#img-starter-code[]
