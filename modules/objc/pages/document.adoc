...
NOTE: Couchbase documents are assigned to a xref:objc:database.adoc#database-concepts[Collection].
...
The values can be made of up several different xref:objc:document.adoc#data-types[] such as numbers, strings, arrays, and nested objects.
...
Couchbase Lite also provides for the direct handling of JSON data implemented in most cases by the provision of a pass:a,q[`toJSON()`] method on appropriate API classes (for example, on MutableDocument, Dictionary, Blob and Array) -- see xref:objc:document.adoc#lbl-json-data[].
...
Couchbase documents are assigned to a xref:objc:database.adoc#database-concepts[Collection].
...
For more on using *Documents*, see xref:objc:document.adoc#document-initializers[] and xref:objc:document.adoc#mutability[].
...
Learn more about xref:objc:document.adoc#using-dictionaries[].
...
Learn more about xref:objc:document.adoc#using-arrays[]
...
This example sets the date on the `createdAt` property and reads it back using the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-ios}{empty}/couchbase-lite-objc/Protocols/CBLDictionary.html#/c:objc(pl)CBLDictionary(im)dateForKey:[dateForKey:] accessor method.
...
NOTE: Any user change to the value of reserved keys (`_id`, `_rev` or `_deleted`) will be detected when a document is saved and will result in an exception (Error Code 5 -- `CorruptRevisionData`) -- see also xref:objc:document.adoc#lbl-doc-constraints[].
...
NOTE: "_id", :"_rev" and "_sequence" are reserved keywords and must not be used as top-level attributes -- see xref:objc:document.adoc#res-keys[].
...
In this section::
xref:objc:document.adoc#lbl-array[]
| xref:objc:document.adoc#lbl-blob[]
| xref:objc:document.adoc#lbl-dictionary[]
| xref:objc:document.adoc#lbl-document[]
| xref:objc:document.adoc#lbl-result[]
...
Convert an `ArrayObject` to and from JSON using the pass:a,q[`toJSON()`] and `toArray` methods -- see xref:objc:document.adoc#ex-array[].

Additionally you can:

* Initialize a 'MutableArrayObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor -- see: xref:objc:document.adoc#ex-array[]
...
[#lbl-blob]
=== Blobs
Convert a `Blob` to JSON using the `toJSON` method -- see xref:objc:document.adoc#ex-blob[].

You can use `isBlob()` to check whether a given dictionary object is a blob or not -- see xref:objc:document.adoc#ex-blob[].
...
Convert a `DictionaryObject` to and from JSON using the `toJSON` and `toDictionary` methods -- see xref:objc:document.adoc#ex-dictionary[].

Additionally you can:

* Initialize a 'MutableDictionaryObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor-- see: xref:objc:document.adoc#ex-dictionary[]
...
Convert a `Document` to and from JSON strings using the pass:a,q[`toJSON()`] and pass:a,q[`setJSON()`] methods -- see xref:objc:document.adoc#ex-document[].

Additionally you can:

* Initialize a 'MutableDocument' using data supplied as a JSON string.
This is done using the `init(json)` or `init(id: json:)` constructor -- see: xref:objc:document.adoc#ex-document[]
