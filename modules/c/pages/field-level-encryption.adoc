...
The API includes a `CBLEncryptable` type, representing an encryptable value to be automatically encrypted/decrypted the replicator -- see xref:c:field-level-encryption.adoc#ex-encryptable-type[].

You declare  properties requiring encryption by the replicator, as a dictionary with the structure shown in xref:c:field-level-encryption.adoc#ex-encryptable-type[].
...
Pull replication detects `encryptable` values before saving them to the local database and decrypts them using the property decryption callback function -- see: xref:c:field-level-encryption.adoc#lbl-decryption-callback[] for more on the decryption callback function.

On successful decryption the replicator saves the property in encryptable dictionary format (removing the `encrypted$` prefix) -- see: xref:c:field-level-encryption.adoc#ex-encryptable-type[].
...
Push replication detects `encryptable` values before pushing them to the remote database and encrypts them using the property encryption callback function -- see: xref:c:field-level-encryption.adoc#lbl-encryption-callback[].

On successful encryption the replicator transforms the property into a format compatible with Couchbase Server SDK -- see: xref:c:field-level-encryption.adoc#ex-server-encryptable[].
...
Both Push and Pull replication transform Couchbase Lite's encryptable dictionary to-and-from Server SDK Encrypted Field dictionary structure -- see: xref:c:field-level-encryption.adoc#ex-server-encryptable[].
...
** Storing the decrypted key/value in xref:c:field-level-encryption.adoc#ex-encryptable-type[CBLEncryptable] format
...
Provide an encryption callback function {url-api-references-classes}replication.html#gab116a23be8bd24b86349379f370ef60c[CBLPropertyEncryptor] to encrypt encryptable properties during replication.

After encryption the FLSliceResult is released and the returned value zeroed.
See xref:c:field-level-encryption.adoc#ex-get-att[] for an example encryptor callback function.
...
Provide a decryption callback function {url-api-references-classes}replication.html#ga24a60a3d6f9816e1d32464cc31a15c0c[CBLPropertyDecryptor] to decrypt any encryptable properties.
After decryption the FLSliceResult is released and the returned value zeroed.
See xref:c:field-level-encryption.adoc#ex-get-att[] for an example decryptor callback function.
...
[source, c]
----
include::c:example$code_snippets/main.cpp[tag=replicator_property_encryptor_decryptor_sample]
----
--
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=replicator_property_encryptor_decryptor_sample]
----
--
=====
====
...
----
include::c:example$code_snippets/main.cpp[tag=replicator_property_encryption]
----
--
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=replicator_property_encryption]
----
--
=====
====
...
Encrypted values can be queried -- see xref:c:field-level-encryption.adoc#ex-query[].
...
In the meantime, adding an encrypted field including a nonce or random value to the document can mitigate against such brute-force computation -- as shown in xref:c:field-level-encryption.adoc#example-brute-force-mitigation[].
...
// NOTE: These are hardcoded C examples. C++ tab to be added after
