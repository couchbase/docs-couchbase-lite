...
* *A replication saves a document change* -- in which case the change with the _most-revisions wins_ (unless one change is a delete). See the example xref:c:conflict.adoc#lbl-conflicts-when-replicating[Case 1: Conflicts when a replication is in progress]
* *An application saves a document change directly to a database instance* -- in which case, _last write wins_, unless one change is a delete -- see xref:c:conflict.adoc#conflicts-when-saving[Case 2: Conflicts when saving a document]
...
Couchbase Lite uses the following rules to handle conflicts such as those described in xref:c:conflict.adoc#bmkRepConScene[]:
...
For additional control over the handling of conflicts, including when a replication is in progress, see xref:c:conflict.adoc#custom-conflict-resolution[].
...
If a custom conflict resolver is not provided, the system will automatically resolve conflicts as discussed in xref:c:conflict.adoc#automatic-conflict-resolution[Automatic Conflict Resolution], and as a consequence there will be no conflicting revisions in the database.
...
. xref:c:conflict.adoc#conflict-resolver[Conflict Resolver]
. xref:c:conflict.adoc#configure-the-replicator[Configure the Replicator]
...
A xref:c:conflict.adoc#replication-events[document replication event] will be posted with an error and an error message will be logged.
+
...
** The exception will be reported in the xref:c:conflict.adoc#replication-events[document replication event].
+
...
. Your code updates the document with its modified properties, for example using {url-api-references-classes}documents.html#[CBLDatabase_SaveDocument()].
====
...
====
Save operations::
{url-api-references-classes}documents.html#gae37e21b95f62419762e521f70850b9c7[CBLDatabase_SaveDocumentWithConcurrencyControl()] -- attempts to save the document with a concurrency control.
+
...
* {url-api-references-classes}database.html#gad9d878c8636a75898d3993537af7e88c[CBLDatabase_DeleteDocument()]: The last write will win if there is a conflict.
* {url-api-references-classes}database.html#ga01b4cf9725de18c41e8bb002255adb9a[CBLDatabase_DeleteDocumentWithConcurrencyControl()]: attempts to delete the document with a concurrency control.
...
To implement custom conflict resolution when saving a document, apps must call the `save` method with a conflict handler block ( {url-api-references-classes}documents.html#ga9c45bcf02e6e2977c702c493a7fe0b54[CBLDatabase_SaveDocumentWithConflictHandler()]).
