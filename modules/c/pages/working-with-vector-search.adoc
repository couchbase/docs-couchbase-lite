...
include::c:example$code_snippets/VectorSearch.h[tags=vs-create-default-config]

----

First, we initialize the `config` object with a new `VectorIndexConfiguration` object initialized with the following parameters:
...
|Required
|No default
|Specifies the query language used. This can be N1Q1 or JSON query.
For JSON query syntax, see link:https://github.com/couchbase/couchbase-lite-core/wiki/JSON-Query-Schema[Couchbase JSON query wiki].
...
. Next, get the colors collection from the database.

. Finally, create a vector index, colors_index, on the collection with the configuration.
...
include::c:example$code_snippets/VectorSearch.h[tags=vs-use-vector-match]
...
IMPORTANT: Similar to the xref:c:fts.adoc[Full Text Search] `match()` function, `vector_match()` can only be called alone or at the top level `AND` expression.
...
include::c:example$code_snippets/VectorSearch.h[tags=vs-use-vector-distance]
