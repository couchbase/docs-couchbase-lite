...
_Couchbase{nbsp}Lite_ for C makes extensive use of the Fleece C API for accessing document data.
...
Use the _FLValue_GetType_ API to check the value's actual type and _FLValue_As<Type Name>_ to get the actual value as shown in xref:c:c_fleece.adoc#ex-flvalue[]

.Use FLValue
[#ex-flvalue]
[source#ex-flvalue,c, subs="attributes+, macros+"}]
----
...
Use the _FLSTR(“Some String”)_ macro to create an FLString from a string literal -- see: xref:c:c_fleece.adoc#ex-flstr[]


.Create an FLString
[#ex-flstr]
[source#ex-flstr,c, subs="attributes+, macros+"}]
----
...
For an example of `FLSliceResult` in use, see: xref:c:c_fleece.adoc#ex-flsliceresult[].

[#ex-flsliceresult]
.Using FLStringResult
[source#ex-flsliceresult,c, subs="attributes+, macros+"}]
----
...
You test a slice for null by comparing its pointer (buf) with NULL -- see: xref:c:c_fleece.adoc#ex-flnullslice[]

[#ex-flnullslice]
.Test for null slice
[source#ex-flnullslice,c, subs="attributes+, macros+"}]
----
FLValue value = FLDict_Get(props, FLSTR("name”));
FLString name = FLValue_AsString(value);
if (name.buf != NULL) {
...
To access a value with a string key from a dictionary, use FLDict_Get -- as shown in: xref:c:c_fleece.adoc#ex-flget[].

[#ex-flget]
.Get dictionary value
[source#ex-flget,c, subs="attributes+, macros+"}]
----
...
To iterate through each key-value pair in the dictionary, use _FLDictIterator_, as shown in: xref:c:c_fleece.adoc#ex-fliterator[]

[#ex-fliterator]
.Iterate key-value pairs in dictionary
[source#ex-fliterator,c, subs="attributes+, macros+"}]
----
...
To create a new mutable dictionary, use FLMutableDict_New() -- see: xref:c:c_fleece.adoc#ex-flmutabledict-new[].

[#ex-flmutabledict-new]
.Set dictionary value
[source#ex-flmutabledict-new,c, subs="attributes+, macros+"}]
----
...
use _FLArray_Count_ and _FLArray_Get_ respectively, to get the numbers of values in an array and to get a value using with an index -- as shown in xref:c:c_fleece.adoc#ex-flarray-get[].

[#ex-flarray-get]
.Use arrays
[source#ex-flarray-get,c, subs="attributes+, macros+"}]
----
...
Use _FLArrayIterator_ to iterate through arrays, as shown in : xref:c:c_fleece.adoc#ex-array-iteration[].

[#ex-array-iteration]
.Array iteration
// Iterate through the array
[source#ex-array-iteration,c, subs="attributes+, macros+"}]
----
...
[source#ex-flmutable-array-append,c, subs="attributes+, macros+"}]
----
...
[source#ex-json-parse,c, subs="attributes+, macros+"}]
----
FLError error;
...
[#ex-json-from-fleese]
.Convert to JSON
[source#ex-json-from-fleese,c, subs="attributes+, macros+"}]
----
FLDict props = CBLDocument_Properties(doc);
