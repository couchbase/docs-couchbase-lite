...
:url-api-references-urlendpointconfiguration: {url-api-references}/group__URLEndpointListenerConfiguration.html
:url-api-references-urlendpointlistener: {url-api-references}/group__URLEndpointListener.html
...
:url-api-class-connection-status: {url-api-references}/group__Type%20Definitions/ConnectionStatus.html
...
<.> Set the local database using the {url-api-references-urlendpointconfiguration}[URLEndpointListenerConfiguration]'s constructor {url-api-references-urlendpointconfiguration}#URLEndpointListenerConfiguration(im)initWithDatabase:[-initWithDatabase:]. +
...
<.> To use a canonical port -- one known to other applications -- specify it explicitly using the {url-api-references-urlendpointconfiguration}#URLEndpointListenerConfiguration(py)port[port] method shown here. +
...
<.> To specify an interface -- one known to other applications -- identify it explicitly, using the {url-api-references-urlendpointconfiguration}#URLEndpointListenerConfiguration(py)networkInterface[networkInterface] method shown here.
...
{url-api-references-urlendpointconfiguration}#URLEndpointListenerConfiguration(py)tlsIdentity[tlsIdentity] method to configure the TLS Identity used in TLS communication.
...
Use the
{url-api-references-urlendpointconfiguration}[URLEndpointListenerConfiguration] class's {url-api-references-urlendpointconfiguration}#URLEndpointListenerConfiguration(py)authenticator[authenticator] method to specify how the client-supplied credentials are to be authenticated.
...
* On first use -- Bring your own certificate and private key; for example, using the {url-api-references-tlsidentity}[TLSIdentity] class's {url-api-references-tlsidentity}#TLSIdentity(cm)createIdentityForServer:attributes:expiration:label:error:[createIdentity()] method to add it to the secure storage.
* Each time -- Use the server identity from the certificate stored in the secure storage; for example, using the {url-api-references-tlsidentity}[TLSIdentity] class's {url-api-references-tlsidentity}#TLSIdentity(cm)identityWithLabel:error:[identityWithLabel:error] method with the alias you want to retrieve.
...
Use the Listener's `{url-api-references-urlendpointlistener}#CBLURLEndpointListener(py)status[status]` property/method to get counts of total and active connections -- see: <<get-connection-counts>>.
