...
<.> Set the local database using the {url-api-references-classes}URLEndpointListenerConfiguration.html[URLEndpointListenerConfiguration]'s constructor {url-api-references-classes}URLEndpointListenerConfiguration.html#URLEndpointListenerConfiguration(im)initWithDatabase:[-initWithDatabase:]. +
...
<.> To use a canonical port -- one known to other applications -- specify it explicitly using the {url-api-references-classes}URLEndpointListenerConfiguration.html#URLEndpointListenerConfiguration(py)port[port] method shown here. +
...
<.> To specify an interface -- one known to other applications -- identify it explicitly, using the {url-api-references-classes}URLEndpointListenerConfiguration.html#URLEndpointListenerConfiguration(py)networkInterface[networkInterface] method shown here.
...
Use {url-api-references-classes}URLEndpointListenerConfiguration.html[URLEndpointListenerConfiguration]'s {url-api-references-classes}URLEndpointListenerConfiguration.html#URLEndpointListenerConfiguration(py)enableDeltaSync[enableDeltaSync] method to activate or deactivate it.
...
You can use {url-api-references-classes}URLEndpointListenerConfiguration.html[URLEndpointListenerConfiguration]'s {url-api-references-classes}URLEndpointListenerConfiguration.html#URLEndpointListenerConfiguration(py)disableTLS[disableTLS] method to disable TLS communication if necessary
...
{url-api-references-classes}URLEndpointListenerConfiguration.html#URLEndpointListenerConfiguration(py)disableTLS[disableTLS] works in conjunction with `TLSIdentity`, to enable developers to define the key and certificate to be used.
...
Use {url-api-references-classes}URLEndpointListenerConfiguration.html[URLEndpointListenerConfiguration]'s
{url-api-references-classes}URLEndpointListenerConfiguration.html#URLEndpointListenerConfiguration(py)tlsIdentity[tlsIdentity] method to configure the TLS Identity used in TLS communication.
...
Use the
{url-api-references-classes}URLEndpointListenerConfiguration.html[URLEndpointListenerConfiguration] class's {url-api-references-classes}URLEndpointListenerConfiguration.html#URLEndpointListenerConfiguration(py)authenticator[authenticator] method to specify how the client-supplied credentials are to be authenticated.

Valid options are:

* No authentication -- If you do not define an Authenticator then all clients are accepted.
* Basic Authentication -- uses the {url-api-references-classes}ListenerPasswordAuthenticator.html[ListenerPasswordAuthenticator] to authenticate the client using the client-supplied username and password (from the http authentication header).
* {url-api-references-classes}ListenerCertificateAuthenticator.html[ListenerCertificateAuthenticator] -- which authenticates the client using a client supplied chain of one or more certificates.
...
* A chain of one or more certificates that ends at a certificate in the list of certificates supplied to the constructor for  {url-api-references-classes}ListenerCertificateAuthenticator.html[ListenerCertificateAuthenticator] -- see: xref:c:p2psync-websocket-using-passive.adoc#ex-set-cert-auth[]
...
Configure the server (listener) to authenticate the client against a list of one or more certificates provided by the server to the the {url-api-references-classes}ListenerCertificateAuthenticator.html[ListenerCertificateAuthenticator].
...
* On first use -- Bring your own certificate and private key; for example, using the {url-api-references-classes}TLSIdentity.html[TLSIdentity] class's {url-api-references-classes}TLSIdentity.html#TLSIdentity(cm)createIdentityForServer:attributes:expiration:label:error:[createIdentity()] method to add it to the secure storage.
* Each time -- Use the server identity from the certificate stored in the secure storage; for example, using the {url-api-references-classes}TLSIdentity.html[TLSIdentity] class's {url-api-references-classes}TLSIdentity.html#TLSIdentity(cm)identityWithLabel:error:[identityWithLabel:error] method with the alias you want to retrieve..
...
Use the Listener's `{url-api-references-classes}URLEndpointListener.html#CBLURLEndpointListener(py)status[status]` property/method to get counts of total and active connections -- see: xref:c:p2psync-websocket-using-passive.adoc#get-connection-counts[].

You should note that these counts can be extremely volatile. So, the actual number of active connections may have changed, by the time the `{url-api-references-classes}Type%20Definitions/ConnectionStatus.html[ConnectionStatus]` class returns a result.
