...
****
. Create a new Couchbase Lite database with the required dataset -- see xref:c:prebuilt-database.adoc#crt-db[]
. Incorporate the pre-built database with your app bundle as an asset/resource -- see xref:c:prebuilt-database.adoc#bundle-db[]
...
Start a sync to update the data -- see xref:c:prebuilt-database.adoc#deploy-db[]
****
...
* You can write a simple CBL app to start a pull sync -- see: xref:c:replication.adoc[Remote Sync Gateway]
...
{url-api-references-classes}database.html#ga027d34b2de65b040ecf42a2a83bf6720[CBL_CopyDatabase()]
...
. Use {url-api-references-classes}database.html#ga76a603bc678ceae18c9610b8a8274a09[CBLDatabase_ChangeEncryptionKey()] on the database copy to set the required encryption key. +
...
+
Use the API's {url-api-references-classes}database.html#ga027d34b2de65b040ecf42a2a83bf6720[CBL_CopyDatabase()] method -- see: xref:c:prebuilt-database.adoc#lbl-code[]; this ensures that a UUID is generated for each copy
+
