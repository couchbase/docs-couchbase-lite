...
You can create a new database and-or open an existing database, using the {url-api-references-classes}database.html[CBLDatabase] class.
Just pass in a database name and optionally a https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-c}{empty}/couchbase-lite-c/C/html/struct_c_b_l_database_configuration.html[DatabaseConfiguration] -- see xref:c:database.adoc#ex-dbopen[].
...
See also xref:c:database.adoc#lbl-find-db-loc[].
--
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=new-database,indent=0]
----
--
=====
====
...
To close a database, use {url-api-references-classes}database.html#ga4d448b2d6809d6f9633d810d3ac6dcfa[CBLDatabase_Close()] -- see: xref:c:database.adoc#ex-dbclose[].
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=close-database,indent=0]
----
--
=====
====
...
Database Full Sync prevents the loss of transactional data due to an unexpected system crash or loss of power.
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=database-fullsync,indent=0]
----
--
=====
====
...
NOTE: It's' not possible to change the configuration of a Database after instantiating the Database with the configuration by updating its `DatabaseConfiguration` property.
...
Provide this encryption key every time the database is opened -- see xref:c:database.adoc#ex-sdb-encrypt[].
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=database-encryption,indent=0]
----
--
=====
====
...
[#changing]
=== Changing
To change an existing encryption key, open the database using its existing encryption-key and use {url-api-references-classes}database.html#ga76a603bc678ceae18c9610b8a8274a09[CBLDatabase_ChangeEncryptionKey()]
...
{url-api-references-classes}database.html#ga76a603bc678ceae18c9610b8a8274a09[CBLDatabase_ChangeEncryptionKey()]
...
Couchbase Lite's API provides the {url-api-references-classes}database.html#gaa4b06dcb7427cafeabde8486f5f03f10[CBLDatabase_PerformMaintenance()] method.
The available maintenance operations, including `compact` are as shown in the enum {url-api-references-classes}database.html#gaace029f966f053946a52f837c285f156[CBLMaintenanceType] to accomplish this.
