...
NOTE: Couchbase documents are assigned to a xref:c:database.adoc#database-concepts[Collection].
...
The values can be made of up several different xref:c:document.adoc#data-types[] such as numbers, strings, arrays, and nested objects.
...
Couchbase Lite also provides for the direct handling of JSON data implemented in most cases by the provision of a `toJSON()` method on appropriate API classes (for example, on MutableDocument, Dictionary, Blob and Array) -- see xref:c:document.adoc#lbl-json-data[].
...
Couchbase documents are assigned to a xref:c:database.adoc#database-concepts[Collection].
...
For more on using *Documents*, see xref:c:document.adoc#document-initializers[] and xref:c:document.adoc#mutability[].
...
Learn more about xref:c:document.adoc#using-dictionaries[].
...
Learn more about xref:c:document.adoc#using-arrays[]
...
* {url-api-references-classes}f_l_dict.html[Fleece Dictionaries]
...
.API References
* {url-api-references-classes}f_l_array.html[Fleece Arrays]
...
* Use the {url-api-references-classes}documents.html#ga226f555fffb7543558048af20b18b737[CBLDocument_Create()] initializer to create a new document where the document ID is randomly generated by the database.

* Use the {url-api-references-classes}documents.html#gaec41cf2eab4e3ac490f11007f665a35e[CBLDocument_CreateWithID()] initializer to create a new document with a specific ID.
...
To make an immutable document mutable, use {url-api-references-classes}documents.html#ga0356b98d2f1798adc8f549510d3eef67[CBLDocument_MutableCopy()].
...
NOTE: Any user change to the value of reserved keys (`_id`, `_rev` or `_deleted`) will be detected when a document is saved and will result in an exception (Error Code 5 -- `CorruptRevisionData`) -- see also xref:c:document.adoc#lbl-doc-constraints[].
...
NOTE: "_id", :"_rev" and "_sequence" are reserved keywords and must not be used as top-level attributes -- see xref:c:document.adoc#res-keys[].
...
In this section::
xref:c:document.adoc#lbl-array[]
| xref:c:document.adoc#lbl-dictionary[]
| xref:c:document.adoc#lbl-document[]
| xref:c:document.adoc#lbl-result[]
...
Convert an `ArrayObject` to and from JSON using the `toJSON()` and `toArray` methods -- see xref:c:document.adoc#ex-array[].

Additionally you can:

* Initialize a 'MutableArrayObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor -- see: xref:c:document.adoc#ex-array[]
...
Convert a `DictionaryObject` to and from JSON using the `toJSON` and `toDictionary` methods -- see xref:c:document.adoc#ex-dictionary[].

Additionally you can:

* Initialize a 'MutableDictionaryObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor-- see: xref:c:document.adoc#ex-dictionary[]
...
Convert a `Document` to and from JSON strings using the `toJSON()` and `setJSON()` methods -- see xref:c:document.adoc#ex-document[].

Additionally you can:

* Initialize a 'MutableDocument' using data supplied as a JSON string.
This is done using the `init(json)` or `init(id: json:)` constructor -- see: xref:c:document.adoc#ex-document[]
...
Use {url-api-references-classes}json.html#ga3450acc0690101545d75986b91e4080[FLValue_ToJSON()] to transform your result string into a JSON string, which can easily be serialized or used as required in your application. See xref:c:document.adoc#ex-json[] for a working example.
