...
_Couchbase Lite for C_ **beta** is available for all supported platorms -- see: <<lbl-beta-platforms>>.
You can obtain the downloads here <<tbl-beta-downloads>> .
...
. Just `wget` the appropriate `.deb` package  -- see <<tbl-beta-downloads>>  for a BETA package URL.
...
. *Download* and *Unpack* the binary release -- see: <<tbl-beta-downloads>>
...
[#lbl-windows-steps]
. Download and extract the release package -- see: <<tbl-beta-downloads>>
