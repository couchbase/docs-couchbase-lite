...
Related Content -- xref:c:conflict.adoc[Handling Data Conflicts] | xref:c:dbreplica.adoc[Intra-Device] | <<p2psync-websocket.adoc,Peer-to-Peer>>
--
...
To use this protocol the replication URL should specify WebSockets as the URL scheme (see the xref:c:replication.adoc#lbl-cfg-tgt[] section below).
...
xref:c:replication.adoc#ex-simple-repl[] shows the configuration and initialization process.
...
<.> The default is to auto-purge documents that this user no longer has access to -- see: xref:c:replication.adoc#anchor-auto-purge-on-revoke[].
...
By default, clients accept only servers presenting certificates that can be verified using the OS bundled Root CA Certificates -- see: xref:c:replication.adoc#lbl-svr-auth[].
<.> Configure the client-authentication credentials (if required).
These are the credential the client will present to sync{nbsp}gateway if requested to do so. +
Here we configure to provide _Basic Authentication_ credentials.
Other options are available -- see: xref:c:replication.adoc#lbl-client-auth[].
...
<.> Initialize the replicator using your configuration -- see: xref:c:replication.adoc#lbl-init-repl[].

<.> Optionally, register an observer, which will notify you of changes to the replication status -- see: xref:c:replication.adoc#lbl-repl-mon[]

<.> Start the replicator  -- see: xref:c:replication.adoc#lbl-repl-start[].
...
+
--
xref:c:replication.adoc#lbl-cfg-tgt[]
|  xref:c:replication.adoc#lbl-cfg-sync[]
|  xref:c:replication.adoc#lbl-cfg-keep-alive[]
|  xref:c:replication.adoc#lbl-user-auth[]
|  xref:c:replication.adoc#lbl-client-auth[]
|  xref:c:replication.adoc#lbl-repl-evnts[]
|  xref:c:replication.adoc#lbl-repl-hdrs[]
|  xref:c:replication.adoc#lbl-repl-ckpt[]
|  xref:c:replication.adoc#lbl-repl-fltrs[]
|  xref:c:replication.adoc#lbl-repl-chan[]
|  xref:c:replication.adoc#anchor-auto-purge-on-revoke[]
|  xref:c:replication.adoc#lbl-repl-delta[]
--
...
The REST API provides configurable control over this replication retry logic using a set of configiurable properties -- see: xref:c:replication.adoc#tbl-repl-retry[].
...
When necessary you can adjust any or all of those configurable values -- see: xref:c:replication.adoc#ex-repl-retry[] for how to do this.
...
You can enable authorization in the sync{nbsp}gateway configuration file, as shown in xref:c:replication.adoc#example-enable-authorization[].
...
There are two ways to authenticate from a Couchbase Lite client: xref:c:replication.adoc#basic-authentication[] or xref:c:replication.adoc#session-authentication[].
...
xref:c:replication.adoc#ex-base-auth[] shows how to initiate a one-shot replication as the user *username* with the password *password*.
...
See xref:c:replication.adoc#ex-session-auth[], which shows how to initiate a one-shot replication with the session ID returned from the `+POST /{tkn-db}/_session+` endpoint.
...
This feature is useful in passing additional credentials, perhaps when an authentication or authorization step is being done by a proxy server (between Couchbase Lite and Sync Gateway) -- see xref:c:replication.adoc#ex-cust-hdr[].
...
For more information, see xref:c:replication.adoc#auto-purge-on-revoke[Auto Purge on Revoke].
****
...
Optionally, it's also possible to specify a string array of channel names on Couchbase Lite's collection configuration object.
...
If a user subsequently regains access to a lost channel, then any previously auto-purged documents still assigned to any of their channels are automatically pulled down by the active Sync Gateway when they are next updated -- see behavior summary in xref:c:replication.adoc#tbl-regain-behavior[]
...
[source, c]
----
include::c:example$code_snippets/main.cpp[tags="sgw-config-autopurge", indent=0]
----
...
This can be done either by setting https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-c}{empty}/couchbase-lite-c/C/html/struct_c_b_l_replicator_configuration.html#a3795c0097264ccd1ed612d9a0746d58d[disableAutoPurge] to false, or for finer control by applying pull-filters -- see: xref:c:replication.adoc#tbl-pull-filters[] and xref:c:replication.adoc#lbl-repl-fltrs[]
...
<<p2psync-websocket.adoc,Peer-to-Peer>>
...
In this section::
xref:c:replication.adoc#lbl-repl-start[]  | xref:c:replication.adoc#lbl-repl-ckpt[]

[#lbl-repl-start]
=== Start Replicator

Use the `{url-api-references-classes}replication.html[Replication]` class's' {url-api-references-classes}replication.html#(im)initWithConfig:[initWith(config:)] constructor, to initialize the replicator with the configuration you have defined.
You can, optionally, add a change listener (see xref:c:replication.adoc#lbl-repl-mon[]) before starting the replicator running using {url-api-references-classes}replication.html#ga224926daa794a424c470bf86dd57aaf9[CBLReplicator_Start()].
...
However, if you do want to force the replication to start again from zero, use the xref:refer-glossary.adoc#checkpoint[checkpoint] reset argument when starting the replicator -- as shown in xref:c:replication.adoc#ex-repl-ckpt[].
...
In this section::
xref:c:replication.adoc#lbl-repl-chng[]  |
xref:c:replication.adoc#lbl-repl-status[]  |
xref:c:replication.adoc#lbl-repl-evnts[] |
xref:c:replication.adoc#lbl-repl-pend[] |
xref:c:replication.adoc#lbl-repl-correlation-id[]

You can monitor a replication’s status by using a combination of xref:c:replication.adoc#lbl-repl-chng[] and the `replication.status.activity` property -- see; {url-api-references-classes}replication.html#ga9421513c63f1d16bf4740c4d2515dd22[CBLReplicatorActivityLevel enum].
This enables you to know, for example, when the replication is actively transferring data and when it has stopped.

You can also choose to monitor document changes -- see: xref:c:replication.adoc#lbl-repl-evnts[].
...
Use the {url-api-references-classes}replication.html[Replication] class to add a change listener as a callback to the Replicator ({url-api-references-classes}replication.html#(im)addChangeListener:[addChangeListener(_:)]) -- see: xref:c:replication.adoc#ex-repl-mon[].
You will then be asynchronously notified of state changes.

You can remove a change listener with {url-api-references-classes}replication.html#(im)removeChangeListenerWithToken[removeChangeListenerWithToken(ListenerToken:)].
...
That is, whether it is actively transferring data or if it has stopped -- see: xref:c:replication.adoc#ex-repl-mon[].

The returned _ReplicationStatus_ structure comprises:

* {url-api-references-classes}replication.html#ga9421513c63f1d16bf4740c4d2515dd22[CBLReplicatorActivityLevel enum] -- stopped, offline, connecting, idle or busy -- see states described in: xref:c:replication.adoc#tbl-states[]
...
[#lbl-repl-states]
==== Replication States
xref:c:replication.adoc#tbl-states[] shows the different states, or activity levels, reported in the API; and the meaning of each.
...
For example, the code snippet in xref:c:replication.adoc#ex-reg-doc-listener[] registers a listener to monitor document replication performed by the replicator referenced by the variable `replicator`.
It prints the document ID of each document received and sent.
Stop the listener as shown in xref:c:replication.adoc#ex-stop-doc-listener[].
...
[source, c]
----
include::c:example$code_snippets/main.cpp[tags="add-document-replication-listener", indent=0]
----
...
TIP: {url-api-references-classes}replication.html#ga493eeac915dd54a274b907a010664a2e[CBLReplicator.isDocumentPending()] is quicker and more efficient.
Use it in preference to returning a list of pending document IDs, where possible.

You can check whether documents are waiting to be pushed in any forthcoming sync by using either of the following API methods:

* Use the {url-api-references-classes}replication.html#ga6e9902ae56d5fec0fc19b29c28f1828f[CBLReplicator_PendingDocumentIDs()] method, which returns a list of document IDs that have local changes, but which have not yet been pushed to the server.
+
...
* Use the {url-api-references-classes}replication.html#ga493eeac915dd54a274b907a010664a2e[CBLReplicator.isDocumentPending()] method to quickly check whether an individual document is pending a push.
...
<.> {url-api-references-classes}replication.html#ga6e9902ae56d5fec0fc19b29c28f1828f[CBLReplicator_PendingDocumentIDs()] returns a list of the document IDs for all documents waiting to be pushed.
This is a snapshot and may have changed by the time the response is received and processed.
<.> {url-api-references-classes}replication.html#ga493eeac915dd54a274b907a010664a2e[CBLReplicator.isDocumentPending()] returns `true` if the document is waiting to be pushed, and `false` otherwise.
...
It's' done using {url-api-references-classes}replication.html#ga548ce284032009546d4092745e89fa8e[CBLReplicator_Stop()].
...
You can find further information about database operations in xref:c:database.adoc[Databases].
...
<.> Here we initiate the stopping of the replication using the {url-api-references-classes}replication.html#ga548ce284032009546d4092745e89fa8e[CBLReplicator_Stop()] method.
It will stop any active xref:c:replication.adoc#lbl-repl-chng[change listener] once the replication is stopped.
...
If Sync Gateway is configured with a self signed certificate, and your app points to a `wss` scheme but the replicator configuration isn't using the certificate you'll encounter an error with status code `5011` -- see: <<ex-5011>>
