...
:url-api-references-replicator: {url-api-references}/group__replication.html
:url-api-references-replicator-config: {url-api-references}/struct_c_b_l_replicator_configuration.html
...
// NOTE: This example uses hardcoded C code, not a tagged include.
// C++ tab to be added after engineering provides tagged include in main.cpp and cbl_cpp.cpp.
...
. Configure the URL endpoint of the server.
Here we connect only to servers presenting a self-signed certificate.
By default, clients accept only servers presenting certificates that can be verified using the OS bundled Root CA Certificates -- see: <<authenticate-listener>>. @/CBLEndpoint_CreateWithURL/

. Configure the credentials the client will present to the server.
Here we say to provide _Basic Authentication_ credentials.
Other options are available -- see: <<lbl-authclnt>>. @/passwd/

. Configure how the replication should perform <<conflict-resolution>>. @/conflictResolver/

. Initialize the replicator using your configuration object. @/CBLReplicator_Create/

. Register an observer, which will notify you of changes to the replication status. @/NULL/
...
Initialize and define the replication configuration with local and remote database locations using the {url-api-references-replicator-config}[CBLReplicatorConfiguration] object.
...
C++::
+
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=sgw-act-rep-initialize,indent=0]
----
--
=====
====

Note use of the scheme prefix: `ws://`, or `wss://`
to ensure TLS encryption -- strongly recommended in production.
...
We use `{url-api-references-replicator-config}[CBLReplicatorConfiguration]` class's {url-api-references-replicator-config}#a40f3195389ab0578aa17e63dd832a390[replicatorType] and
{url-api-references-replicator-config}#a3d17159fc65a7491c2cde2f56a5016df[continuous] parameters, to tell the replicator:
...
+
--
[source.no-callouts, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=p2p-act-rep-config-type;p2p-act-rep-config-cont,indent=0]
----
--
=====
====
...
The REST API provides configurable control over this replication retry logic using a set of configurable properties -- see: <<tbl-repl-retry>>.
...
|{url-api-references-replicator-config}#a524408f9430d13c783762dce32f1126e[heartbeat]
...
|{url-api-references-replicator-config}#a5b6435c711d93f71566d8814506e0dff[maxAttempts]
...
|{url-api-references-replicator-config}#a21d4fffd270c721d1600d7a835f17fd1[maxAttemptWaitTime]
...
<.> Here we use {url-api-references-replicator-config}#a524408f9430d13c783762dce32f1126e[heartbeat] to set the required interval (in seconds) between the heartbeat pulses.
<.> Here we use {url-api-references-replicator-config}#a5b6435c711d93f71566d8814506e0dff[maxAttempts] to set the required number of retry attempts.
<.> Here we use {url-api-references-replicator-config}#a21d4fffd270c721d1600d7a835f17fd1[maxAttemptWaitTime] to set the required interval between retry attempts.
--
...
<.> Here we use maxAttemptWaitTime to set the required interval between retry attempts.
--
=====
...
Use the `{url-api-references-replicator-config}[CBLReplicatorConfiguration]` properties
{url-api-references-replicator-config}#ad1a35fd126871670d2677cabff87f2c3[acceptOnlySelfSignedServerCertificate] and {url-api-references-replicator-config}#a4a96fb9fba93dc93f8373b76c3816af6[pinnedServerCertificate()],
to tell the replicator how to verify server-supplied TLS server certificates.

* If there is a pinned certificate, nothing else matters, the server cert must *exactly* match the pinned certificate.
* If there are no pinned certs and {url-api-references-replicator-config}#ad1a35fd126871670d2677cabff87f2c3[acceptOnlySelfSignedServerCertificate] is `true` then any self-signed certificate is accepted.
Certificates that are not self signed are rejected, no matter who signed them.
* If there are no pinned certificates and {url-api-references-replicator-config}#ad1a35fd126871670d2677cabff87f2c3[acceptOnlySelfSignedServerCertificate] is `false` (default), the client validates the server's' certificates against the system CA certificates.
The server must supply a chain of certificates whose root is signed by one of the certificates in the system CA bundle.
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=p2p-act-rep-config-cacert,indent=0]
----
This is the default.
Only certificate chains with roots signed by a trusted CA are allowed.
Self signed certificates are not allowed.
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=p2p-act-rep-config-self-cert,indent=0]
----
Set this to `true` to accept any self signed cert.
Any certificates that are not self-signed are rejected.
--
...
[source, c]
----
include::c:example$code_snippets/main.cpp[tags=p2p-act-rep-config-pinnedcert, indent=0]
----
--
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=p2p-act-rep-config-pinnedcert,indent=0]
----
--
======
====
...
We use {url-api-references-replicator-config}[CBLReplicatorConfiguration]'s {url-api-references-replicator-config}#aa2cd20835cc4e6b516510d441d419d60[authenticator] method to define the authentication method to the replicator.
...
Use the `{url-api-references-replicator}[Replication]` class's {url-api-references-replicator}#ga5f5ffc039ce0ac834d1f1f4cd940d65d[CBLReplicator_Create()] constructor, to initialize the replicator with the configuration you have defined.
You can, optionally, add a change listener (see <<lbl-repl-mon>>) before starting the replicator running using {url-api-references-replicator}#ga224926daa794a424c470bf86dd57aaf9[CBLReplicator_Start()].
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=p2p-act-rep-start-full,indent=0]
----
--
=====
====
...
You can monitor a replication's status by using a combination of <<lbl-repl-chng>> and the `replication.status.activity` property -- see; {url-api-references-replicator}#ga9421513c63f1d16bf4740c4d2515dd22[CBLReplicatorActivityLevel enum].
...
Use the {url-api-references-replicator}[Replication] class to add a change listener as a callback to the Replicator ({url-api-references-replicator}#gaa49518d403aaa95f9b342759e0e30769[CBLReplicator_AddChangeListener()]) -- see: <<ex-repl-mon>>.
You'll then be asynchronously notified of state changes.
...
You can use the
{url-api-references}/struct_c_b_l_replicator_status.html[CBLReplicatorStatus] struct
...
* {url-api-references-replicator}#ga9421513c63f1d16bf4740c4d2515dd22[CBLReplicatorActivityLevel enum] -- stopped, offline, connecting, idle or busy -- see states described in: <<tbl-states>>
* {url-api-references}/struct_c_b_l_replicator_progress.html[CBLReplicatorProgress struct]
** completed -- the total number of changes completed
** total -- the total number of changes to be processed
* {url-api-references}/struct_c_b_l_error.html[CBLError struct] -- the current error, if any
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=p2p-act-rep-add-change-listener,indent=0]
----
--
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=p2p-act-rep-status,indent=0]
----
--
======
====
...
TIP: {url-api-references-replicator}#ga493eeac915dd54a274b907a010664a2e[CBLReplicator_IsDocumentPending()] is quicker and more efficient.
Use it in preference to returning a list of pending document IDs, where possible.

You can check whether documents are waiting to be pushed in any forthcoming sync by using either of the following API methods:

* Use the {url-api-references-replicator}#ga6e9902ae56d5fec0fc19b29c28f1828f[CBLReplicator_PendingDocumentIDs()] method, which returns a list of document IDs that have local changes, but which have not yet been pushed to the server.
+
...
* Use the {url-api-references-replicator}#ga493eeac915dd54a274b907a010664a2e[CBLReplicator_IsDocumentPending()] method to quickly check whether an individual document is pending a push.
...
<.> {url-api-references-replicator}#ga6e9902ae56d5fec0fc19b29c28f1828f[CBLReplicator_PendingDocumentIDs()] returns a list of the document IDs for all documents waiting to be pushed.
This is a snapshot and may have changed by the time the response is received and processed.
<.> {url-api-references-replicator}#ga493eeac915dd54a274b907a010664a2e[CBLReplicator_IsDocumentPending()] returns `true` if the document is waiting to be pushed, and `false` otherwise.
--
...
This is a snapshot and may have changed by the time the response is received and processed.
<.> Returns `true` if the document is waiting to be pushed, and `false` otherwise.
--
=====
...
It is done using {url-api-references-replicator}#ga548ce284032009546d4092745e89fa8e[CBLReplicator_Stop()].
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=p2p-act-rep-stop,indent=0]
----
--
=====
====

Here we initiate the stopping of the replication using the {url-api-references-replicator}#ga548ce284032009546d4092745e89fa8e[CBLReplicator_Stop()] method.
...
When conflicts occur during pull replication, the change with the latest timestamp wins.
Timestamps are local to each device.
See xref:java:conflict.adoc[Handling Data Conflicts].

To use a different policy, specify a _conflict resolver_ using {url-api-references-replicator-config}#aa664454f386c8300dd458ece30a3a8c6[conflictResolver] as shown in <<using-conflict-resolvers>>.
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=local-win-conflict-resolver,indent=0]
----
--
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=remote-win-conflict-resolver,indent=0]
----
--
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=merge-conflict-resolver,indent=0]
----
--
...
[source, cpp]
----
include::c:example$code_snippets_cpp/cbl_cpp.cpp[tags=update-document-with-conflict-handler,indent=0]
----
--
=====
====
