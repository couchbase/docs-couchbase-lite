...
[source, c]
----
include::c:example$code_snippets/p2preplication.cpp[tags="p2p-act-rep-func;!autopurge-override", indent=0]
----
====
...
{url-api-references-replicator-config}#abba75db71f5e08718a924d76bc1a0e1e[acceptOnlySelfSignedServerCertificate] and {url-api-references-replicator-config}#a4a96fb9fba93dc93f8373b76c3816af6[pinnedServerCertificate()],
...
* If there are no pinned certs and {url-api-references-replicator-config}#abba75db71f5e08718a924d76bc1a0e1e[acceptOnlySelfSignedServerCertificate] is `true` then any self-signed certificate is accepted.
Certificates that are not self signed are rejected, no matter who signed them.
* If there are no pinned certificates and {url-api-references-replicator-config}#abba75db71f5e08718a924d76bc1a0e1e[acceptOnlySelfSignedServerCertificate] is `false` (default), the client validates the server's certificates against the system CA certificates.
