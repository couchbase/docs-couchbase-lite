...
[#output]
=== Output
The output from `https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-c}{empty}/couchbase-lite-c/C/html/group__queries.html#qe-output>> for an example of how it looks.
...
For example, you can observe a similar situation to that shown in <<use-like-based-queries>> when using the `https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-c}{empty}/couchbase-lite-c/C/html/group__QueryFunction.html#CBLQueryFunction(cm)lower:[lower()]` function on an indexed property.
