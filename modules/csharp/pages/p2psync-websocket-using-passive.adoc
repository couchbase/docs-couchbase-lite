...
Related Content -- https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net[API Reference]  |  xref:csharp:p2psync-websocket-using-active.adoc[Active Peer]
--
...
<.> Retrieve the total number of connections (active and passive) currently established with the listener.

<.> Retrieve the number of connections currently replicating data, a subset of the total connection count.
...
<.> Set the list of local collections using the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.P2P.URLEndpointListenerConfiguration.html[URLEndpointListenerConfiguration].
...
Configure the server (listener) to authenticate the client against a list of one or more certificates provided by the server to the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.P2P.ListenerCertificateAuthenticator.html[ListenerCertificateAuthenticator].

[source, C#]
----
include::csharp:example$code_snippets/Program.cs[tags="listener-config-client-auth-root", indent=0]
----
...
<.> Remove the TLS identity stored under the specified label from secure storage.
...
<.> Initialize the listener using the completed configuration.
<.> Start the listener.
...
Use the Listener's `https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.P2P.URLEndpointListener.html#Couchbase_Lite_P2P_URLEndpointListener_Status[Status]` property/method to get counts of total and active connections -- see: <<get-connection-counts>>.
...
. Retrieve the total number of connections established with the listener.
. Retrieve the number of connections actively replicating data.
This is a subset of the total connection count.


[#stop-listener]
== Stop Listener

It's best practice to check the status of the Listener's connections and stop only when you have confirmed that there are no active connections -- see <<get-connection-counts>>.
