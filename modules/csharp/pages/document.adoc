...
Couchbase Lite also provides for the direct handling of JSON data implemented in most cases by the provision of a `ToJSON()` method on appropriate API classes (for example, on MutableDocument, Dictionary, Blob and Array) -- see xref:csharp:document.adoc#lbl-json-data[].
...
Convert an `ArrayObject` to and from JSON using the `ToJSON()` and `toArray` methods -- see xref:csharp:document.adoc#ex-array[].
...
Convert a `Document` to and from JSON strings using the `ToJSON()` and `SetJSON()` methods -- see xref:csharp:document.adoc#ex-document[].
