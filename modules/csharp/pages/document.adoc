...
NOTE: Couchbase documents are assigned to a xref:csharp:database.adoc#database-concepts[Collection].
...
The values can be made of up several different xref:csharp:document.adoc#data-types[] such as numbers, strings, arrays, and nested objects.
...
Couchbase Lite also provides for the direct handling of JSON data implemented in most cases by the provision of a pass:a,q[`ToJSON()`] method on appropriate API classes (for example, on MutableDocument, Dictionary, Blob and Array) -- see xref:csharp:document.adoc#lbl-json-data[].
...
Couchbase documents are assigned to a xref:csharp:database.adoc#database-concepts[Collection].
...
For more on using *Documents*, see xref:csharp:document.adoc#document-initializers[] and xref:csharp:document.adoc#mutability[].
...
Learn more about xref:csharp:document.adoc#using-dictionaries[].
...
Learn more about xref:csharp:document.adoc#using-arrays[]
...
This example sets the date on the `createdAt` property and reads it back using the https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.Document.html#Couchbase_Lite_Document_GetDate_System_String_[Document.GetDate()] accessor method.
...
NOTE: Any user change to the value of reserved keys (`_id`, `_rev` or `_deleted`) will be detected when a document is saved and will result in an exception (Error Code 5 -- `CorruptRevisionData`) -- see also xref:csharp:document.adoc#lbl-doc-constraints[].
...
NOTE: "_id", :"_rev" and "_sequence" are reserved keywords and must not be used as top-level attributes -- see xref:csharp:document.adoc#res-keys[].
...
In this section::
xref:csharp:document.adoc#lbl-array[]
| xref:csharp:document.adoc#lbl-blob[]
| xref:csharp:document.adoc#lbl-dictionary[]
| xref:csharp:document.adoc#lbl-document[]
| xref:csharp:document.adoc#lbl-result[]
...
Convert an `ArrayObject` to and from JSON using the pass:a,q[`ToJSON()`] and `toArray` methods -- see xref:csharp:document.adoc#ex-array[].

Additionally you can:

* Initialize a 'MutableArrayObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor -- see: xref:csharp:document.adoc#ex-array[]
...
[#lbl-blob]
=== Blobs
Convert a `Blob` to JSON using the `toJSON` method -- see xref:csharp:document.adoc#ex-blob[].

You can use `isBlob()` to check whether a given dictionary object is a blob or not -- see xref:csharp:document.adoc#ex-blob[].
...
Convert a `DictionaryObject` to and from JSON using the `toJSON` and `toDictionary` methods -- see xref:csharp:document.adoc#ex-dictionary[].

Additionally you can:

* Initialize a 'MutableDictionaryObject' using data supplied as a JSON string.
This is done using the `init(json)` constructor-- see: xref:csharp:document.adoc#ex-dictionary[]
...
Convert a `Document` to and from JSON strings using the pass:a,q[`ToJSON()`] and pass:a,q[`SetJSON()`] methods -- see xref:csharp:document.adoc#ex-document[].

Additionally you can:

* Initialize a 'MutableDocument' using data supplied as a JSON string.
This is done using the `init(json)` or `init(id: json:)` constructor -- see: xref:csharp:document.adoc#ex-document[]
