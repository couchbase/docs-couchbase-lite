...
==== Output
The output from `https://docs.couchbase.com/mobile/{version-maintenance-net}/couchbase-lite-net/api/Couchbase.Lite.Query.#Couchbase_Lite__Query_IQuery_Explain[Explain()]` remains the same whether invoked by an app, or `cblite` -- see <<csharp:troubleshooting-queries:::qe-output>> for an example of how it looks.
