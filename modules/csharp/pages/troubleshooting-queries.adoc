...
This output (xref:csharp:troubleshooting-queries.adoc#qe-output[]) comprises three main elements:
...
Something you may well be looking to optimize -- see xref:csharp:troubleshooting-queries.adoc#ret-method[] for more.
<.> *Grouping method* --- This line shows that the *Group By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see xref:csharp:troubleshooting-queries.adoc#order-group[].
<.> *Ordering method* -- This line shows that the *Order By* clause used in the query requires the data to be sorted and that a b-tree will be used for temporary storage -- see xref:csharp:troubleshooting-queries.adoc#order-group[].
====
...
The _retrieval method_ shows the approach decided upon by the optimizer -- see xref:csharp:troubleshooting-queries.adoc#ret-meths[].
...
So, if your analysis of the xref:csharp:troubleshooting-queries.adoc#qe-output[Query Explain output] indicates a sub-optimal query and your rewrites fail to sufficiently optimize it, consider compacting the database.
...
By contrast, by removing the wildcard prefix `%` (in xref:csharp:troubleshooting-queries.adoc#like-no-wild-pfx-qry[]), we see that the query plan's retrieval method changes to become an index search.
