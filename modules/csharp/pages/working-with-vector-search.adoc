...
IMPORTANT: Similar to the xref:csharp:fts.adoc[Full Text Search] `match()` function, the `APPROX_VECTOR_DISTANCE()` function and Hybrid Vector Search cannot use the `OR` expression with the other expressions in the related `WHERE` clause.
...
The `LIMIT` clause is required for all Vector Search queries to prevent accidental, exhaustive, slow, and resource intensive searches of the database.
