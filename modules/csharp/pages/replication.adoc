...
Related Content -- xref:csharp:conflict.adoc[] | xref:csharp:dbreplica.adoc[Intra-Device] | xref:csharp:replication:p2psync-websocket.adoc[Peer-to-Peer]
--
...
To use this protocol the replication URL should specify WebSockets as the URL scheme (see the xref:csharp:replication:::csharp:replication.adoc#lbl-cfg-tgt[] section below).

Incompatibilities::
Couchbase Lite's replication protocol is *incompatible* with CouchDB-based databases.
And since Couchbase Lite 2.x+ only supports the new protocol, you will need to run a version of Sync Gateway that supports it -- see: xref:csharp:compatibility.adoc[].
...
xref:csharp:replication:::csharp:replication.adoc#ex-simple-repl[] shows the configuration and initialization process.
...
By default, clients accept only servers presenting certificates that can be verified using the OS bundled Root CA Certificates -- see: xref:csharp:replication:::csharp:replication.adoc#lbl-svr-auth[].
...
Other options are available -- see: xref:csharp:replication:::csharp:replication.adoc#lbl-client-auth[].

<.> Configure how the replication should handle conflict resolution -- see: xref:csharp:conflict.adoc[] topic for mor on conflict resolution.

<.> Initialize the replicator using your configuration -- see: xref:csharp:replication:::csharp:replication.adoc#lbl-init-repl[].

<.> Optionally, register an observer, which will notify you of changes to the replication status -- see: xref:csharp:replication:::csharp:replication.adoc#lbl-repl-mon[]

<.> Start the replicator  -- see: xref:csharp:replication:::csharp:replication.adoc#lbl-repl-start[].
...
+
--
xref:csharp:replication:::csharp:replication.adoc#lbl-cfg-tgt[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-cfg-sync[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-cfg-keep-alive[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-user-auth[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-svr-auth[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-client-auth[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-repl-evnts[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-repl-hdrs[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-repl-ckpt[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-repl-fltrs[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-repl-chan[]
|  xref:csharp:replication:::csharp:replication.adoc#anchor-auto-purge-on-revoke[]
|  xref:csharp:replication:::csharp:replication.adoc#lbl-repl-delta[]
--
...
The REST API provides configurable control over this replication retry logic using a set of configiurable properties -- see: xref:csharp:replication:::csharp:replication.adoc#tbl-repl-retry[].
...
When necessary you can adjust any or all of those configurable values -- see: xref:csharp:replication:::csharp:replication.adoc#ex-repl-retry[] for how to do this.
...
You can enable authorization in the pass:q,a[sync{nbsp}gateway] configuration file, as shown in xref:csharp:replication:::csharp:replication.adoc#example-enable-authorization[].
...
This all assumes that you have configured the Sync Gateway to provide the appropriate SSL certificates, and have included the appropriate certificate in your app bundle -- for more on this see: xref:csharp:replication:::csharp:replication.adoc#lbl-cert-pinning[].


[#lbl-client-auth]
= Client Authentication

There are two ways to authenticate from a Couchbase Lite client: xref:csharp:replication:::csharp:replication.adoc#basic-authentication[] or xref:csharp:replication:::csharp:replication.adoc#session-authentication[].
...
xref:csharp:replication:::csharp:replication.adoc#ex-base-auth[] shows how to initiate a one-shot replication as the user *username* with the password *password*.
...
See xref:csharp:replication:::csharp:replication.adoc#ex-session-auth[], which shows how to initiate a one-shot replication with the session ID returned from the `+POST /{tkn-db}/_session+` endpoint.
...
This feature is useful in passing additional credentials, perhaps when an authentication or authorization step is being done by a proxy server (between Couchbase Lite and Sync Gateway) -- see xref:csharp:replication:::csharp:replication.adoc#ex-cust-hdr[].
...
For more information, see xref:csharp:replication:::csharp:replication.adoc#auto-purge-on-revoke[Auto Purge on Revoke].
****
...
If a user subsequently regains access to a lost channel, then any previously auto-purged documents still assigned to any of their channels are automatically pulled down by the active Sync Gateway when they are next updated -- see behavior summary in xref:csharp:replication:::csharp:replication.adoc#tbl-regain-behavior[]
...
xref:csharp:replication:p2psync-websocket.adoc[Peer-to-Peer]
...
In this section::
xref:csharp:replication:::csharp:replication.adoc#lbl-repl-start[]  | xref:csharp:replication:::csharp:replication.adoc#lbl-repl-ckpt[]
...
You can, optionally, add a change listener (see xref:csharp:replication:::csharp:replication.adoc#lbl-repl-mon[]) before starting the replicator running using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.Sync.Replicator.html#Couchbase_Lite_Sync_Replicator_Start[Start()].
...
However, if you do want to force the replication to start again from zero, use the https://docs.couchbase.com/couchbase-lite/current/csharp/refer-glossary.html#checkpoint[checkpoint] reset argument when starting the replicator -- as shown in xref:csharp:replication:::csharp:replication.adoc#ex-repl-ckpt[].
...
In this section::
xref:csharp:replication:::csharp:replication.adoc#lbl-repl-chng[]  |
xref:csharp:replication:::csharp:replication.adoc#lbl-repl-status[]  |
xref:csharp:replication:::csharp:replication.adoc#lbl-repl-evnts[] |
xref:csharp:replication:::csharp:replication.adoc#lbl-repl-pend[]

You can monitor a replication’s status by using a combination of xref:csharp:replication:::csharp:replication.adoc#lbl-repl-chng[] and the `replication.status.activity` property -- see; https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.Sync.ReplicatorStatus.html#Couchbase_Lite_Sync_ReplicatorStatus_Activity[Activity].
This enables you to know, for example, when the replication is actively transferring data and when it has stopped.

You can also choose to monitor document changes -- see: xref:csharp:replication:::csharp:replication.adoc#lbl-repl-evnts[].
...
That is, whether it is actively transferring data or if it has stopped -- see: xref:csharp:replication:::csharp:replication.adoc#ex-repl-mon[].


The returned _ReplicationStatus_ structure comprises:

* https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.Sync.ReplicatorStatus.html#Couchbase_Lite_Sync_ReplicatorStatus_Activity[Activity] -- stopped, offline, connecting, idle or busy -- see states described in: xref:csharp:replication:::csharp:replication.adoc#tbl-states[]
...
[#lbl-repl-states]
== Replication States
xref:csharp:replication:::csharp:replication.adoc#tbl-states[] shows the different states, or activity levels, reported in the API; and the meaning of each.
...
For example, the code snippet in xref:csharp:replication:::csharp:replication.adoc#ex-reg-doc-listener[] registers a listener to monitor document replication performed by the replicator referenced by the variable `replicator`.
It prints the document ID of each document received and sent.
Stop the listener as shown in xref:csharp:replication:::csharp:replication.adoc#ex-stop-doc-listener[].
...
You can find further information on database operations in xref:csharp:database.adoc[].
...
It will stop any active xref:csharp:replication:::csharp:replication.adoc#lbl-repl-chng[change listener] once the replication is stopped.
...
You can increase the log output for activity related to replication with Sync Gateway -- see xref:csharp:replication:::csharp:replication.adoc#ex-logs[].
