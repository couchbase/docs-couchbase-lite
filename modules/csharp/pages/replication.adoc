...
xref:sync-gateway:ROOT:rest_api_admin.adoc#tag/Database-Security/operation/post_db-_user-[`+POST /{db}/_user+`]
...
xref:sync-gateway:ROOT:rest_api_public.adoc#tag/Session/operation/post_db-_session[`+POST /{db}/_session+`]
...
See <<csharp:replication:::ex-session-auth>>, which shows how to initiate a one-shot replication with the session ID returned from the `+POST /{db}/_session+` endpoint.
