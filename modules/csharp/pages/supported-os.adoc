...
.Deprecation Notice
--
Support for the following is deprecated in this release and removed in a future release:
* Windows 10
...
--
The *Minimum Runtime Version* column specifies the minimum required version for each .NET runtime. *Later versions of .NET (including .NET 10 and beyond) are also supported* and expected to work seamlessly due to Microsoft's backward compatibility design.

Couchbase Lite is built to be compatible with newer .NET runtime versions as they're released.
If you encounter any issues when using a newer .NET version, file a support ticket so any breaking changes introduced by Microsoft can be addressed.
--
