...
Sync Gateway users can be created through the xref:sync-gateway::refer/admin-rest-api.adoc#/user/post\__db___user_[`+POST /{tkn-db}/_user+`] endpoint on the Admin REST API.
...
A user session must first be created through the xref:sync-gateway::refer/rest-api.adoc#/session/post\__db___session[`+POST /{tkn-db}/_session+`] endpoint on the Public REST API.
