...
xref:csharp:p2psync-websocket-using-active.adoc#simple-replication-to-listener[] shows the initialization and configuration process.
...
By default, clients accept only servers presenting certificates that can be verified using the OS bundled Root CA Certificates -- see: xref:csharp:p2psync-websocket-using-active.adoc#authenticate-listener[].

<.> Configure the credentials the client will present to the server.
Here we say to provide _Basic Authentication_ credentials. Other options are available -- see: xref:csharp:p2psync-websocket-using-active.adoc#configuring-client-authentication[].

<.> Configure how the replication should perform xref:csharp:p2psync-websocket-using-active.adoc#conflict-resolution[].
...
== Configure Replicator
In this section::
xref:csharp:p2psync-websocket-using-active.adoc#lbl-cfg-tgt[]
|  xref:csharp:p2psync-websocket-using-active.adoc#lbl-cfg-sync[]
|  xref:csharp:p2psync-websocket-using-active.adoc#lbl-cfg-retry[]
|  xref:csharp:p2psync-websocket-using-active.adoc#authenticate-listener[]
|  xref:csharp:p2psync-websocket-using-active.adoc#lbl-authclnt[]
...
The REST API provides configurable control over this replication retry logic using a set of configiurable properties -- see: xref:csharp:p2psync-websocket-using-active.adoc#tbl-repl-retry[].
...
When necessary you can adjust any or all of those configurable values -- see: xref:csharp:p2psync-websocket-using-active.adoc#ex-repl-retry[] for how to do this.
...
[source, C#]
----
include::csharp:example$code_snippets/Program.cs[tags=p2p-act-rep-config-cacert, indent=0]
----
<.> This is the default.
...
[source, C#]
----
include::csharp:example$code_snippets/Program.cs[tags=p2p-act-rep-config-self-cert, indent=0]
----
...
[source, C#]
----
include::csharp:example$code_snippets/Program.cs[tag=p2p-act-rep-config-cacert-pinned, indent=0]
----
// include::csharp:example$code_snippets/Program.cs[tag=p2p-act-rep-config-cacert-pinned-callouts, indent=0]
--
...
You can, optionally, add a change listener (see xref:csharp:p2psync-websocket-using-active.adoc#lbl-repl-mon[]) before starting the replicator running using https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.Sync.Replicator.html#Couchbase_Lite_Sync_Replicator_Start[Start()].
...
In this section::
xref:csharp:p2psync-websocket-using-active.adoc#lbl-repl-chng[]  |
xref:csharp:p2psync-websocket-using-active.adoc#lbl-repl-status[]  |
xref:csharp:p2psync-websocket-using-active.adoc#lbl-repl-pend[]

You can monitor a replication’s status by using a combination of xref:csharp:p2psync-websocket-using-active.adoc#lbl-repl-chng[] and the `replication.status.activity` property -- see; https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.Sync.ReplicatorStatus.html#Couchbase_Lite_Sync_ReplicatorStatus_Activity[Activity].
...
That is, whether it is actively transferring data or if it has stopped -- see: xref:csharp:p2psync-websocket-using-active.adoc#ex-repl-mon[].

The returned _ReplicationStatus_ structure comprises:

* https://docs.couchbase.com/mobile/{major}.{minor}.{maintenance-net}{empty}/couchbase-lite-net/api/Couchbase.Lite.Sync.ReplicatorStatus.html#Couchbase_Lite_Sync_ReplicatorStatus_Activity[Activity] -- stopped, offline, connecting, idle or busy -- see states described in: xref:csharp:p2psync-websocket-using-active.adoc#tbl-states[]
...
[source, C#]
----
include::csharp:example$code_snippets/Program.cs[tags=p2p-act-rep-add-change-listener;!p2p-act-rep-add-change-listener-label,indent=0]
----
--
+
...
[source, C#]
----
include::csharp:example$code_snippets/Program.cs[tags=p2p-act-rep-status,indent=0]
----
--
======
...
[#lbl-repl-states]
=== Replication States
xref:csharp:p2psync-websocket-using-active.adoc#tbl-states[] shows the different states, or activity levels, reported in the API; and the meaning of each.
...
It will stop any active xref:csharp:p2psync-websocket-using-active.adoc#lbl-repl-chng[change listener] once the replication is stopped.
...
To address that possibility, a version of the `Database.save()` method also takes a conflict resolver as shown in xref:csharp:p2psync-websocket-using-active.adoc#ex-merge-props[].
